...
public:
    ...
    const proto::FileDescriptor * Import( const std::string & filename ) { return m_pool.FindFileByName( filename ); }

    void AddError( const std::string & filename, int line, int column, const std::string & message ) override
    {
        CSP_THROW( RuntimeException,
                   "Failed to load proto schema " << filename << ":" << line << ":" << column << ": " << message );
    }
...
proto::FieldDescriptor::CppType ProtobufHelper::cspToProtoCppType( const CspType & type )
{
    switch( type.type() )
    {
        ...
        case CspType::Type::BOOL:
            return proto::FieldDescriptor::CPPTYPE_BOOL;
        case CspType::Type::INT32:
            return proto::FieldDescriptor::CPPTYPE_INT32;
        case CspType::Type::UINT32:
            return proto::FieldDescriptor::CPPTYPE_UINT32;
        case CspType::Type::INT64:
            return proto::FieldDescriptor::CPPTYPE_INT64;
        case CspType::Type::UINT64:
            return proto::FieldDescriptor::CPPTYPE_UINT64;
        case CspType::Type::DOUBLE:
            return proto::FieldDescriptor::CPPTYPE_DOUBLE;
        case CspType::Type::STRING:
            return proto::FieldDescriptor::CPPTYPE_STRING;
        case CspType::Type::STRUCT:
            return proto::FieldDescriptor::CPPTYPE_MESSAGE;
...
bool ProtobufHelper::isCoercible( proto::FieldDescriptor::CppType src, CspType::Type dest )
{
    switch( src )
    {
        case proto::FieldDescriptor::CPPTYPE_INT32:
            return ( dest == CspType::Type::UINT32 || dest == CspType::Type::INT64 || dest == CspType::Type::UINT64 );
        case proto::FieldDescriptor::CPPTYPE_UINT32:
            return ( dest == CspType::Type::INT32 || dest == CspType::Type::INT64 || dest == CspType::Type::UINT64 );
        case proto::FieldDescriptor::CPPTYPE_INT64:
            return dest == CspType::Type::UINT64;
        case proto::FieldDescriptor::CPPTYPE_UINT64:
            return dest == CspType::Type::INT64;
        case proto::FieldDescriptor::CPPTYPE_FLOAT:
            return dest == CspType::Type::DOUBLE;
        case proto::FieldDescriptor::CPPTYPE_ENUM:
            return dest == CspType::Type::STRING;
        default:
            return false;
    }
}
...
template<>
int32_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message,
                                      const proto::FieldDescriptor * descr, int index )
{
    switch( descr->cpp_type() )
    {
        case proto::FieldDescriptor::CPPTYPE_INT32:
            return index == -1 ? access->GetInt32( message, descr ) : access->GetRepeatedInt32( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_UINT32:
        {
            uint32_t v = index == -1 ? access->GetUInt32( message, descr )
                                     : access->GetRepeatedUInt32( message, descr, index );
            if( v > uint32_t( std::numeric_limits<int32_t>::max() ) )
                CSP_THROW( RangeError,
                           "coercion out of range for UINT32 value into INT32 value for proto msg type "
                               << message.GetTypeName() << " field " << descr->name() );
            return (int32_t)v;
        }
        default:
...
template<>
uint32_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message,
                                       const proto::FieldDescriptor * descr, int index )
{
    switch( descr->cpp_type() )
    {
        case proto::FieldDescriptor::CPPTYPE_UINT32:
            return index == -1 ? access->GetUInt32( message, descr )
                               : access->GetRepeatedUInt32( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_INT32:
        {
            int32_t v
                = index == -1 ? access->GetInt32( message, descr ) : access->GetRepeatedInt32( message, descr, index );
            if( v < 0 )
                CSP_THROW( RangeError,
                           "coercion out of range for INT32 value into uint32_t value for proto msg type "
                               << message.GetTypeName() << " field " << descr->name() );
            return (uint32_t)v;
        }
        default:
            CSP_THROW( TypeError,
                       "Attempting to coerce proto field type " << descr->cpp_type_name() << " to uint32_t" );
    }
}

template<>
int64_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message,
                                      const proto::FieldDescriptor * descr, int index )
{
    switch( descr->cpp_type() )
    {
        case proto::FieldDescriptor::CPPTYPE_INT64:
            return index == -1 ? access->GetInt64( message, descr ) : access->GetRepeatedInt64( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_UINT64:
        {
            uint64_t v = index == -1 ? access->GetUInt64( message, descr )
                                     : access->GetRepeatedUInt64( message, descr, index );
            if( v > uint64_t( std::numeric_limits<int64_t>::max() ) )
                CSP_THROW( RangeError,
                           "coercion out of range for UINT64 value into int64_t value for proto msg type "
                               << message.GetTypeName() << " field " << descr->name() );
            return (int64_t)v;
        }
        case proto::FieldDescriptor::CPPTYPE_UINT32:
...
template<>
uint64_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message,
                                       const proto::FieldDescriptor * descr, int index )
{
    switch( descr->cpp_type() )
    {
        case proto::FieldDescriptor::CPPTYPE_UINT64:
            return index == -1 ? access->GetUInt64( message, descr )
                               : access->GetRepeatedUInt64( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_INT64:
        {
            int64_t v
                = index == -1 ? access->GetInt64( message, descr ) : access->GetRepeatedInt64( message, descr, index );
            if( v < 0 )
                CSP_THROW( RangeError,
                           "coercion out of range for INT64 value into uint64_t value for proto msg type "
                               << message.GetTypeName() << " field " << descr->name() );
            return (uint64_t)v;
        }
        case proto::FieldDescriptor::CPPTYPE_UINT32:
        ...
        case proto::FieldDescriptor::CPPTYPE_INT32:
        {
            ...
            if( v < 0 )
                CSP_THROW( RangeError,
                           "coercion out of range for INT32 value into uint64_t value for proto msg type "
                               << message.GetTypeName() << " field " << descr->name() );
            return (uint64_t)v;
        }
...
template<>
double ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message,
                                     const proto::FieldDescriptor * descr, int index )
{
    switch( descr->cpp_type() )
    {
        ...
        case proto::FieldDescriptor::CPPTYPE_UINT64:
            return index == -1 ? access->GetUInt64( message, descr )
                               : access->GetRepeatedUInt64( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_FLOAT:
            return index == -1 ? access->GetFloat( message, descr ) : access->GetRepeatedFloat( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_DOUBLE:
            return index == -1 ? access->GetDouble( message, descr )
                               : access->GetRepeatedDouble( message, descr, index );
...
void ProtobufStructMapper::init( const CspTypePtr & type, const Dictionary & fieldMap,
                                 const google::protobuf::Descriptor * protoDesc )
{
    m_fields = buildFields( type, fieldMap, protoDesc );
}

ProtobufStructMapper::Fields ProtobufStructMapper::buildFields( const CspTypePtr & type, const Dictionary & fieldMap,
                                                                const google::protobuf::Descriptor * protoDesc )
{
    ...
    for( auto it = fieldMap.begin(); it != fieldMap.end(); ++it )
    {
        ...
        else
        {
            ...
            if( subdict->size() != 1 )
                CSP_THROW( ValueError,
                           "Expected sub-fieldmap for incoming protofield " << protoField
        ...
        if( !protoFieldDesc )
            CSP_THROW( ValueError,
                       "field '" << protoField << "' is not a valid field on proto descriptor for "
                                 << protoDesc->name() );

        auto protoType = ProtobufHelper::cspToProtoCppType( *sField->type() );
        if( ( protoType != protoFieldDesc->cpp_type()
              && !ProtobufHelper::isCoercible( protoFieldDesc->cpp_type(), sField->type()->type() ) )
            || ( sField->type()->type() == CspType::Type::ARRAY && !protoFieldDesc->is_repeated() ) )
        {
            CSP_THROW( TypeError,
                       "Type mismatch on struct field " << structField << " with type " << sField->type()->type()
                                                        << " and protofield " << protoField << " with type "
                                                        << ( protoFieldDesc->is_repeated() ? "repeated " : "" )
                                                        << protoFieldDesc->cpp_type_name() );
        }
        ...
        if( sField->type()->type() == CspType::Type::STRUCT )
        {
            if( !subFieldMap )
                CSP_THROW( ValueError,
                           "invalid field_map entry for sub-struct field " << sField->fieldname() << " on struct type "
                                                                           << structMeta->name() );
            subfields = std::make_shared<Fields>(
                buildFields( sField->type(), *subFieldMap, protoFieldDesc->message_type() ) );
        }
...
using SupportedArrayCspTypeSwitch
    = PartialSwitchCspType<csp::CspType::Type::BOOL, csp::CspType::Type::INT32, csp::CspType::Type::UINT32,
...
void ProtobufStructMapper::mapProtoToStruct( StructPtr & struct_, const proto::Message & protoMsg,
                                             const Fields & fields )
{
    ...
    for( auto & entry : fields )
    {
        ...
        {
            case CspType::Type::BOOL:
                sField->setValue<bool>( struct_.get(), protoAccess->GetBool( protoMsg, pField ) );
                break;
            case CspType::Type::INT32:
                sField->setValue<int32_t>( struct_.get(),
                                           ProtobufHelper::coercedValue<int32_t>( protoAccess, protoMsg, pField ) );
                break;
            case CspType::Type::UINT32:
                sField->setValue<uint32_t>( struct_.get(),
                                            ProtobufHelper::coercedValue<uint32_t>( protoAccess, protoMsg, pField ) );
                break;
            case CspType::Type::INT64:
                sField->setValue<int64_t>( struct_.get(),
                                           ProtobufHelper::coercedValue<int64_t>( protoAccess, protoMsg, pField ) );
                break;
            case CspType::Type::UINT64:
                sField->setValue<uint64_t>( struct_.get(),
                                            ProtobufHelper::coercedValue<uint64_t>( protoAccess, protoMsg, pField ) );
                break;
            case CspType::Type::DOUBLE:
                sField->setValue<double>( struct_.get(),
                                          ProtobufHelper::coercedValue<double>( protoAccess, protoMsg, pField ) );
                break;
            case CspType::Type::STRING:
            {
            ...
            case CspType::Type::ARRAY:
            {
                ...
                SupportedArrayCspTypeSwitch::invoke(
                    elemType.get(),
                    [&count, &protoMsg, &pField, &sField, &struct_]( auto tag )
                    {
            ...
            default:
                CSP_THROW( TypeError,
                           "Struct field type " << sField->type()->type() << " not currently mappable to proto field" );
        }
    }
}
