...
public:
  ...
  DynamicBitSet(DynamicBitSet &&other)
      : m_size(other.m_size), m_numNodes(other.m_numNodes) {
  ...
  void resize(index_type size) {
    ...
    if (newNodes > m_numNodes) {
      ...
      memset(m_nodes + m_numNodes, 0,
             (newNodes - m_numNodes) * sizeof(node_type));
...
private:
  ...
  std::pair<index_type, nbit_type> locateValue(index_type value) const {
    return std::pair<index_type, nbit_type>(value >> _logBits,
                                            value & (_bits - 1));
  }
