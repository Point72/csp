...
inline std::string dirname( const std::string & path ) { return std::filesystem::path( path ).parent_path().string(); }

// files and directories are treated equally
inline bool fileExists( const std::string & fileOrDir ) { return std::filesystem::exists( fileOrDir ); }
