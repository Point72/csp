...
// clz (count leading zeros) returns number of leading zeros before MSB (i.e.
// clz(00110..) = 2 )
// __builtin_clz auto-promotes to 32-bits: need to subtract off extra leading
// zeros
inline constexpr uint8_t clz(uint16_t n) {
  return clz(static_cast<uint32_t>(n)) - 16;
}
...
// ffs (find first set) returns offset of first set bit (i.e. ffs(..0110) = 2 ),
// with ffs(0) = 0
template <typename U, std::enable_if_t<std::is_unsigned<U>::value, bool> = true>
