...
TimeSeriesProvider * PyInputProxy::ts() const { return const_cast<TimeSeriesProvider *>( m_node->input( m_id ) ); }

bool PyInputProxy::ticked() const { return m_node->inputTicked( m_id ); }

bool PyInputProxy::valid() const { return ts()->valid(); }

uint32_t PyInputProxy::count() const { return ts()->count(); }

uint32_t PyInputProxy::num_ticks() const { return ts()->numTicks(); }

bool PyInputProxy::makeActive() { return m_node->makeActive( m_id ); }

bool PyInputProxy::makePassive() { return m_node->makePassive( m_id ); }
