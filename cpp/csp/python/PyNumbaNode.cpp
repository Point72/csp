...
extern "C" CSPTYPESIMPL_EXPORT int64_t csp_numba_struct_enum_field_value( const void * struct_ptr, int64_t field_offset )
{
    ...
    const char * bytes = static_cast<const char *>( struct_ptr );
    const auto * enumField = reinterpret_cast<const CspEnum *>( bytes + field_offset );
...
void PyNumbaNode::clearOutputTicked()
{
