        ...
        while( PyDict_Next( metadata, &pos, &key, &type ) )
        {
            ...
            switch( csptype->type() )
            {
                ...
                case csp::CspType::Type::ARRAY:
                {
                    ...
                    field                          = switchCspType( arrayType.elemType(),
...
void repr_array( const std::vector<ElemT> & val, const CspArrayType & arrayType, std::string & tl_repr,
                 bool show_unset );
    ...
    switch( type )
    {
        ...
        case CspType::Type::ARRAY:
        {
            ...
            switchCspType( elemType,
                           [field, struct_, &arrayType, &tl_repr, show_unset]( auto tag )
                           {
                               using CElemType       = typename decltype( tag )::type;
                 ...
                 bool show_unset )
{
    ...
    for( auto it = val.begin(); it != val.end(); it++ )
    {
        ...
        else if constexpr( std::is_same<ElemT, StructPtr>::value )
        {
            ...
            repr_array( *it, static_cast<const CspArrayType &>( arrayType.elemType() ), tl_repr,
                        show_unset ); // recursive, allows for nested arrays!
        else
        {
...
PyObject * PyStruct_all_fields_set( PyStruct * self ) { return toPython( self->struct_->allFieldsSet() ); }
