...
//AdapterManager
csp::AdapterManager * create_csv_adapter_manager( PyEngine * engine, const Dictionary & properties )
{
    return engine -> engine() -> createOwnedObject<CsvInputAdapterManager>( properties );
}

static InputAdapter * create_csv_input_adapter( csp::AdapterManager * manager, PyEngine * pyengine, PyObject * pyType, PushMode pushMode, PyObject * args )
{
    auto & cspType = pyTypeAsCspType( pyType );
    ...
    return csvManager -> getInputAdapter( cspType, fromPython<Dictionary>( pyProperties ), pushMode );
}

REGISTER_ADAPTER_MANAGER( _csv_adapter_manager, create_csv_adapter_manager );
REGISTER_INPUT_ADAPTER(   _csv_input_adapter,   create_csv_input_adapter );

static PyModuleDef _csvadapterimpl_module = {
...
>>>>>>> 6bf96b2 (Create linker and update cmakes)
