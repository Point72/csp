...
{
public:
    AddNode( Engine * engine )
        : Node( NodeDef( 2, 1 ), engine )
    {
    ...
    void executeImpl()
    {
        ...
        {
            double sum
                = input( InputId( 0 ) )->lastValueTyped<double>() + input( InputId( 1 ) )->lastValueTyped<double>();
...
{
public:
    WriteNode( Engine * engine, std::string tag )
        : Node( NodeDef( 1, 0 ), engine )
        , m_tag( tag )
    {
    }
    ...
    void execute()
    {
        // std::cerr << engine() -> time() << ": " << m_tag << " " << input( InputId( 0 ) ) -> lastValueTyped<double>()
        // << std::endl;
