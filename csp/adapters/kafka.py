...
from csp.adapters.utils import BytesMessageProtoMapper, DateTimeType, JSONTextMessageMapper, RawBytesMessageMapper, RawTextMessageMapper, MsgMapper
...
class KafkaAdapterManager:
    def __init__( self, broker,
                  ...
                  group_id : str = None,
                  group_id_prefix : str = '',
                  max_threads=4, max_queue_size=1000000,
                  auth = False,
        ...
        producer_properties = {
            'queue.buffering.max.messages': str(max_queue_size)
        }
    ...
    def subscribe(self, ts_type: type, msg_mapper: MsgMapper, topic,
    ...
    def publish(self, msg_mapper: MsgMapper, topic: str, key: str, x: ts['T'], field_map: typing.Union[dict, str] = None):
        if isinstance(field_map, str):
            field_map = {'': field_map}
...
_kafka_input_adapter_def = input_adapter_def('kafka_input_adapter', _kafkaadapterimpl._kafka_input_adapter, ts['T'], KafkaAdapterManager,
                                             typ='T', properties=dict)
_kafka_output_adapter_def = output_adapter_def('kafka_output_adapter', _kafkaadapterimpl._kafka_output_adapter, KafkaAdapterManager,
