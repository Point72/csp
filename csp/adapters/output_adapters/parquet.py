...
class ParquetOutputConfig(csp.Struct):
...
class ParquetWriter:
    ...
    def __init__(self,
        ...
        if file_name and os.path.exists(file_name):
            if split_columns_to_files:
                assert os.path.isdir(file_name), f"split_columns_to_files is True, but existing past {file_name} is file, not folder"
            else:
                assert os.path.isfile(file_name), f"split_columns_to_files is False, but existing past {file_name} is folder, not file"

        self._all_column_names = set()
        self._properties = {'file_name': file_name if file_name else '',
    ...
    def publish_struct(self, value: ts[csp.Struct], field_map: Dict[str, str] = None):
    ...
    def publish(self, column_name, value: ts[object], array_dimensions_column_name=None):
        ...
        if CspTypingUtils.is_numpy_array_type(ts_type):
            if CspTypingUtils.get_origin(ts_type) is csp.typing.NumpyNDArray:
                ...
                array_dimensions_column_name = resolve_array_shape_column_name(column_name, array_dimensions_column_name)
        ...
        return _parquet_output_adapter_def(self, value, ContainerTypeNormalizer.normalized_type_to_actual_python_type(ts_type), properties)
    ...
    def _parquet_dict_basket_writer(self):
        ...
        def _parquet_dict_basket_writer(column_name: str, writer: object, input: {'K': csp.ts['V']},
                                        filename_provider: csp.ts[str]):
    ...
    def publish_dict_basket(self, column_name, value, key_type, value_type):
        ...
        if CspTypingUtils.is_generic_container(value_type) and CspTypingUtils.get_orig_base(value_type) is numpy.ndarray:
            raise NotImplementedError("Writing of baskets with array values is not supported")

        return self._parquet_dict_basket_writer(column_name=column_name, writer=self, input=value, filename_provider=filename_provider)
    ...
    def _create(self, engine, memo):
        ...
        self._parquet_output_adapter_manager = _parquetadapterimpl._parquet_output_adapter_manager(engine, self._properties)
        return self._parquet_output_adapter_manager


_parquet_output_adapter_def = output_adapter_def('parquet_output_adapter', _parquetadapterimpl._parquet_output_adapter, ParquetWriter,
                                                 input=ts['T'], typ='T', properties=dict)
_parquet_output_filename_adapter_def = output_adapter_def('parquet_output_filename_adapter', _parquetadapterimpl._parquet_output_filename_adapter, ParquetWriter,
                                                          input=ts[str])
