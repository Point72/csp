...
from csp.impl.wiring import input_adapter_def, py_managed_adapter_def, status_adapter_def
...
class ParquetReader:
    def __init__(self, filename_or_list, symbol_column=None, time_column=None, binary_arrow=False, tz=None,
                 start_time=None, end_time=None,
                 split_columns_to_files=False, read_from_memory_tables=False, allow_overlapping_periods=False,
                 time_shift: datetime.timedelta = None, allow_missing_columns=False, allow_missing_files=False):
        """
        ...
        if read_from_memory_tables:
            ...
            self._filenames_gen = lambda starttime, endtime: self._arrow_in_memory_table_to_buffers(wrapped, starttime, endtime)
            binary_arrow = True
    ...
    @node
    def _reconstruct_struct_array_fields(self, s: csp.ts['T'], fields: {str: csp.ts[object]}, struct_typ: 'T') -> csp.ts['T']:
        if csp.ticked(s):
            res = s
        else:
            res = struct_typ()
    ...
    def _subscribe_impl(self, symbol, typ, field_map, push_mode, basket_name=None, array_dimensions_column_name=None):
        ...
        if CspTypingUtils.is_generic_container(typ) and CspTypingUtils.get_orig_base(typ) is numpy.ndarray:
            ...
            if CspTypingUtils.get_origin(typ) is csp.typing.NumpyNDArray:
                ...
                flat_values = self._subscribe_impl(symbol=symbol, typ=csp.typing.Numpy1DArray[typ.__args__[0]],
                                                   field_map=field_map, push_mode=push_mode)
                shape = self._subscribe_impl(symbol=symbol, typ=csp.typing.Numpy1DArray[int],
                                             field_map=array_dimensions_column_name, push_mode=push_mode)
    ...
    def subscribe(self, symbol, typ, field_map=None, push_mode: csp.PushMode = csp.PushMode.NON_COLLAPSING, array_dimensions_column_name=None):
        ...
        column name.
        """
        return self._subscribe_impl(symbol, typ, field_map, push_mode, array_dimensions_column_name=array_dimensions_column_name)

    def subscribe_all(self, typ, field_map=None, push_mode: csp.PushMode = csp.PushMode.NON_COLLAPSING, array_dimensions_column_name=None):
        ...
        return self.subscribe('', typ, field_map, push_mode=push_mode, array_dimensions_column_name=array_dimensions_column_name)

    def subscribe_dict_basket(self, typ, name, shape, push_mode: csp.PushMode = csp.PushMode.NON_COLLAPSING):
        return {v: self._subscribe_impl(v, typ, None, push_mode, name) for v in shape}

    def subscribe_dict_basket_struct_column(self, typ, name, shape, field_name, push_mode: csp.PushMode = csp.PushMode.NON_COLLAPSING):
...
_parquet_input_adapter_def = input_adapter_def('parquet_adapter', _parquetadapterimpl._parquet_input_adapter, ts['T'],
                                               ParquetReader, typ='T', properties=dict)
