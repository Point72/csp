...
try:
    from perspective import (
        PerspectiveManager, set_threadpool_size, __version__,
        Table as Table_, View as View_
    )
...
class PerspectiveAdapter(DelayedNodeWrapperDef):
    ...
    def _instantiate(self):
        ...
        for table_name,table in self._tables.items():
            schema = { k : v.tstype.typ if not issubclass(v.tstype.typ,csp.Enum) else str for k,v in table.columns.items() }
