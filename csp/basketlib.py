...
def sync_list(x: [ts['T']], threshold: timedelta, output_incomplete: bool = True) -> csp.OutputBasket(List[ts['T']], shape_of='x'):
    with csp.alarms():
...
@csp.graph
def sync_dict(x: {'K': ts['T']}, threshold: timedelta, output_incomplete: bool = True) -> csp.OutputBasket(Dict['K', ts['T']], shape_of='x'):
