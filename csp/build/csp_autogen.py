...
_GEN_COMMAND = f"""// AUTOGENERATED BY CSP_AUTOGEN
// DO NOT MODIFY DIRECTLY
// command: python {" ".join(sys.argv)}
"""


def _normalize_generated_code(code):
...
class CodeGenerator:
    def __init__(
        self, module_name: str, output_filename: str, namespace: str, generate_imported_types: bool, omit_asserts: bool
    ):
        ...
        self._omit_asserts = omit_asserts
        ...
        for v in self._module.__dict__.values():
...
"""
    ...
    def _generate_struct_class(self, struct_type):
        ...
        if base_struct:
            ...
            base_fields = {f["fieldname"] for f in base_metainfo["fields"]}
        ...
        for field in fields:
            ...
            field_alignment_assert = f"static_assert( alignof( {ctype} ) == {field_alignment} );"
            field_size_assert = f"static_assert( sizeof( {ctype} ) == {field_size} );"
            ...
            common_setter = f"""
        {"" if self._omit_asserts else field_offset_assert}
        {"" if self._omit_asserts else field_alignment_assert}
        {"" if self._omit_asserts else field_size_assert}
...
"""
    ...
    {{
        {"" if self._omit_asserts else mask_offset_assert}
...
{decls}
    ...
    {{
        {"" if self._omit_asserts else maskloc_offset_assert}
    }}
}};
"""
        return out
...
{{
{"" if self._omit_asserts else assertions}
    if( Py_IsInitialized() )
    {{
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="pass flag to omit static size assertions from generated code",
    )
    parser.add_argument(
    ...
    try:
        struct_gen = CodeGenerator(
            args.module_name, args.outname, args.namespace, bool(args.generate_imports), bool(args.omit_asserts)
        )
        ...
        header_code = _normalize_generated_code(struct_gen.generate_header_code())
        cpp_code = _normalize_generated_code(struct_gen.generate_cpp_code())
        ...
        logger.exception("Error running `csp_autogen`. Your build may fail!")
