...
class DataFrame:
    ...
    def __getitem__(self, columns):
        if isinstance(columns, Edge):
            if columns.tstype.typ is not bool:
                raise KeyError("csp.DataFrame access by edge expected ts[bool] got ts[" + columns.tstype.typ.__name__ + "]")
    ...
    def to_perspective(self, starttime: datetime, endtime: datetime = None, realtime: bool = False):
        ...
        runner = csp.run_on_thread(apply_updates, table, self._data, timecol, timedelta(seconds=0.5), starttime=starttime, endtime=endtime, realtime=True)
        widget = RealtimePerspectiveWidget(runner, table,
                                           plugin='Y Line',
