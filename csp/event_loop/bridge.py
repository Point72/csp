"""
CSP Event Loop Bridge Module

This module provides integration between asyncio and running CSP graphs.
Unlike csp.event_loop (which provides an asyncio event loop backed by CSP),
this module allows asyncio operations to interleave with a running CSP graph.

Key features:
- Push data from asyncio callbacks into CSP graphs via GenericPushAdapter
- Schedule callbacks with call_later/call_at that align with CSP time
- Run asyncio coroutines that interact with CSP graphs
- Bidirectional communication between asyncio and CSP

Usage:
    from csp.event_loop.bridge import AsyncioBridge
    ...
    bridge.call_at(some_datetime, lambda: bridge.push(100))
