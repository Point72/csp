"""
CSP Asyncio Event Loop Implementation

This module implements a Python asyncio-compatible event loop backed by CSP's scheduler.
"""
...
class CspEventLoop(asyncio.AbstractEventLoop):
    ...
    def _init_csp_engine(self) -> None:
        """Initialize the CSP engine for stepping."""
        if self._csp_engine is None:
            self._csp_engine = _cspimpl.PyEngine(realtime=self._realtime)

    def _start_csp_stepping(self) -> None:
        """Start CSP engine in step mode."""
        if not self._csp_stepping:
            ...
            self._csp_engine.start_stepping(start, end)
            self._csp_stepping = True
            self._sim_start_time = start  # Track simulation start for time()

    def _stop_csp_stepping(self) -> None:
        """Stop CSP engine stepping."""
        if self._csp_stepping and self._csp_engine is not None:
            try:
                self._csp_engine.stop_stepping()
            except Exception:
    ...
    def run_forever(self) -> None:
        ...
        try:
            ...
            self._start_csp_stepping()
            self._run_until_stopped()
        finally:
            self._stop_csp_stepping()
    ...
    def _run_once_realtime(self, timeout: Optional[float] = None) -> None:
        ...
        # Also check CSP's next scheduled time if stepping
        if self._csp_stepping and self._csp_engine is not None:
        ...
        # Step CSP engine if active (process any CSP scheduled events)
        if self._csp_stepping and self._csp_engine is not None:
            try:
                # Step with 0 wait - just process what's ready
                self._csp_engine.step(timedelta(0))
            except Exception:
    ...
    def time(self) -> float:
        ...
        """
        if not self._realtime and self._csp_stepping and self._csp_engine is not None:
    ...
    # =========================================================================
    # Subprocess methods (basic implementation)
    # =========================================================================
