...
def my_graph():
    ...
    bid = csp.curve(float, [(st + timedelta(seconds=.5), 99.0),
                            (st + timedelta(seconds=1.5), 99.1),
                            (st + timedelta(seconds=5), 99.2)])

    ask = csp.curve(float, [(st + timedelta(seconds=.6), 99.1),
                            (st + timedelta(seconds=1.3), 99.2),
                            (st + timedelta(seconds=4.2), 99.25)])

    trades = csp.curve(Trade, [(st + timedelta(seconds=1), Trade(price=100.0, qty=50, buy=True)),
