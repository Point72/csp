...
if __name__ == '__main__':
    with tempfile.NamedTemporaryFile(suffix='.parquet') as struct_file:
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet') as series_file:
            ...
            g = csp.run(my_graph, struct_file.name, series_file.name,
                        starttime=datetime(2020, 1, 1), endtime=timedelta(minutes=1))

            try:
                import pandas
            except ModuleNotFoundError:
                logging.warning("Pandas is not installed, not loading the dataframes, for fully functional example, consider installing pandas")
            else:
