...
@csp.graph
def process_symbol(symbol: str, order: ts[Order], initial_order: Order,
                   timer: ts[int], scalar: str) -> ts[int]:
...
def main_graph():
    ...
    # dynamic graphs
    cum_sizes = csp.dynamic(trigger, process_symbol,
                            ...
                            csp.snap(orders),  # csp.snap will provide the current value of the given timeseries at the time of dynamic graph instantiation
...
realtime = True
csp.run(main_graph, starttime=datetime.utcnow().replace(microsecond=0), endtime=timedelta(seconds=10), realtime=realtime)
