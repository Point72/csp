"""
Async adapter for bridging Python's asyncio async generators with CSP's synchronous graph processing.
"""
...
# ============================================================================
# Shared Async Loop: Global singleton for running async operations
# ============================================================================
...
def get_csp_asyncio_loop() -> Optional[asyncio.AbstractEventLoop]:
    ...
    Returns:
        The asyncio loop if csp.run() was called with use_asyncio=True, else None.
    """
...
def is_csp_asyncio_mode() -> bool:
    ...
    Returns:
        True if csp.run() was called with use_asyncio=True, else False.
    """
...
def get_async_loop() -> asyncio.AbstractEventLoop:
    ...
    Priority:
    1. If csp.run() was called with use_asyncio=True, use that loop
...
def _run_on_async_loop(coro: Awaitable[T], timeout: Optional[float] = None) -> T:
    ...
    """
    # Check if we're already inside an asyncio context
    running_loop = get_running_loop_or_none()
    if running_loop is not None:
        # We're inside an async context - can't block, but can schedule
        # This case shouldn't happen for blocking await_(), caller should use await directly
        raise RuntimeError(
            "Cannot use blocking await_() inside an async context. Use 'await' directly instead, or use block=False."
        )

    # We're in sync context - use the shared loop
...
# ============================================================================
# async_in: Run a coroutine and tick when it completes
# ============================================================================
...
# ============================================================================
# async_out: Invoke an async function when CSP input ticks
# ============================================================================
...
# ============================================================================
# async_node: Take CSP input, run async function, output result
# ============================================================================
...
# ============================================================================
# await_: Await an async coroutine from synchronous code (e.g., inside a node)
# ============================================================================
...
# ============================================================================
# AsyncContext: Context manager for async operations within nodes
# ============================================================================
...
# ============================================================================
# AsyncAlarm: Alarm-like pattern for async operations within nodes
# ============================================================================
