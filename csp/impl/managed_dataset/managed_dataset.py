...
from csp.impl.managed_dataset.cache_partition_argument_serializer import SerializedArgument, StructPartitionArgumentSerializer
...
class _MetadataRWUtil:
    ...
    def _write_metadata(self):
        ...
        with self._lock_file_util.write_lock(locked_folder) as lock:
...
class ManagedDatasetPartition:
    ...
    def get_data_for_period(self, starttime: datetime, endtime: datetime, missing_range_handler):
        return self.data_paths.get_data_files_in_range(starttime, endtime, missing_range_handler=missing_range_handler,
    ...
    def publish_file(self, file_name, start_time, end_time, file_permissions=None, lock_file_permissions=None):
        output_file_name = self.data_paths.get_output_file_name(start_time, end_time, split_columns_to_files=self.dataset.metadata.split_columns_to_files)
    ...
    def merge_files(self, start_time: datetime, end_time: datetime, cache_config, parquet_output_config):
        ...
        with self.dataset.use_lock_context():
            file_merger = SinglePartitionFileMerger(dataset_partition=self, start_time=start_time, end_time=end_time,
    ...
    def cleanup_unneeded_files(self, start_time: datetime, end_time: datetime, cache_config):
        unused_files = self.data_paths.get_unused_files(starttime=start_time, endtime=end_time, split_columns_to_files=self.dataset.metadata.split_columns_to_files)
        if unused_files:
            with self.dataset.use_lock_context():
                lock_util = ManagedDatasetLockUtil(cache_config.lock_file_permissions)
                for f in unused_files:
                    try:
                        with lock_util.write_lock(f, is_lock_in_root_folder=True, timeout_seconds=0, retry_period_seconds=0) as lock:
                            rm_file_or_folder(f)
                            lock.delete_file()
...
class ManagedDataset:
    ...
    def __init__(self,
        ...
        self._metadata = DatasetMetadata(name=name,
    ...
    def load_from_disk(cls, cache_config, name, data_category: Optional[List[str]] = None):
        data_paths = DatasetPaths(parent_folder=cache_config.data_folder,
                                  read_folders=getattr(cache_config, 'read_folders', None), name=name, data_category=data_category)
        metadata_file_path = data_paths.get_metadata_file_path(existing = True)
        if metadata_file_path:
            with open(metadata_file_path, 'r') as f:
                metadata = DatasetMetadata.from_yaml(f.read())
            res = ManagedDataset(name=metadata.name,
    ...
    def is_supported_partition_type(cls, typ):
        if typ in ManagedDataset.SUPPORTED_PARTITION_TYPES or (isinstance(typ, type) and (issubclass(typ, Enum) or issubclass(typ, Struct))):
            return True
        else:
            return False
    ...
    def validate_and_lock_metadata(self, lock_file_permissions: Optional[FilePermissions] = None,
        ...
        with self.use_lock_context():
            ...
            metadata_rw_util = _MetadataRWUtil(dataset=self,
            ...
            if is_metadata_different:
                read_lock.unlock()
                raise RuntimeError(f"Metadata mismatch at {metadata_file_path}\nCurrent:\n{metadata}\nExisting:{existing_meta}\n")
            return read_lock
    ...
    def _set_folders(self, parent_folder, read_folders):
        ...
        if parent_folder:
            self._data_paths = DatasetPaths(parent_folder=parent_folder,
                                            read_folders=read_folders,
                                            name=self._name, data_category=self._category, time_aggregation=self.metadata.time_aggregation)
        else:
