...
class LockContext:
    ...
    def resolve_lock_file_path_and_create_folders(self, file_path: str, use_read_folders: bool):
        parent_folder, lock_file_path = self._data_set.data_paths.resolve_lock_file_path(file_path, use_read_folders=use_read_folders)
        ...
        create_folder_with_permissions(os.path.dirname(lock_file_path), self._data_set.cache_config.lock_file_permissions)
...
class ManagedDatasetLockUtil:
    ...
    def _create_lock(self, file_path, lock_name, shared, is_lock_in_root_folder, timeout_seconds, retry_period_seconds):
        ...
        if os.path.isfile(file_path) or is_lock_in_root_folder:
            ...
            return LockFile(file_path=cur_context.resolve_lock_file_path_and_create_folders(os.path.join(dir_name, lock_file_name), use_read_folders=shared),
                            shared=shared,
                            file_permissions=self._lock_file_permissions,
                            timeout_seconds=timeout_seconds,
                            retry_period_seconds=retry_period_seconds)
        else:
            return LockFile(file_path=cur_context.resolve_lock_file_path_and_create_folders(os.path.join(file_path, lock_name), use_read_folders=shared),
                            shared=shared,
                            file_permissions=self._lock_file_permissions,
                            timeout_seconds=timeout_seconds,
                            retry_period_seconds=retry_period_seconds)
    ...
    def write_lock(self, file_path, is_lock_in_root_folder=None, timeout_seconds=None, retry_period_seconds=None):
        return self._create_lock(file_path, lock_name=self._READ_WRITE_LOCK_FILE_NAME, shared=False, is_lock_in_root_folder=is_lock_in_root_folder,
                                 timeout_seconds=timeout_seconds, retry_period_seconds=retry_period_seconds)

    def read_lock(self, file_path, is_lock_in_root_folder=None, timeout_seconds=None, retry_period_seconds=None):
        return self._create_lock(file_path, lock_name=self._READ_WRITE_LOCK_FILE_NAME, shared=True, is_lock_in_root_folder=is_lock_in_root_folder,
                                 timeout_seconds=timeout_seconds, retry_period_seconds=retry_period_seconds)

    def merge_lock(self, file_path, is_lock_in_root_folder=None, timeout_seconds=None, retry_period_seconds=None):
        return self._create_lock(file_path, lock_name=self._MERGE_LOCK_FILE_NAME, shared=False, is_lock_in_root_folder=is_lock_in_root_folder,
                                 timeout_seconds=timeout_seconds, retry_period_seconds=retry_period_seconds)
