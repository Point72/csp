...
import csp
from csp import DuplicatePolicy, ts


@csp.node
def _pandas_at(trigger: ts[object], xs: {object: ts[object]}, window: object, has_tindex: bool, know_all_valid: bool) -> ts[pandas.DataFrame]:
    ...
    if csp.ticked(trigger):
        if has_tindex and know_all_valid: # More efficient (2-3x) code path if we know a priori that all the times are the same
            keys = xs.keys()
...
@csp.graph
def make_pandas(trigger: ts[object],
