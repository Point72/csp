from csp import ts
from csp.impl.pandas_ext_type import is_csp_type, TsDtype, TsArray
...
def _series_to_python_type(series: pd.Series):
    ...
    if pd.api.types.is_datetime64_any_dtype(series.dtype):
        ...
        if (first_idx is not None or last_idx is not None) and (type(series.iloc[first_idx]) == type(series.iloc[last_idx])):
...
def _run(eval_graph, starttime: datetime, endtime: datetime, realtime: bool = False,
         tick_count: int = -1, tick_history: timedelta = timedelta(), snap: bool = False) -> Dict:
    results = csp.run(eval_graph, tick_count, tick_history, snap,
                      starttime=starttime, endtime=endtime, realtime=realtime, output_numpy=True)
    results = results or {}
    output = {}
...
class CspSeriesAccessor(object):
    ...
    def _validate(obj):
        if not is_csp_type(obj):
            raise AttributeError(
                "Cannot use 'csp' accessor on objects of "
    ...
    def run(self, starttime: datetime, endtime: datetime, realtime: bool = False,
            tick_count: int = -1, tick_history: timedelta = timedelta(), snap: bool = False):
...
class ToCspSeriesAccessor(object):
    ...
    def __call__(self, drop_na: bool = False):
        ...
        if nlevels == 1:
            ...
            return self._obj.groupby(level=levels, group_keys=False)\
                .apply(lambda s: _from_series(s, drop_na)).astype(TsDtype(typ))
