...
try:
    ...
    raise ImportError("perspective must be installed to use this module. "
                      "To install, run 'pip install perspective-python'.")


@csp.node
def _apply_updates(table: perspective.Table,
    ...
    if csp.ticked(data):
        ...
        for (idx, col), value in data.tickeditems():
            ...
            if localize and col in s_datetime_cols and value.tzinfo == None:
...
class CspPerspectiveTable:
    ...
    def graph(self):
        ...
        if self._basket:
            _apply_updates(self._table, self._basket, self._index_col, self._time_col, self._throttle,
                           self._localize, self._static_records)
    ...
    def run(self, starttime=None, endtime=timedelta(seconds=60), realtime=True):
    ...
    def start(self, starttime=None, endtime=timedelta(seconds=60), *, realtime=True, clear=True, auto_shutdown=True):
        ...
        if clear:
            self.clear()
        self._runner = csp.run_on_thread(self.graph, starttime=starttime, endtime=endtime, realtime=realtime, auto_shutdown=auto_shutdown)
    ...
    def get_widget(self, **override_kwargs):
        ...
        if self._keep_history:
            kwargs = {'columns': list(self._data.columns),
    ...
    def start(self, starttime=None, endtime=timedelta(seconds=60), *, realtime=True, clear=True, auto_shutdown=True):
        ...
        if clear:
            for table in self._tables.values():
                table.clear()
        self._runner = csp.run_on_thread(_start_many_graph, set(self.tables.values()),
                                         starttime=starttime, endtime=endtime, realtime=realtime, auto_shutdown=auto_shutdown)
