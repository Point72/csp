        ...
        if annotations:
            for k, v in annotations.items():
                ...
                if not isinstance(actual_type, type) and not isinstance(actual_type, list):
                    raise TypeError('struct field \'%s\' expected field annotation as a type got \'%s\'' % (k, type(v).__name__))

                if isinstance(actual_type, list) and (len(actual_type) != 1 or not isinstance(actual_type[0], type)):
                    raise TypeError('struct field \'%s\' expected list field annotation to be single element list of type got \'%s\'' % (k, type(v).__name__))
...
class Struct(_csptypesimpl.PyStruct, metaclass=StructMeta):
    ...
    def _obj_to_python(cls, obj):
        if isinstance(obj, Struct):
            return obj._postprocess_dict_to_python({k: cls._obj_to_python(getattr(obj, k)) for k in obj.__full_metadata_typed__ if hasattr(obj, k)})
    ...
    def _obj_from_python(cls, json, obj_type):
        ...
        if CspTypingUtils.is_generic_container(obj_type):
            if CspTypingUtils.get_origin(obj_type) in (typing.List, typing.Set, typing.Tuple):
                ...
                return {cls._obj_from_python(k, expected_key_type): cls._obj_from_python(v, expected_value_type) for k, v in json.items()}
...
def defineNestedStruct(name, metadata: dict, defaults: dict = {}, base=Struct):
    ...
    child_structs = {field: defineNestedStruct(f'{name}_{field}', submeta, defaults.get(field, {}))
                     for field, submeta in metadata.items() if isinstance(submeta, dict)}
