        ...
        if res is None:
            if CspTypingUtils.is_generic_container(expected_type):
                ...
                if CspTypingUtils.is_generic_container(new_type):
                    ...
                    if expected_origin in array_types and new_type_origin in array_types and expected_type.__args__ == new_type.__args__:
...
class ContainerTypeVarResolutionError(TypeError):
    def __init__(self, tvar, tvar_value):
        ...
        super().__init__(f"Unable to resolve container type for type variable {tvar} explicit value must have" +
...
class ArgTypeMismatchError(TypeMismatchError):
    def __init__(self, expected_t, actual_arg, arg_name, tvar_info=None):
        self._expected_t, self._actual_arg, self._arg_name, self._tvar_info = expected_t, actual_arg, arg_name, tvar_info
        super().__init__(f"Expected {self.pretty_typename(expected_t)} for argument '{arg_name}', got {actual_arg} " +
...
class TSArgTypeMismatchError(TypeMismatchError):
    def __init__(self, expected_t, actual_arg_type, arg_name, tvar_info=None):
        self._expected_t, self._actual_arg_type, self._arg_name, self._tvar_info = expected_t, actual_arg_type, arg_name, tvar_info
        actual_type_str = f"ts[{self.pretty_typename(actual_arg_type)}]" if actual_arg_type else "None"
        super().__init__(f"Expected ts[{self.pretty_typename(expected_t)}] for argument '{arg_name}'," +
...
class NestedTsTypeError():
    def __init__(self):
        super().__init__(f"Found nested ts type - this is not allowed")
...
class _InstanceTypeResolverBase(metaclass=ABCMeta):
    ...
    def _is_expected_generic_meta(self, typ, expected_generic_meta):
        ...
        return ((is_generic_container and CspTypingUtils.get_origin(typ) is expected_generic_meta) or
                (not is_generic_container and isinstance(expected_generic_meta, typ) and issubclass(expected_generic_meta, typ)))


    def _rec_validate_type_spec_vs_type_spec_and_resolve_tvars(self, actual_sub_type,
                                                               expected_sub_type,
                                                               is_in_ts=False):
        if expected_sub_type is typing.Any:
            ...
            if not self._is_expected_generic_meta(actual_sub_type, origin_typ):
                ...
                if origin_typ is not csp.typing.NumpyNDArray or not self._is_expected_generic_meta(actual_sub_type, csp.typing.Numpy1DArray):
                    return False
            if origin_typ is TsType:
                if is_in_ts:
                    raise NestedTsTypeError()
                return self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(actual_sub_type.__args__[0],
                                                                                   expected_sub_type.__args__[0], is_in_ts=True)
            elif origin_typ is typing.List or origin_typ is typing.Set:
                return self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(actual_sub_type.__args__[0],
                                                                                   expected_sub_type.__args__[0], is_in_ts=is_in_ts)
            elif origin_typ is typing.Dict:
                if not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(actual_sub_type.__args__[0],
                                                                                   expected_sub_type.__args__[0], is_in_ts=is_in_ts):
                    return False
                return self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(actual_sub_type.__args__[1],
                                                                                   expected_sub_type.__args__[1], is_in_ts=is_in_ts)
            ...
            res_type = UpcastRegistry.instance().resolve_type(expected_sub_type, actual_sub_type,
                                                              allow_subtypes=self._allow_subtypes, raise_on_error=False)
        ...
        if self._cur_def.kind.is_single_ts():
            ...
            if arg is not None:
                if not isinstance(arg, Edge):
                    raise ArgTypeMismatchError(expected_t=self._cur_def.typ, actual_arg=arg,
                                               arg_name=self._cur_def.name, tvar_info=tvar_info)
                if isTsDynamicBasket(arg.tstype):
                    arg_type = arg.tstype
                else:
                    arg_type = arg.tstype.typ
            raise TSArgTypeMismatchError(expected_t=self._cur_def.typ.typ, actual_arg_type=arg_type,
                                         arg_name=self._cur_def.name, tvar_info=tvar_info)
        else:
            ...
            raise ArgTypeMismatchError(expected_t=expected_type, actual_arg=arg,
                                       arg_name=self._cur_def.name, tvar_info=tvar_info)
    ...
    def _is_scalar_value_matching_spec(self, inp_def_type, arg):
        if inp_def_type is typing.Any:
            return True
        if UpcastRegistry.instance().resolve_type(inp_def_type, type(arg), allow_subtypes=self._allow_subtypes, raise_on_error=False) is inp_def_type:
            return True
    ...
    def _add_list_basket_ts_value(self, args, in_out_def):
        ...
        for arg in args:
            if (not isinstance(arg, Edge) or
                    not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(arg.tstype.typ, ts_arg_type.typ)):
                self._raise_arg_mismatch_error(args)
    ...
    def _add_dict_basket_ts_value(self, args, in_out_def):
        ...
        for k, v in args.items():
            if (not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(type(k), key_type) or
                    not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(v.tstype.typ, ts_arg_type.typ)):
                self._raise_arg_mismatch_error(args)
    ...
    def _add_t_var_resolution(self, tvar, resolved_type, arg=None):
        ...
        combined_type = UpcastRegistry.instance().resolve_type(resolved_type, old_tvar_type, allow_subtypes=self._allow_subtypes, raise_on_error=False)
    ...
    def _resolve_tvar_container_internal_types(self, tvar, container_typ, arg, raise_on_error=True):
        ...
        if not container_typ in (set, dict, list, numpy.ndarray):
            return container_typ
    ...
    def _try_resolve_tvar_conflicts(self):
        for tvar, conflicting_types in self._conflicting_tvar_types.items():
            ...
            for conflicting_type in conflicting_types:
                if not UpcastRegistry.instance().resolve_type(resolved_type, conflicting_type, allow_subtypes=self._allow_subtypes, raise_on_error=False) is resolved_type:
                    raise TypeError(f"Conflicting type resolution for {tvar} when calling to {self._function_name} : " +
...
class InputInstanceTypeResolver(_InstanceTypeResolverBase):
    def __init__(self, function_name: str,
        ...
        super().__init__(function_name=function_name, input_or_output_definitions=input_definitions,
                         values=arguments, forced_tvars=forced_tvars, allow_subtypes=allow_subtypes,
    ...
    def _add_ts_value(self, arg, in_out_def):
        if isinstance(arg, AttachType):
            ...
            if not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(arg.value_tstype.typ, in_out_def.typ.typ):
                self._raise_arg_mismatch_error(arg)
            self._scalar_inputs.append(arg)
            return
...
class GraphOutputTypeResolver(_InstanceTypeResolverBase):
    def __init__(self, function_name: str,
                 ...
                 forced_tvars: typing.Union[typing.Dict[str, typing.Type], None],
                 allow_subtypes=True):
        super().__init__(function_name=function_name, input_or_output_definitions=output_definitions,
                         values=values, forced_tvars=forced_tvars, allow_subtypes=allow_subtypes,
                         allow_none_ts=False)
    ...
    def _add_list_basket_ts_value(self, args, in_out_def):
        ...
        if in_out_def.kind == ArgKind.BASKET_TS and in_out_def.shape is not None:
            if len(args) != in_out_def.shape:
                raise RuntimeError(f"Expected output shape for output {in_out_def.name} is of length {in_out_def.shape}, actual length is {len(args)}")
    ...
    def _add_dict_basket_ts_value(self, args, in_out_def):
        ...
        if in_out_def.kind == ArgKind.BASKET_TS and in_out_def.shape is not None:
            if len(args) != len(in_out_def.shape):
                raise RuntimeError(f"Expected output shape for output {in_out_def.name} is of length {len(in_out_def.shape)}, actual length is {len(args)}")

            for k in in_out_def.shape:
                if k not in args:
                    raise RuntimeError(f"Expected key {k} for output {in_out_def.name} is missing from the actual returned value")
