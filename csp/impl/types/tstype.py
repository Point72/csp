...
def isTsBasket(t):
    return (CspTypingUtils.is_generic_container(t) and
           ((CspTypingUtils.get_origin(t) is typing.Dict and isTsType(t.__args__[1])) or
            (CspTypingUtils.get_origin(t) is typing.List and isTsType(t.__args__[0]))))


def isTsDynamicBasket(t):
    return CspTypingUtils.is_generic_container(t) and CspTypingUtils.get_origin(t) is typing.Dict and \
           isTsType(t.__args__[1]) and isTsType(t.__args__[0])
...
class _GenericTypesMeta(type):
    ...
    def __getitem__(cls, typ):
        ...
        return type(f'GenericTSTypes[{typ}]', tuple(),
                    {'T_VAR': t_var,
                     'TS_TYPE': ts_type,
