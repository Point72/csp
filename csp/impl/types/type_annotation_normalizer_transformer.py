...
class TypeAnnotationNormalizerTransformer(ast.NodeTransformer):
    ...
    def visit_List(self, node):
        ...
        node = ast.Subscript(value=ast.Attribute(value=ast.Name(id='typing', ctx=ast.Load()), attr='List',
                                                 ctx=ast.Load()),
                             slice=ast.Index(value=self.visit(node.elts[0])), ctx=ast.Load())
        return node
    ...
    def visit_Set(self, node):
        ...
        node = ast.Subscript(value=ast.Attribute(value=ast.Name(id='typing', ctx=ast.Load()), attr='Set',
                                                 ctx=ast.Load()),
                             slice=ast.Index(value=self.visit(node.elts[0])), ctx=ast.Load())
        return node
    ...
    def visit_Dict(self, node):
        ...
        node = ast.Subscript(value=ast.Attribute(value=ast.Name(id='typing', ctx=ast.Load()), attr='Dict',
                                                 ctx=ast.Load()),
                             slice=ast.Index(value=ast.Tuple(elts=[self.visit(node.keys[0]),
                                                                   self.visit(node.values[0])], ctx=ast.Load()), ctx=ast.Load()),
                             ctx=ast.Load())
        return node
    ...
    def visit_Constant(self, node):
        ...
        if self._cur_arg:
            return ast.Call(func=ast.Attribute(value=ast.Name(id='typing', ctx=ast.Load()), attr='TypeVar',
                                               ctx=ast.Load()),
                            args=[node], keywords=[])
        else:
            return node
