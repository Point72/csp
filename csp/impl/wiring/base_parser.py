        ...
        if isinstance(node, ast.With):
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                try:
                    ...
                    raise CspParseError(f"Error parsing csp builtin function")
        else:
            # should never get here either
            raise CspParseError(f"Unable to parse csp builtin function")

        if name in LEGACY_METHODS:
            if name == '__return__':
                warn("Calling __return__ is deprecated. Instead, call return csp.output(v1=va1, v2=val2 ...)", DeprecationWarning)
            else:
                warn("Calling __{0}__ is deprecated: instead, use a 'with csp.{0}()' context (see the csp docs)".format(name[2:-2]), DeprecationWarning)
        return func(*args, **kwargs)
    return wrapper
...
class BaseParser(ast.NodeTransformer, metaclass=ABCMeta):
    ...
    def _eval_expr(self, exp):
        return eval(compile(ast.Expression(body=exp), filename='<csp>', mode='eval'),
    ...
    def _is_pass_or_ellipsis(node):
        return isinstance(node, ast.Pass) or (isinstance(node, ast.Expr) and isinstance(node.value, ast.Constant) and node.value.value == ...)
    
    @staticmethod
    def _is_csp_output_call(node):
        return ( isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id == 'csp' and node.func.attr == 'output' )
    ...
    def _validate_output(cls, node):
        if not BaseParser._is_csp_output_call(node.value) or ( len(node.value.args) + len(node.value.keywords) ) == 0:
            raise CspParseError('Returning multiple outputs must use the following syntax: return csp.output(out1=val1, ...)', node.lineno)
    ...
    def _parse_single_output_definition_with_shapes(self, name, arg_type_node, ts_idx, typ=None, enforce_shape_for_baskets=True):
        assert typ is None
        if (isinstance(arg_type_node, ast.Call) and isinstance(arg_type_node.func, ast.Attribute) and
                arg_type_node.func.attr in OutputBasketContainer.SHAPE_FUNCS):
            value = self._eval_expr(self._type_annotation_normalizer.normalize_single_type_annotation(arg_type_node.func.value))
            if tstype.isTsBasket(value):
                ...
                if tstype.isTsDynamicBasket(value):
                    raise CspParseError(f"dynamic baskets cannot declare shape", arg_type_node.lineno)
                typ = OutputBasketContainer.SHAPE_FUNCS[arg_type_node.func.attr](typ=value,
    ...
    def _parse_output_def(self, node):
        if WARN_PYTHONIC:
            warn("Declaring __outputs__ is deprecated; instead, use output annotation syntax (func -> Outputs(...)). Consult the csp docs.", DeprecationWarning)
    ...
    def parse_outputs(self, node: ast.Call):
        ...
        if node.keywords:
            return tuple(self._parse_single_output_definition(k.arg, k.value, idx) for idx, k in enumerate(node.keywords))
        else:
            name = None
    ...
    def _extract_outputs_from_body(self, body):
        ...
        if i < len(body):
            _, res = self._consume_special_block(body, i, '__outputs__', self._parse_output_def, allow_with_block=False, allow_flat_call=True)
            if res:
        ...
        for idx, (name, typ) in enumerate(type_annotations.items()):
            ...
            if isinstance(typ, type) and issubclass(typ, OutputBasket):
                ...
                if enforce_shape and not shape and not is_dynamic_basket:
                    ...
                    basket_container = OutputBasketContainer.SHAPE_FUNCS[basket.shape_func](typ=typ, shape=shape,
                                                                                            lineno=returns.lineno, col_offset=returns.col_offset,
                                                                                            end_lineno=returns.end_lineno, end_col_offset=returns.end_col_offset)
    ...
    def parse_func_signature(self, funcdef):
        ...
        for arg_idx, arg in enumerate(args.args):
            if arg.annotation is None:
                ...
                # seems to be <locals>
                if (arg.arg in ('self', 'cls') and arg_idx == 0 and self._raw_func.__qualname__ != self._raw_func.__name__
        ...
        if annotation_outputs and body_outputs:
            raise CspParseError("csp.node and csp.graph outputs must be via return annotation or __outputs__ call, not both")
    ...
    def _is_csp_special_func_call(cls, call: ast.Call, func_name_or_list):
        name_pred = (lambda x: x == func_name_or_list) if isinstance(func_name_or_list, str) else (lambda x: x in func_name_or_list)
        ...
        if (isinstance(call, ast.Call) and
                ((isinstance(call.func, ast.Name) and name_pred(call.func.id))
                 or (isinstance(call.func, ast.Attribute) and isinstance(call.func.value, ast.Name)
                     and call.func.value.id == 'csp' and name_pred(call.func.attr)))):
            return True
        ...
        elif (isinstance(call, ast.Attribute) and \
              isinstance(call.value, ast.Name) and \
              name_pred(call.attr)):
                ...
                if output.kind == ArgKind.BASKET_TS:
                    if enforce_shape_for_baskets or isinstance(output.typ, OutputBasketContainer):
                        ...
                        if not self._is_ts_args_removed_from_signature():
                            args.args = [arg for input, arg in zip(self._signature.raw_inputs(), args.args) if input.kind.is_scalar()]
                        ...
                        if isinstance( output.typ.shape, ast.AST ):
                            shape_func = ast.FunctionDef(name=f'{name}_shape_eval',
                                                         args=args,
    ...
    def _resolve_special_outputs(self):
        if self._add_cache_control_output:
            self._special_outputs += (OutputDef(name=CSP_CACHE_ENABLED_OUTPUT, typ=TsType[bool],
                                                kind=ArgKind.TS, ts_idx=self._outputs[-1].ts_idx + 1,
                                                shape=None),)
