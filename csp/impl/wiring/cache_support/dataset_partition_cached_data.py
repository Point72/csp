...
class DataSetCachedData:
    ...
    def __call__(self, *args, **kwargs):
        return DatasetPartitionCachedData(self._data_set_partition_calculator_func(*args, **kwargs), self._cache_serializers)
...
class DatasetPartitionCachedData:
    ...
    def get_data_files_for_period(self,
        ...
        """
        return self._dataset_partition.get_data_for_period(self._normalize_time(starttime, True),
                                                           self._normalize_time(endtime, True),
                                                           missing_range_handler)[0]
    ...
    def _load_single_file_all_columns(self, starttime, endtime, file_path, column_list, basket_column_list, struct_basket_sub_columns):
        import pandas
        import numpy
    ...
    def _load_data_split_to_columns(self, starttime, endtime, file_path, column_list, basket_column_list, struct_basket_sub_columns):
        ...
        if array_columns and value_arrays and value_arrays and value_arrays[0]:
            value_arrays, column_list = self._convert_array_columns(value_arrays, column_list, array_columns, shape_columns)
        ...
        if basket_column_list:
            ...
            for column in basket_column_list:
                ...
                if issubclass(value_type, csp.Struct):
                    ...
                    assert column not in struct_basket_sub_columns, f"Specified sub columns for {column} but it's not a struct"
                ...
                if basket_data_raw.empty:
                    ...
                    for f in value_columns:
                        ...
                        field_array, pandas_dtype = self._create_empty_full_array(dtype, field_array_shape, pandas_dtype)
                    ...
                    basket_data_aligned = pandas.concat(field_matrices.values(), keys=list(field_matrices.keys()), axis=1)
    ...
    def _read_flat_data_from_files(self, symbol_file, value_files, num_values_to_skip, num_values_to_read):
        ...
        for row_group_index in range(symbol_parquet_file.num_row_groups):
            ...
            cur_row_group_num_values_to_read = int(min(row_group.num_rows - cur_row_group_start_index, num_values_to_read))
    ...
    def _load_flat_basket_data(self, starttime, endtime, timestamp_file_name, symbol_file_name, value_count_file_name, value_files, need_timestamp=True):
        import numpy
        import pyarrow.parquet
        ...
        if num_values_to_read == 0:
            return None
        res = pyarrow.concat_tables(filter(None, self._read_flat_data_from_files(symbol_file_name, value_files, num_values_to_skip, num_values_to_read)))
        ...
        if need_timestamp:
            ...
            value_counts_sub_array_cumsum_aux = value_counts_sub_array_cumsum[value_counts_sub_array_cumsum < timestamp_array_size]
    ...
    def _get_flat_basket_df_for_period(self,
        ...
        if load_values:
            ...
            if issubclass(value_type, csp.Struct):
                ...
                value_files = [{field: os.path.join(f, f'{basket_field_name}.{field}.parquet') for field in struct_fields} for f in data_files.values()]
            else:
                assert struct_fields is None, f"Trying to provide struct_fields for non struct output {basket_field_name}"
                value_files = [{basket_field_name: os.path.join(f, f'{basket_field_name}.parquet')} for f in data_files.values()]
        else:
            value_files = list(itertools.repeat({}, len(symbol_files)))
        value_count_files = [os.path.join(f, f'{basket_field_name}__csp_value_count.parquet') for f in data_files.values()]
        timestamp_files = [os.path.join(f, f'{self.metadata.timestamp_column_name}.parquet') for f in data_files.values()]
        file_tuples = [(t, s, c, d) for t, s, c, d in zip(timestamp_files, symbol_files, value_count_files, value_files)]

        if num_threads > 1:
            with ThreadPoolExecutor(max_workers=num_threads) as pool:
                tasks = [pool.submit(self._load_flat_basket_data, starttime, endtime, *tup, need_timestamp=need_timestamp) for tup in file_tuples]
                results = list(task.result() for task in tasks)
        else:
            results = [self._load_flat_basket_data(starttime, endtime, *tup, need_timestamp=need_timestamp) for tup in file_tuples]
    ...
    def get_flat_basket_df_for_period(self,
                                      symbol_column: str,
                                      ...
                                      missing_range_handler: Callable[[datetime.datetime, datetime.datetime], bool] = None,
                                      num_threads=1):
        res = self._get_flat_basket_df_for_period(basket_field_name=basket_field_name, symbol_column=symbol_column, struct_fields=struct_fields, starttime=starttime,
                                                  endtime=endtime, missing_range_handler=missing_range_handler, num_threads=num_threads, concat=True)
        if res is None:
            return None
    ...
    def get_all_basket_ids_in_range(self, basket_field_name=None, starttime: Optional[datetime.datetime] = None,
                                    endtime: Optional[datetime.datetime] = None,
                                    missing_range_handler: Callable[[datetime.datetime, datetime.datetime], bool] = None,
                                    num_threads=1):
        import numpy
        symbol_column_name = '__csp_symbol__'
        parquet_tables = self._get_flat_basket_df_for_period(basket_field_name=basket_field_name, symbol_column=symbol_column_name,
                                                             starttime=starttime, endtime=endtime, missing_range_handler=missing_range_handler,
                                                             num_threads=num_threads, load_values=False, concat=False,
    ...
    def invalidate_cache(self, starttime: Optional[datetime.datetime] = None, endtime: Optional[datetime.datetime] = None):
        ...
        if starttime is not None:
            ...
            if starttime != agg_period_starttime:
                raise RuntimeError(f"Trying to invalidate data starting on {starttime} - invalidation should be for full aggregation period (starting on {agg_period_starttime})")

        if endtime is not None:
            agg_period_endtime = aggregation_period_utils.resolve_period_end(endtime, exclusive_end=False)
            if endtime != agg_period_endtime:
                raise RuntimeError(f"Trying to invalidate data ending on {endtime} - invalidation should be for full aggregation period (ending on {agg_period_endtime})")
    ...
    def get_data_df_for_period(self,
                               starttime: Optional[datetime.datetime] = None,
                               endtime: Optional[datetime.datetime] = None,
                               missing_range_handler: Callable[[datetime.datetime, datetime.datetime], bool] = None,
        ...
        if num_threads > 1:
            with ThreadPoolExecutor(max_workers=num_threads) as pool:
                tasks = [pool.submit(data_loader_function, file_start_time, file_end_time, data_file, column_list, basket_column_list, struct_basket_sub_columns) for
                         (file_start_time, file_end_time), data_file in
                         data_files.items()]
                dfs = [task.result() for task in tasks]
        else:
            dfs = [data_loader_function(file_start_time, file_end_time, data_file, column_list, basket_column_list, struct_basket_sub_columns) for (file_start_time, file_end_time), data_file in
                   data_files.items()]
