...
from csp.impl.wiring import Edge, Signature
...
class WrappedStructEdge(Edge):
    def __init__(self, wrapped_edge, parquet_reader, field_map):
        super().__init__(tstype=wrapped_edge.tstype, nodedef=wrapped_edge.nodedef, output_idx=wrapped_edge.output_idx, basket_idx=wrapped_edge.basket_idx)
...
class CacheCategoryOverridesTree:
    ...
    def construct_from_cache_config(cls, cache_config: Optional[CacheConfig] = None):
        ...
        if cache_config is None:
            raise RuntimeError(f"data_folder must be set in global cache_config to use caching")
...
@node
def _deserialize_value(value: csp.ts[bytes], type_serializer: CacheObjectSerializer, typ: 'T') -> csp.ts['T']:
    if csp.ticked(value):
...
class GraphBuildPartitionCacheManager(object):
    ...
    def __init__(self, function_name, dataset: ManagedDataset, partition_values, expected_outputs, cache_options, csp_cache_start=None, csp_cache_end=None,
        ...
        data_for_period, is_full_period_covered = self.get_data_for_period(self._csp_cache_start,
                                                                           self._csp_cache_end,
                                                                           missing_range_handler=missing_period_callback)
        ...
        if is_full_period_covered:
            ...
            reader = ParquetReader(filename_or_list=self._read_files_provider(dataset,
                                                                              data_for_period,
                                                                              cache_config.lock_file_permissions,
            ...
            for output in expected_outputs:
                ...
                if output.kind == ArgKind.BASKET_TS:
                    ...
                    if isinstance(output.typ.typ, type) and issubclass(output.typ.typ, csp.Struct):
                        ...
                        output_value = WrappedStructEdge(reader.subscribe_all(typ=output.typ.typ, field_map=field_map), reader, write_field_map)
                    else:
                        type_serializer = cache_serializers.get(output.typ.typ)
                        if type_serializer:
                            output_value = _deserialize_value(reader.subscribe_all(typ=bytes, field_map=cache_options.field_mapping.get(output_name, output_name)),
                                                              type_serializer,
                                                              output.typ.typ)
                        else:
                            output_value = reader.subscribe_all(typ=output.typ.typ, field_map=cache_options.field_mapping.get(output_name, output_name))
    ...
    def _read_files_provider(self, dataset, data_files, lock_file_permissions, engine_lock_releaser):
        ...
        while not finished:
            if num_failures > 10:
                raise RuntimeError(f"Failed to read cached files too many times, last attempted file is {next_filename}")
            try:
    ...
    def get_dataset_for_func(cls, graph, func, cache_options, data_folder):
        ...
        return ManagedDataset.load_from_disk(cache_config=cache_config,
                                             name=dataset_name, data_category=category)
    ...
    def _resolve_dataset(cls, graph, func, signature, cache_options, expected_outputs, tvars):
        ...
        if len(expected_outputs) == 1 and expected_outputs[0].name is None:
            ...
            expected_outputs = (OutputDef(name=DatasetNameConstants.UNNAMED_OUTPUT_NAME,
                                          typ=cur_def.typ, kind=cur_def.kind,
                                          ts_idx=cur_def.ts_idx, shape=cur_def.shape),)
        else:
        ...
        for i, out in enumerate(expected_outputs):
            ...
            elif isinstance(out.typ.typ, type) and issubclass(out.typ.typ, csp.Struct):
        ...
        dataset = cls._create_dataset(func,
    ...
    @classmethod
    def _create_dataset(cls, func, cache_config, category, dataset_name,
        ...
        dataset = ManagedDataset(name=name,
                                 category=category,
    ...
    def get_data_for_period(self, starttime: datetime, endtime: datetime, missing_range_handler):
        res, full_period_covered = self.dataset_partition.get_data_for_period(starttime=starttime - self._csp_timestamp_shift,
                                                                              endtime=endtime - self._csp_timestamp_shift,
                                                                              missing_range_handler=missing_range_handler)
        if self._csp_timestamp_shift:
            res = {(start + self._csp_timestamp_shift, end + self._csp_timestamp_shift): path for (start, end), path in res.items()}
    ...
    @classmethod
    def create_cache_manager(cls, graph, func, signature, non_ignored_scalars, all_scalars, cache_options, expected_outputs, tvars, csp_cache_start=None, csp_cache_end=None, csp_timestamp_shift=None):
        ...
        key = _CacheManagerKey(all_scalars, id(func), tuple(tvars.items()), csp_cache_start, csp_cache_end, csp_timestamp_shift)
        ...
        for output in expected_outputs:
            if output.kind == ArgKind.TS and isinstance(output.typ.typ, type) and issubclass(output.typ.typ, csp.Struct):
        ...
        res = GraphBuildPartitionCacheManager(function_name=QualifiedNameUtils.get_qualified_object_name(func),
                                              dataset=dataset, partition_values=partition_values,
                                              expected_outputs=expected_outputs, cache_options=cache_options,
                                              csp_cache_start=csp_cache_start, csp_cache_end=csp_cache_end,
                                              csp_timestamp_shift=csp_timestamp_shift)
        Context.TLS.instance.cache_data.cache_managers[key] = res
        return res


class GraphCacheOptions(csp.Struct):
...
class ResolvedGraphCacheOptions(csp.Struct):
...
def resolve_graph_cache_options(signature: Signature,
                                cache_enabled,
    ...
    if cache_enabled:
            ...
            if isinstance(output.typ, OutputBasketContainer):
                ...
                if tstype.isTsStaticBasket(output.typ):
                    if CspTypingUtils.get_origin(output.typ) is List:
                        ...
                        raise TypeError(f"Cached output basket {output.name} must have shape provided using with_shape or with_shape_of")
        ...
        for input in signature.scalars:
            if input.name not in ignored_inputs and not ManagedDataset.is_supported_partition_type(input.typ):
                raise NotImplementedError(f"Caching is unsupported for argument type {input.typ} (argument {input.name})")
        resolved_cache_options = ResolvedGraphCacheOptions(prefix_struct_names=cache_options.prefix_struct_names)

        for attr in ('dataset_name', 'data_timestamp_column_name', 'category', 'ignored_inputs', 'parquet_output_config',
                     'time_aggregation', 'controlled_cache', 'default_cache_enabled'):
