...
class SignalSetSource(VariableSource):
    ...
    def __init__(
        ...
        self._keys_in_order = tuple(key_to_child_name.keys())
        self._idx_to_key = {i: k for i, k in enumerate(self._keys_in_order)}
        self._key_to_index = {k: i for i, k in enumerate(self._keys_in_order)}
...
class SignalSetKeySource(VariableSource):
    """Represents the logical key produced while iterating a SignalSet."""

    def __init__(self, name: str, iter_var_name: str, logical_keys: tuple, variable_factory=None):
        super().__init__(UnknownType(UnknownNumbaType(), None), name, variable_factory=variable_factory)
    ...
    def get_storage_location(self):
        raise ValueError("SignalSetKeySource has no direct storage location")

    def read(self):
        return None

    def _iter_index_expr(self) -> ast.Name:
        return ast.Name(id=self.iter_var_name, ctx=ast.Load())

    def _build_choice_expr(self, values: tuple) -> ast.AST:
        if not values:
            raise ValueError("SignalSetKeySource requires at least one key")

        if len(values) == 1:
            return ast.Constant(value=values[0])

        expr = ast.Constant(value=values[-1])
        for idx in range(len(values) - 2, -1, -1):
            expr = ast.IfExp(
                test=ast.Compare(
                    left=self._iter_index_expr(),
                    ops=[ast.Eq()],
                    comparators=[ast.Constant(value=idx)],
                ),
                body=ast.Constant(value=values[idx]),
                orelse=expr,
            )
        return expr

    def get(self):
        return self._build_choice_expr(self.logical_keys)

    def resolve_index_expr(self, container: SignalSetSource) -> ast.AST:
        indices = tuple(container.get_key_index(key) for key in self.logical_keys)
        if indices == tuple(range(len(indices))):
            return self._iter_index_expr()
        return self._build_choice_expr(indices)
...
def handle_signal_set_iteration(converter, node: ast.For) -> Optional[ast.AST]:
    ...
    transformed_body = []
    previous_loop_var = converter.variable_factory.variable_name_map.get(loop_var)
    converter.variable_factory.variable_name_map[loop_var] = SignalSetKeySource(
        name=loop_var,
        iter_var_name=iter_var,
        logical_keys=signal_set._keys_in_order,
        variable_factory=converter.variable_factory,
    )
    try:
        ...
        if previous_loop_var is None:
            del converter.variable_factory.variable_name_map[loop_var]
        else:
            converter.variable_factory.variable_name_map[loop_var] = previous_loop_var
