...
class DelayedEdge(Edge):
    ...
    @graph(memoize=False)
    def _bind(self, edge:csp.ts['T']):
        if self.is_bound():
            raise RuntimeError(f'Attempted to bind DelayedEdge multiple times, previously bound to output from node "{self.nodedef._signature._name}"')
