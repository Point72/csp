...
from csp.impl.wiring import runtime
from csp.impl.types.common_definitions import InputDef, OutputDef, ArgKind, BasketKind
...
class DynamicNodeDef:
    ...
    def _create(self, engine, memo):
        ...
        for idx, output in enumerate(self._outputs):
            node.create_output(idx, (None, ContainerTypeNormalizer.normalized_type_to_actual_python_type(output.typ.__args__[1].typ)))
...
def dynamic(trigger, sub_graph, *args, **kwargs):
    ...
    for out in sub_graph._signature._outputs:
        ...
        sig_outputs.append(OutputDef(out.name, tstype.DynamicBasket[trigger_keytype, out.typ.typ], ArgKind.DYNAMIC_BASKET_TS, out.ts_idx, None))
    ...
    signature = Signature(sub_graph._signature._name, sub_graph._signature._inputs, sig_outputs, sub_graph._signature._defaults)
