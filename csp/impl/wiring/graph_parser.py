...
class GraphParser(BaseParser):
    ...
    def _wrap_returned_value_and_add_special_outputs(self, returned_value):
        res = ast.Return(value=ast.Call(func=ast.Attribute(
            value=ast.Attribute(value=ast.Attribute(value=ast.Name(id='csp', ctx=ast.Load()), attr='impl', ctx=ast.Load()), attr='wiring', ctx=ast.Load()),
            attr='OutputsContainer', ctx=ast.Load()), args=[],
    ...
    def _parse_single_output_definition(self, name, arg_type_node, ts_idx, typ=None):
        return self._parse_single_output_definition_with_shapes(name, arg_type_node, ts_idx, typ,
                                                                enforce_shape_for_baskets=False)
    ...
    def _parse_return(self, node, special_outputs=None):
        ...
        if len(node.args) > 1 or (len(node.args) and len(node.keywords)):
            raise CspParseError('csp return expects to be called with (value) or (output = value, output2 = value2)', node.lineno)
        if len(node.args) == 1:
            if len(self._signature._outputs) > 1:
                raise CspParseError(f'cannot return single unnamed arg in graph returning %d outputs' % len(self._signature._outputs), node.lineno)
            elif len(self._outputs) == 0:
                raise CspParseError('returning from graph without any outputs defined', node.lineno)
            elif len(self._signature._outputs) == 1 and self._signature._outputs[0].name is None:  # graph only has one unnamed output
                if self._add_cache_control_output:
        ...
        if len(node.keywords) == 1 and node.keywords[0].arg is None:
            if not isinstance(node.keywords[0].value, ast.Call):
                raise CspParseError(f'only unpacking of other csp.node or csp.graph calls are allowed', node.lineno)
            ...
            node.keywords[0].value = ast.Attribute(value=node.keywords[0].value, attr='_dict', lineno=node.lineno,
                                                   ctx=ast.Load())
        elif len(node.keywords) != len(self._signature._outputs):
            raise CspParseError(f'returning {len(node.keywords)} values from graph when expected {len(self._signature._outputs)}', node.lineno)

        # Multiple named arguments
        node.func = ast.Attribute(
            value=ast.Attribute(
                value=ast.Attribute(
                    value=ast.Name(id='csp', ctx=ast.Load()),
                    attr='impl', lineno=node.lineno, ctx=ast.Load()),
                attr='wiring', lineno=node.lineno, ctx=ast.Load()),
            attr='OutputsContainer', lineno=node.lineno, ctx=ast.Load())
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id == 'csp' and node.func.attr == 'set_cache_enable_ts':
            if len(node.args) != 1 or node.keywords:
                raise CspParseError(f'Invalid call to csp.set_cache_enable_ts', node.lineno)
            if self._add_cache_control_output:
                return ast.Assign(targets=[ast.Name(id=CSP_CACHE_ENABLED_OUTPUT, ctx=ast.Store())],
                                  value=node.args[0])
            else:
                raise CspParseError(f'Invalid call to csp.set_cache_enable_ts in graph with non controlled cache', node.lineno)
    ...
    def _parse_impl(self):
        ...
        self._signature = Signature(self._name, self._inputs, self._outputs, input_defaults, special_outputs=self._special_outputs)
        # We need to set default value for the cache control variable as the first command in the function
        if self._add_cache_control_output:
            self._funcdef.body = [
                                     ast.Assign(targets=[ast.Name(id=CSP_CACHE_ENABLED_OUTPUT, ctx=ast.Store())],
                                            value=ast.Call(func=ast.Attribute(value=ast.Name(id='csp', ctx=ast.Load()), attr='null_ts', ctx=ast.Load()),
                                                       args=[ast.Name(id='bool', ctx=ast.Load())],
                                                       keywords=[]))] + self._funcdef.body
