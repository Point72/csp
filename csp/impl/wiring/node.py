...
class NodeDefMeta(type):
    def __init__(self, *args, **kwargs):
        ...
        if self.memoize or self.force_memoize:
            self._instantiate_func = csp_memoized_graph_object(self._instantiate_impl, force_memoize=self.force_memoize, function_name=function_full_name(self._impl))
...
class NodeDef:
        ...
        for input in inputs:
            if isinstance(input, Edge):
                if tstype.isTsDynamicBasket(input.tstype):
                    converted.append((None,ContainerTypeNormalizer.normalized_type_to_actual_python_type(input.tstype.__args__[1].typ)))
                else:
                    converted.append(ContainerTypeNormalizer.normalized_type_to_actual_python_type(input.tstype.typ))
            elif isinstance(input, (list, tuple)):
                converted.append((len(input), ContainerTypeNormalizer.normalized_type_to_actual_python_type(input[0].tstype.typ)))
            else:  # dict
                if not (isinstance(input, dict)):
                    raise ValueError(f"Unexpected value {input} passed to {name}")
                converted.append((list(input.keys()), ContainerTypeNormalizer.normalized_type_to_actual_python_type(next(iter(input.values())).tstype.typ)))
    ...
    def _create(self, engine, memo):
        ...
        node = None
        if self._cppimpl:
            cppinputs = [ ( input_def.name, input_type, input_def.ts_idx, input_def.kind == ArgKind.ALARM ) for input_type,input_def in zip(inputs,self._signature.ts_inputs) ]
            cppoutputs = [ ( output_def.name if output_def.name else "", output_type, output_def.ts_idx ) for output_type,output_def in zip(self._output_types, self._signature._outputs) ]
            ...
            try:
                ...
                logging.debug( f'cppimpl for node "{self._signature._name}" doesnt supported the given inputs, defaulting to python impl ({err})')
...
def _create_node(func, func_frame, debug_print, memoize, force_memoize, cppimpl, pre_create_hook, name, cache: bool = False, cache_options=None):
    create_wrapper = cache or cache_options
    parser = NodeParser(name, func, func_frame, debug_print=debug_print, add_cache_control_output=cache_options and cache_options.controlled_cache)
    parser.parse()

    nodetype = NodeDefMeta(name, (NodeDef,),
...
def _node_internal_use(func=None, *, func_frame=None, memoize=True, force_memoize=False, debug_print=False, cppimpl=None, pre_create_hook=None,
                       cache: bool = False, cache_options=None, name=None):
