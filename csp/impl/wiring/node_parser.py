...
class _SingleProxyFuncArgResolver(object):
    ...
    def __call__(self, proxy, node):
        ...
        assert len(node.args) <= len(self._arg_name_to_index) + 1, f"Invalid number of arguments provided for {self._func_ast.name}"
        ...
        for arg in node.keywords:
            ...
            if resolved_args[arg_index] is not self.INVALID_VALUE:
                raise CspParseError(f"{self._func_ast.name}() got multiple values for argument '{arg.arg}'", node.lineno)
        ...
        for arg_index in range(len(resolved_args)):
            if resolved_args[arg_index] is self.INVALID_VALUE:
                if self._all_defaults[arg_index] is self.INVALID_VALUE:
                    ...
                    raise CspParseError(f"{self._func_ast.name}() missing 1 required positional argument '{arg_name}'", node.lineno)
                else:
                    resolved_args[arg_index] = self._all_defaults[arg_index]

        return ast.Call(func=ast.Attribute(value=proxy, attr=self._func_ast.name, ctx=ast.Load()), args=resolved_args,
                        keywords=[])
...
class NodeParser(BaseParser):
    ...
    _LOCAL_METHODS = {_CSP_NOW_FUNC: _cspimpl._csp_now,
    ...
    def __init__(self, name, raw_func, func_frame, debug_print=False, add_cache_control_output=False):
        super().__init__(name=name, raw_func=raw_func, func_frame=func_frame, debug_print=debug_print, add_cache_control_output=add_cache_control_output)
        self._stateblock = []
        self._startblock = []
        self._stopblock = []
    ...
    def _parse_alarms(self, node):
        ...
        if WARN_PYTHONIC and len(call.keywords):
            warn("Variable declarations within alarms() are deprecated. Instead, declare the variable in a with alarms() context.", DeprecationWarning)
        ...
        if body:
            ...
            for node in body:
                ...
                if len(call_node.keywords):
                    raise TypeError(f"function `csp.alarm` does not take keyword arguments")
                if len(call_node.args) != 1:
                    raise TypeError(f"function `csp.alarm` requires a single type argument: {len(call_node.args)} arguments given")
    ...
    def _parse_state(self, node):
        ...
        if WARN_PYTHONIC and len(call.keywords):
            warn("Variable declarations within state() are deprecated. Instead, declare the variable in a with state() context.", DeprecationWarning)
        for kwd in call.keywords:
        ...
        if body:
            for syntax in body:
                if not isinstance(syntax, (ast.Pass, ast.Expr, ast.Assign, ast.AnnAssign)):
                    warn("Only variable assignments and declarations should be present in a csp.state block. Any logic should be moved to csp.start", DeprecationWarning)
    ...
    def _ts_inproxy_expr(self, id, ctx=ast.Load()):
        if isinstance(id, ast.Name) or isinstance(id, int):
            input_def = self._signature.ts_input_by_id(id) if isinstance(id, int) else self._signature.input(id.id, allow_missing=True)
            if not input_def:
            ...
            return ast.Call(func=ast.Attribute(value=basket_proxy, attr='_proxy', ctx=ast.Load()), args=[index], keywords=[])
            ...
            if not input_def.kind.is_dynamic_basket():
                raise CspParseError(f'invalid use of .shape on input \'{id.value.id}\', .shape property is only available on dynamic basket inputs', self._cur_node.lineno)
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id == 'csp':
            if is_special_func_call:
                raise CspParseError(f"Invalid usage of {node.func.attr}, it should appear at the beginning of the function (consult documentation for details)")
            ...
            if is_special_func_call:
                raise CspParseError(f"Invalid usage of {node.func.id}, it should appear at the beginning of the function (consult documentation for details)")
    ...
    def visit_Return(self, node):
        ...
        if len(self._outputs) == 1:
            ...
            if node.value is not None:
                return [ast.Expr(ast.BinOp(left=out_proxy, op=ast.Add(), right=self.visit(node.value)),
    ...
    def _parse_output_or_return(self, node, is_return):
        ...
        if len(node.args) > 2 or (len(node.args) and len(node.keywords)):
            raise CspParseError(f'{func_name} expects to be called with (output, value) or (output = value, output2 = value2)', node.lineno)
        nodes = []
        if len(node.args) == 1:
            if len(self._signature._outputs) > 1 and self._signature.output(0).name is not None:
                raise CspParseError(f'cannot {func_name} single unnamed arg in node returning %d outputs' % len(self._signature._outputs), node.lineno)
            elif len(self._outputs) == 0:
                raise CspParseError('returning from node without any outputs defined', node.lineno)
            else:  # node only has one output
                output = self._signature.output(0)
                if isinstance(output.typ, OutputBasketContainer):  # if it's a basket, the arg is a dict: csp.output({k1:v1, k2:v2})
                    if isinstance(node.args[0], ast.Dict):  # unnamed dict: convert to subscript
                        nodes.extend(ast.BinOp(left=ast.Subscript(value=self._ts_outproxy_expr(output.name), slice=ASTUtils.create_subscript_index(k), ctx=ast.Load()), op=ast.Add(), right=v) for k, v in
                                     zip(node.args[0].keys, node.args[0].values))
                    else:  # named dict: pass to PyBasketOutputProxy for handling
                        nodes.append(ast.BinOp(left=self._ts_outproxy_expr(output.name), op=ast.Add(), right=node.args[0]))
                ...
                self._returned_outputs.add(None)
        if len(node.args) == 2:
            if isinstance(node.args[0], ast.Subscript):  # if node.args[0] is an ast.Subscript then node.args[0].value.id must name an output basket: csp.output(x[k], v)
                if is_return:
                    raise CspParseError(f"Invalid use of {func_name} basket element returns is not possible with return")
                ...
                nodes.append(ast.BinOp(left=ast.Subscript(value=self._ts_outproxy_expr(output_name), slice=node.args[0].slice, ctx=ast.Load()), op=ast.Add(), right=node.args[1]))
        ...
        for arg in node.keywords:
            ...
            if isinstance(output.typ, OutputBasketContainer) and isinstance(arg.value, ast.Dict):
                nodes.extend(ast.BinOp(left=ast.Subscript(value=self._ts_outproxy_expr(arg.arg), slice=ASTUtils.create_subscript_index(k), ctx=ast.Load()), op=ast.Add(), right=v) for k, v in
                             zip(arg.value.keys, arg.value.values))
            else:
        ...
        if len(nodes) == 0:
            if not is_return:
                raise CspParseError(f'Empty output is not allowed', node.lineno)
            res = ast.Pass(lineno=node.lineno, end_lineno=node.end_lineno)
        else:
            res = ast.BoolOp(op=ast.Or(), values=nodes, lineno=node.lineno, end_lineno=node.end_lineno) if len(nodes) > 1 else nodes[0]
        if is_return:
            if isinstance(res, ast.Pass):
                return [res, ast.Continue(lineno=node.lineno, end_lineno=node.end_lineno)]
            else:
                return [ast.Expr(res, lineno=node.lineno, end_lineno=node.end_lineno), ast.Continue(lineno=node.lineno, end_lineno=node.end_lineno)]
    ...
    def _parse_remove_dynamic_key(self, node):
        ...
        if len(node.args) != expected_args:
            raise CspParseError(f'csp.remove_dynamic_key expects {expected_args} arguments for {"" if is_named_outputs else "un"}named outputs, got {len(node.args)}')
        ...
        if output_def.kind != ArgKind.DYNAMIC_BASKET_TS:
            raise CspParseError('output %sis not a dynamic basket output' % f'{name} ' if name is not None else '', node.lineno)

        # Convert csp.remove_dynamic_key call to outputting dict of { key : csp.impl.REMOVE_DYNAMIC_KEY }
        value = ast.Dict(keys=[key], values=[ast.Attribute(value=ast.Attribute(value=ast.Name(id='csp', ctx=ast.Load()), attr='impl', ctx=ast.Load()),
                                                           attr='REMOVE_DYNAMIC_KEY', ctx=ast.Load())])
    ...
    def _parse_now(self, node):
        ...
        return ast.Call(func=ast.Name(id=self._CSP_NOW_FUNC, ctx=ast.Load()),
                        args=[self._node_proxy_expr()], keywords=[])

    def _parse_stop_engine(self, node):
        args = [self._node_proxy_expr()] + node.args
        return ast.Call(func=ast.Name(id=self._CSP_STOP_ENGINE_FUNC, ctx=ast.Load()),
                        args=args, keywords=node.keywords)
    ...
    def _parse_schedule_alarm_func(self, node, funcname, name_node=None):
        ...
        proxy = self._ts_inproxy_expr(node.args[0])
        return ast.Call(func=ast.Attribute(value=proxy, attr=funcname, ctx=ast.Load()), args=node.args[1:], keywords=node.keywords)
    ...
    def _parse_set_buffering_policy(self, node):
        ...
        return ast.Call(func=ast.Attribute(value=proxy, attr='set_buffering_policy', ctx=ast.Load()), args=args, keywords=node.keywords)

    def _parse_engine_start_time(self, node):
        if len(node.args) or len(node.keywords):
            raise CspParseError('csp.engine_start_time takes no arguments', node.lineno)
        return ast.Call(func=ast.Name(id=self._CSP_ENGINE_START_TIME_FUNC, ctx=ast.Load()),
                        args=[self._node_proxy_expr()], keywords=[])

    def _parse_engine_end_time(self, node):
        if len(node.args) or len(node.keywords):
            raise CspParseError('csp.engine_end_time takes no arguments', node.lineno)
        return ast.Call(func=ast.Name(id=self._CSP_ENGINE_END_TIME_FUNC, ctx=ast.Load()),
                        args=[self._node_proxy_expr()], keywords=[])
    ...
    def _parse_csp_engine_stats(self, node):
        ...
        return ast.Call(func=ast.Name(id=self._CSP_ENGINE_STATS_FUNC, ctx=ast.Load()),
                        args=[self._node_proxy_expr()], keywords=[])
    ...
    def _parse_special_blocks(self, body):
        ...
        index, last_special_block = consume_block(body, index, 'alarms', self._parse_alarms, allow_with_block=True, allow_flat_call=False)
        index, last_special_block = consume_block(body, index, '__alarms__', self._parse_alarms, allow_with_block=True, allow_flat_call=True)

        index, last_special_block = consume_block(body, index, 'state', self._parse_state, allow_with_block=True, allow_flat_call=False)
        index, last_special_block = consume_block(body, index, '__state__', self._parse_state, allow_with_block=True, allow_flat_call=True)

        index, last_special_block = consume_block(body, index, 'start', self._parse_start, allow_with_block=True, allow_flat_call=False)
        index, last_special_block = consume_block(body, index, '__start__', self._parse_start, allow_with_block=True, allow_flat_call=False)

        index, last_special_block = consume_block(body, index, 'stop', self._parse_stop, allow_with_block=True, allow_flat_call=False)
        index, last_special_block = consume_block(body, index, '__stop__', self._parse_stop, allow_with_block=True, allow_flat_call=False)
            ...
            if self._is_special_with_or_call(first_body_elem, self._SPECIAL_BLOCKS):
                for special_block_name in self._SPECIAL_BLOCKS:
                    if self._is_special_with_or_call(first_body_elem, special_block_name):
                        ...
                        raise CspParseError(f"{special_block_name} must be declared before {last_special_block}", lineno=first_body_elem.lineno,
    ...
    @classmethod
    def _create_ast_args(cls, posonlyargs=[], args=[], kwonlyargs=[], defaults=[], vararg=None, kwarg=None, kw_defaults=[]):
        # position only args are introducing in 3.8
        if sys.version_info.major > 3 or sys.version_info.minor >= 8:
            return ast.arguments(posonlyargs=posonlyargs, args=args, kwonlyargs=kwonlyargs, defaults=defaults, vararg=vararg, kwarg=kwarg, kw_defaults=kw_defaults)
        else:
            return ast.arguments(args=args, kwonlyargs=kwonlyargs, defaults=defaults, vararg=vararg, kwarg=kwarg, kw_defaults=kw_defaults)
