...
class NumbaNumbaNodeDefMeta(type):
    ...
    def _instantiate_impl(self, __forced_tvars, args, kwargs):
        ...
        NumbaNodeDef = super().__call__(inputs, scalars, self._state_types, tvars, self._impl, pre_create_hook=self._pre_create_hook)
...
class NumbaNodeDef(NodeDef):
    ...
    def _create(self, engine, memo):
        ...
        for ((ts_idx, basket_idx), input) in self.ts_inputs():
            exposed_utility_values[NumbaNodeParser.get_ts_input_value_getter_name(ts_idx)] = NumbaTSTypedFunctionResolver.get_value_getter_function(input.tstype.typ)
            cache_key.append(input.tstype)

        for output in self._outputs:
            assert output.kind.is_single_ts()
            ts_idx = output.ts_idx
            exposed_utility_values[NumbaNodeParser.get_ts_out_value_return_name(ts_idx)] = NumbaTSTypedFunctionResolver.get_value_returner_function(output.typ.typ)
        ...
        node = _cspimpl.PyNumbaNode(engine, inputs, outputs, state_ptr_as_int, init_callback_ptr_as_int, callback_ptr_as_int, data_reference)
...
# The decorator
def numba_node(func=None, *, memoize=True, force_memoize=False, debug_print=False, debug_print_python=False,
               state_types: typing.Union[typing.Dict[str, typing.Any], None] = None, pre_create_hook=None):
    """
    :param func:
    ...
    def _impl(func):
        with ExceptionContext():
            ...
            nodetype = NumbaNumbaNodeDefMeta(func.__name__, (NumbaNodeDef,),
                                             {'_signature': parser._signature,
                                              '_impl': parser._impl,
                                              '_state_types': parser._state_types,
                                              'memoize': memoize,
                                              'force_memoize': force_memoize,
                                              'python': _create_node(func=func, func_frame=func_frame,
