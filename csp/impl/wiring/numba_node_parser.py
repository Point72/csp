...
class NumbaNodeParser(NodeParser):
    ...
    def __init__(self, name, raw_func, func_frame, debug_print=False, state_types=None):
        ...
        from csp.impl.wiring.numba_utils.csp_numba_functions import csp_print, csp_unsafe_cast_class_ptr_to_int, csp_unsafe_cast_int_to_class_ptr
        ...
        for kwd in call.keywords:
            if kwd.arg not in self._state_types:
                if isinstance(kwd.value, ast.Num):
                    ...
                    raise CspParseError(f"Unable to resolve type of state variable {kwd.arg}, explicit type must be provided in numba_node decorator", node.lineno)
            self._start_state_block_body.append(ast.Assign(targets=[ast.Name(id=kwd.arg, ctx=ast.Store())],
                                                           value=kwd.value))
    ...
    def visit_Return(self, node):
        ...
        if node.value is not None:
            ...
            res.append(ast.Expr(value=ast.Call(func=ast.Name(id=self.get_ts_out_value_return_name(0), ctx=ast.Load()),
                                               args=[ast.Name(id='__csp_node__', ctx=ast.Load()), ast.Num(n=0), return_value], keywords=[])))
    ...
    def visit_Name(self, node):
        ...
        if ts_idx is not None:
            return ast.Call(func=ast.Name(id=self.get_ts_input_value_getter_name(ts_idx), ctx=ast.Load()),
                            args=[ast.Name(id='__csp_node__', ctx=ast.Load()), ast.Num(n=ts_idx)], keywords=[])
        else:
            return node
    ...
    def _create_single_input_builtin_call(self, arg, func):
        ...
        ts_idx = input.ts_idx
        return ast.Call(func=ast.Name(id=func, ctx=ast.Load()), args=[ast.Name(id='__csp_node__', ctx=ast.Load()), ast.Num(n=ts_idx)], keywords=[])
    ...
    def _create_state_class(self, state_body: typing.List[typing.Any], args: typing.List[str], state_vars: typing.List[str]):
        l_ctx = ast.Load()
        ...
        cls_field_types = [ast.Tuple(elts=[ast.Str(s=arg.arg), ast.Name(id=self.get_arg_type_var_name(arg.arg), ctx=l_ctx)], ctx=l_ctx) for arg in self.get_non_ts_args()]
        cls_field_types += [ast.Tuple(elts=[ast.Str(s=state_var), ast.Name(id=self.get_state_type_var_name(state_var), ctx=l_ctx)], ctx=l_ctx) for state_var in state_vars]
        res = ast.ClassDef(name=self._state_class_name,
                           bases=[],
                           keywords=[],
                           body=[
                               ast.FunctionDef(name='__init__',
                                               ...
                                               decorator_list=[],
                                               returns=None),
                               ast.FunctionDef(name='get_ptr',
                                               args=self._create_ast_args(posonlyargs=[],
                                               ...
                                               body=[
                                                   ast.Return(
                                                       value=ast.Call(func=ast.Name(id='csp_unsafe_cast_class_ptr_to_int', ctx=l_ctx), args=[ast.Name(id='self', ctx=l_ctx)], keywords=[]))],
                                               decorator_list=[],
                                               returns=None)],
                           decorator_list=[
                               ast.Call(func=ast.Attribute(value=ast.Name(id='__csp_numba__', ctx=l_ctx), attr='jitclass', ctx=l_ctx),
                                        args=[
                                            ast.List(
                                                elts=cls_field_types, ctx=l_ctx)],
                                        keywords=[])])
    ...
    def _transform_state_block(self):
        ...
        if self.has_state():
            ...
            self._stateblock = [self._create_state_class(state_body=state_init_body,
                                                         args=self.get_non_ts_args(),
                                                         state_vars=list(self._state_types.keys()))]
    ...
    def _create_function_def(cls, name, body, args=None, decorator_list=None):
        ...
        if args is None:
            res.args = cls._create_ast_args(posonlyargs=[], args=[], kwonlyargs=[], defaults=[], vararg=None, kwarg=None, kw_defaults=[])
        else:
            res.args = args
    ...
    def _create_jit_func(self, func_name, inner_body):
        args = self._create_ast_args(posonlyargs=[], args=[ast.arg(arg='__csp_node__', annotation=None),
                                   ast.arg(arg=self._STATE_ARG_NAME, annotation=None)],
                             kwonlyargs=[], defaults=[], vararg=None, kwarg=None, kw_defaults=[])
        if self.has_state():
            body = [
                       ast.Assign(targets=[Name(id='__csp_state_inst__', ctx=ast.Store())],
                                  value=Call(func=Name(id='csp_unsafe_cast_int_to_class_ptr', ctx=ast.Load()),
                                             args=[Name(id='__csp_state_ptr__', ctx=ast.Load()),
        ...
        decorator_list = [
            Call(func=Attribute(value=Name(id='__csp_numba__', ctx=ast.Load()), attr='cfunc', ctx=ast.Load()),
                 args=[
                     Call(
                         func=Attribute(value=Attribute(value=Name(id='__csp_numba__', ctx=ast.Load()), attr='types', ctx=ast.Load()), attr='void', ctx=ast.Load()),
                         args=[
                             Attribute(value=Attribute(value=Name(id='__csp_numba__', ctx=ast.Load()), attr='types', ctx=ast.Load()), attr='int64', ctx=ast.Load()),
                             Attribute(value=Attribute(value=Name(id='__csp_numba__', ctx=ast.Load()), attr='types', ctx=ast.Load()), attr='int64', ctx=ast.Load())],
                         keywords=[])],
                 keywords=[ast.keyword(arg='cache', value=ast.NameConstant(value=False)), ast.keyword(arg='debug', value=ast.NameConstant(value=False))])]
        inner_body_func = self._create_function_def(name=func_name,
                                                    args=args,
                                                    body=body,
    ...
    def _finalize_func_body(self, newbody):
        ...
        newbody.append(ast.Return(value=ast.Tuple(elts=[state_cls,
                                                        ast.Name(id='__csp__init__state__', ctx=ast.Load()),
                                                        ast.Name(id='__csp__impl__', ctx=ast.Load())], ctx=ast.Load())))
        return newbody
    ...
    def _parse_impl(self):
        ...
        for arg in self.get_non_ts_args():
            self._stateblock.append(ast.Assign(targets=[ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()), attr=arg.arg, ctx=ast.Store())],
    ...
    def _init_internal_maps(cls):
        ...
        cls.METHOD_MAP['csp.make_passive'] = cls._csp_single_input_builtin_function_transformer('__csp_numba_node_make_passive__')
        cls.METHOD_MAP['csp.make_active'] = cls._csp_single_input_builtin_function_transformer('__csp_numba_node_make_active__')
