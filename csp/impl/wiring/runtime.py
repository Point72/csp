...
def build_graph(f, *args, config: Optional[Config] = None, starttime=None, endtime=None, realtime=False, **kwargs):
    assert (starttime is None) == (endtime is None), "Start time and end time should either both be specified or none of them should be specified when building a graph"
    if starttime:
        starttime, endtime = _normalize_run_times(starttime, endtime, realtime)
    with ExceptionContext(), GraphRunInfo(starttime=starttime, endtime=endtime, realtime=realtime), Context(start_time=starttime, end_time=endtime, config=config) as c:
        ...
        if outputs is not None and not isinstance(outputs, CacheWriteOnlyOutputsContainer):
            if isinstance(outputs, Edge):
                ...
                if not isinstance(outputs, OutputsContainer):
                    raise TypeError("graph methods need to return single Edge or OutputsContainer, got '%s'" % (type(outputs)))
...
def run(g, *args, starttime=None, endtime=MAX_END_TIME, config: Optional[Config] = None, queue_wait_time=None, realtime=False, output_numpy=False, **kwargs):
    with ExceptionContext():
        ...
        if isinstance(g, Context):
            ...
            if g.start_time is not None:
                assert (g.start_time, g.end_time) == (starttime, endtime), f"Trying to run graph on period {(starttime, endtime)} while it was built for {(g.start_time, g.end_time)}"

            if Profiler.instance() is not None:
                engine_settings['profile'] = True
                engine_settings['cycle_profile_file'] = getattr(Profiler.instance(), 'cycle_file', '') # optional files to write profiling data to
            ...
            with mem_cache:
                return _run_engine(engine, starttime=starttime, endtime=endtime, context_config=context_config, cache_data=cache_data)
        ...
        graph = _WrappedContext(build_graph(g, *args, starttime=starttime, endtime=endtime, realtime=realtime, config=config, **kwargs))
