...
class Signature:
        ...
        if drop_alarms:
            ...
            for input in self._inputs:
                if input.kind == ArgKind.ALARM:
                    ...
                    new_inputs.append(InputDef(name=input.name, typ=input.typ, kind=input.kind, basket_kind=input.basket_kind, ts_idx=cur_ts_dx, arg_idx=input.arg_idx))
                    cur_ts_dx += 1
        else:
            new_inputs = self._inputs
        return Signature(self._name, new_inputs, self._outputs, defaults=self._defaults, special_outputs=self._special_outputs)
    ...
    def _create_alarms(self, tvars):
        ...
        for alarm in self._alarms:
            alarm = InputDef(name=alarm.name, typ=GenericValuesResolver.resolve_generic_values(alarm.typ, tvars),
                             kind=alarm.kind, basket_kind=alarm.basket_kind, ts_idx=alarm.ts_idx,
    ...
    def resolve_output_definitions(self, basket_shape_eval_inputs, tvars):
        ...
        for output_def in self._iter_all_outputs():
            ...
            if resolved_typ is not output_def.typ or shape is not None:
                ...
                output_def = OutputDef(name=output_def.name, typ=tstyp, kind=output_def.kind,
                                       ts_idx=output_def.ts_idx, shape=shape)
    ...
    def create_outputs(self, nodedef, tvars, basket_shape_eval_inputs=None):
        ...
        if len(output_definitions) > 1 and output_definitions[0].name is None:
            output_definitions = (OutputDef(name=UNNAMED_OUTPUT_NAME, typ=output_definitions[0].typ, kind=output_definitions[0].kind,
                                            ts_idx=output_definitions[0].ts_idx, shape=output_definitions[0].shape),) + output_definitions[1:]

        if output_definitions[0].name is not None:
            idx = itertools.count()
            return OutputsContainer(**{o.name: self._create_output_edges(nodedef, o, next(idx)) for o in output_definitions}), output_definitions
