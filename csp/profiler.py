...
class GraphInfo(Struct):
    ...
    def format_info(self, sort_by, max_nodes):
        ...
        for node, count in node_data[:max_nodes]:
            s += "{0:30} {1}\n".format(node, count)
        s += spacing + "\n"
    ...
    def correct_from_profiler(self):
        ...
        for node in ignore_nodes:
            if node in self.nodetype_counts:
                self.node_count -= self.nodetype_counts[node]
                del self.nodetype_counts[node]
...
def graph_info(g, *args, **kwargs ):
    ...
    return GraphInfo(node_count=node_count, edge_count=edge_count, nodetype_counts=nodetype_counts, longest_path=lp_path[::-1])
...
class ProfilerInfo(Struct):
    ...
    def _cycle_data_as_df(self):
        import pandas as pd
        return pd.DataFrame({"names": ["Number of cycles executed", "Average cycle execution time (s)", "Maximum cycle execution time (s)", "Average graph utilization"], \
            "data": [self.cycle_count, round(self.average_cycle_time, 8), round(self.max_cycle_time, 8), round(self.utilization, 3)]})
    ...
    def _node_data_as_df(self, sort_by: str, max_nodes: int):
        ...
        return pd.DataFrame({"Node Type": [node for node, _ in node_data[:max_nodes]], "Executions":  [data['executions'] for _, data in node_data[:max_nodes]], \
                    "Total Time (s)": [data['total_time'] for _, data in node_data[:max_nodes]], "Average Time (s)": [data['total_time'] / data['executions'] for _, data in node_data[:max_nodes]], \
                    "Max. Time (s)": [data['max_time'] for _, data in node_data[:max_nodes]], "% Total Time": [ round(data['total_time']/total_time * 100, 2) for _, data in node_data[:max_nodes]]})
    ...
    def __sub__(self, other):
        ...
        new_profile = ProfilerInfo( average_cycle_time = other.average_cycle_time, max_cycle_time = other.max_cycle_time, build_time = other.build_time)
