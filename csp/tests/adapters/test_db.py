...
from csp.adapters.db import DBReader, EngineStartTimeAccessor, TimestampAccessor, DateTimeAccessor
...
class TestDBReader(unittest.TestCase):
    ...
    def test_sqlite_basic(self):
        ...
        for time_accessor in (EngineStartTimeAccessor(), TimestampAccessor( time_column = 'TIME', tz = pytz.timezone( 'US/Eastern' ) )):
            def graph():
                reader = DBReader.create_from_connection( connection = engine, time_accessor = time_accessor, table_name = 'test', symbol_column = 'SYMBOL' )
                ...
                aapl_dyn = reader.subscribe('AAPL', None,
                                            field_map={'PRICE': 'PRICE', 'SIZE': 'SIZE', 'SIDE': 'SIDE'})
                aapl_dyn2 = reader.subscribe('AAPL', reader.schema_struct(),
                                            field_map={'PRICE': 'PRICE', 'SIZE': 'SIZE', 'SIDE': 'SIDE'})
            ...
            self.assertEqual([v[1] for v in result['aapl_dyn']], [
    ...
    def test_sqlite_constraints(self):
        ...
        values_list = [
            ...
            { 'DATE': startdate, 'TIME': starttime.replace( second = 1 ), 'SYMBOL': 'IBM', 'PRICE': 100.0, 'SIZE': 200, 'SIDE': 'BUY' },
            { 'DATE': startdate, 'TIME': starttime.replace( second = 2 ), 'SYMBOL': 'AAPL', 'PRICE': 400.0, 'SIZE': 100, 'SIDE': 'BUY' },
            { 'DATE': startdate, 'TIME': starttime.replace( second = 3 ), 'SYMBOL': 'IBM', 'PRICE': 200.0, 'SIZE': 300, 'SIDE': 'SELL' },
            { 'DATE': startdate, 'TIME': starttime.replace( second = 4 ), 'SYMBOL': 'AAPL', 'PRICE': 300.0, 'SIZE': 200, 'SIDE': 'SELL' },
            { 'DATE': startdate, 'TIME': starttime.replace( second = 5 ), 'SYMBOL': 'AAPL', 'PRICE': 200.0, 'SIZE': 400, 'SIDE': 'BUY' },
            { 'DATE': startdate, 'TIME': starttime.replace( second = 6 ), 'SYMBOL': 'GM', 'PRICE': 2.0, 'SIZE': 1, 'SIDE': 'BUY' }
        ]
        engine.execute( query, values_list )

        def graph():
            time_accessor = DateTimeAccessor( date_column = 'DATE', time_column = 'TIME', tz = pytz.timezone( 'US/Eastern' ) )
            reader = DBReader.create_from_connection( connection = engine, table_name = 'test', time_accessor = time_accessor, symbol_column = 'SYMBOL', constraint = db.text( 'PRICE>:price' ).bindparams( price = 100.0 ) )
    ...
    def test_join_query( self ):
        engine = db.create_engine( 'sqlite:///:memory:' ) # in-memory sqlite db
        metadata = db.MetaData()
        test1 = db.Table( 'test1', metadata,
                        db.Column( 'TIME', db.DateTime( timezone = True ) ),
                        db.Column( 'SYMBOL', db.String( 255 ) ),
                        db.Column( 'PRICE', db.Float() ),
                        )
        test2 = db.Table( 'test2', metadata,
                          db.Column( 'TIME', db.DateTime( timezone = True ) ),
                          db.Column( 'SIZE', db.Integer() ),
                          db.Column( 'SIDE', db.String( 255 ) )
                        )
        ...
        def graph():
            ...
            reader = DBReader.create_from_connection( connection = engine.connect(), query = query, time_accessor = time_accessor, symbol_column = 'SYMBOL' )
        ...
        self.assertEqual([v[1] for v in result['aapl_price']], [ 500., 400. ] )
    ...
    def test_DateTimeAccessor(self):
        ...
        query = db.insert(emp)
        values = [ (datetime(2020,3,3,0), 100.),
                   (datetime(2020,3,3,12), 200.),
                   (datetime(2020,3,4,0), 300.),
                   (datetime(2020,3,4,8), 400.),
                   (datetime(2020,3,4,16), 500.),
                   (datetime(2020,3,5,0), 600.),
                   (datetime(2020,3,5,12), 700.),]
        values_list = [
            {'DATE': v[0].date(), 'TIME': v[0].time(), 'SYMBOL': 'AAPL', 'PRICE': v[1]} for v in values
        ]
        engine.execute(query, values_list)

        def graph():
            time_accessor = DateTimeAccessor(date_column='DATE', time_column='TIME', tz=pytz.timezone('US/Eastern'))
            reader = DBReader.create_from_connection(connection=engine, table_name='test', time_accessor=time_accessor,
        ...
        periods = [ (datetime(2020, 3, 3), datetime(2020, 3, 4)),
        ...
        for starttime, endtime in periods:
            result = csp.run(graph, starttime=tz.localize(starttime),
                                    endtime=tz.localize(endtime) )
            target = [ (tz.localize(v[0]).astimezone(pytz.UTC).replace(tzinfo=None), v[1])
                       for v in values if v[0] >= starttime and v[0] <= endtime ]
