import csp
from csp import ts
from csp.adapters.kafka import KafkaAdapterManager, JSONTextMessageMapper, RawBytesMessageMapper, RawTextMessageMapper, DateTimeType, KafkaStartOffset
...
class TestKafka:
    ...
    def test_metadata(self, kafkaadapter):
        def graph(count: int):
            ...
            pub_field_map = {'mapped_a'    : {'a': {'mapped_b': {'b': {'mapped_c': 'c'}}}},
                             'mapped_count': 'count'}

            sub_field_map = {'a'    : {'mapped_a': {'b': {'mapped_b': {'c': 'mapped_c'}}}},
                             'count': 'mapped_count'}

            meta_field_map = {'partition': 'mapped_partition',
    ...
    def test_basic(self, kafkaadapter):
        ...
        def graph(symbols: list, count: int):
            ...
            for symbol in symbols:
                ...
                sub_data = kafkaadapter.subscribe(ts_type=SubData,
        ...
        symbols = ['AAPL', 'MSFT']
        count = 100
        results = csp.run(graph, symbols, count, starttime=datetime.utcnow(), endtime=timedelta(seconds=30), realtime=True)
        for symbol in symbols:
    ...
    def test_start_offsets(self, kafkaadapter, kafkabroker):
        ...
        def get_times_graph():
            ...
            data = kafkaadapter.subscribe(MyData, msg_mapper=msg_mapper, topic=topic, key='AAPL', meta_field_map={'timestamp': 'dt'},
                                          push_mode=csp.PushMode.NON_COLLAPSING)
        ...
        all_data = csp.run(get_times_graph, starttime=datetime.utcnow(), endtime=timedelta(seconds=30), realtime=True)['data']
        ...
        res = csp.run(get_data, KafkaStartOffset.EARLIEST, 10, starttime=datetime.utcnow(), endtime=timedelta(seconds=30), realtime=True)['data']
        # print(res)
        # If we playback from earliest but start "now", all data should still arrive but as realtime ticks
        assert len(res) == 10

        res = csp.run(get_data, KafkaStartOffset.LATEST, 1, starttime=datetime.utcnow(), endtime=timedelta(seconds=1), realtime=True)['data']
        assert len(res) == 0

        res = csp.run(get_data, KafkaStartOffset.START_TIME, 10, starttime=min_time, endtime=timedelta(seconds=30), realtime=True)['data']
        assert len(res) == 10
        ...
        res = csp.run(get_data, stime, len(expected), starttime=datetime.utcnow(), endtime=timedelta(seconds=30), realtime=True)['data']
        assert len(res) == len(expected)

        res = csp.run(get_data, timedelta(seconds=0), len(expected), starttime=stime, endtime=timedelta(seconds=30), realtime=True)['data']
        assert len(res) == len(expected)
    ...
    def test_raw_pubsub(self, kafkaadapter):
        ...
        def graph(symbols: list, count: int):
            t = csp.timer(timedelta(seconds=.1), True)
            d = data(t)
            ...
            for symbol in symbols:
                ...
                sub_data_bytes = kafkaadapter.subscribe(ts_type=bytes,
                                                        msg_mapper=RawTextMessageMapper(),
                                                        field_map='',
                                                        topic=topic,
                                                        key=symbol,
    ...
    def test_invalid_topic(self, kafkaadapter):
        ...
        def graph():
            #csp.print('status', kafkaadapter.status())
            return kafkaadapter.subscribe(ts_type=SubData,
                                                msg_mapper=RawTextMessageMapper(),
                                                field_map={'': 'msg'},
                                                topic='foobar',
                                                key='none')
