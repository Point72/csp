...
class TestNumpyAdapter(unittest.TestCase):
    def test_int(self):
        ...
        for dtype in ('b', 'h', 'i', 'l', 'object'):
            res = csp.run(g, typ=int, values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        unsigned_raw_vals = [2, 6, 13]
        for dtype in ('B', 'H', 'I', 'object'):
            res = csp.run(g, typ=int, values=np.array(unsigned_raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
    ...
    def test_float(self):
        ...
        for dtype in ('f', 'd', 'object'):
            res = csp.run(g, typ=float, values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
    ...
    def test_bool(self):
        ...
        res = csp.run(g, typ=bool, values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
    ...
    def test_complex(self):
        ...
        res = csp.run(g, typ=Foo, values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray,
    ...
    def test_datetime(self):
        ...
        res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype='datetime64[ns]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype='datetime64[us]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype='object'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [datetime(1980,1,1), datetime(1980,1,2), datetime(1980,1,3)]
        res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype='datetime64[s]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype='datetime64[D]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
    ...
    def test_timedelta(self):
        ...
        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='timedelta64[ns]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='timedelta64[ms]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='object'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [timedelta(seconds=3), timedelta(minutes=5), timedelta(hours=7)]
        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='timedelta64[s]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [timedelta(days=3), timedelta(hours=15), timedelta(days=7)]
        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='timedelta64[h]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [timedelta(days=3), timedelta(days=15), timedelta(days=1)]
        res = csp.run(g, typ=timedelta, values=np.array(raw_vals, dtype='timedelta64[D]'),
                      dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
    ...
    def test_array(self):
        ...
        res = csp.run(g, typ=[int], values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [['hello', 'world'], ['hows'], ['it', 'going']]
        res = csp.run(g, typ=[str], values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray,
                      starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
    ...
    def test_numpy_arrays(self):
        # 1D array
        # float
        raw_vals = [ np.array([1., 2.]), np.array([3., 4.]), np.array([5., 6.]) ]
        for dtype in ('f', 'd', 'object'):
            res = csp.run(g, typ=csp.typing.Numpy1DArray[float], values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            np.testing.assert_equal(res['out'], list(zip(test_dts, raw_vals)))

        # int
        raw_vals = [ np.array([1]), np.array([2]), np.array([3]) ]
        for dtype in ('b', 'h', 'i', 'l', 'object'):
            res = csp.run(g, typ=csp.typing.Numpy1DArray[int], values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            np.testing.assert_equal(res['out'], list(zip(test_dts, raw_vals)))

        # str
        raw_vals = [ np.array(['spam', 'ff']), np.array(['eggs', 'bacon']), np.array(['pancakes', 'toast']) ]
        for dtype in ('U', 'U10', 'object'):
            res = csp.run(g, typ=csp.typing.Numpy1DArray[str], values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            np.testing.assert_equal(res['out'], list(zip(test_dts, raw_vals)))

        # object
        raw_vals = [ np.array(Foo(7)), np.array(Foo(11)), np.array(Foo(-5)) ]
        res = csp.run(g, typ=csp.typing.Numpy1DArray[Foo], values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
        ...
        for dtype in ('datetime64[ns]', 'datetime64[s]', 'datetime64[D]'):
            res = csp.run(g, typ=datetime, values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))

        # ND array
        raw_vals = [ np.array([ [1., 2.], [3., 4.] ]), np.array([ [5., 6.], [7., 8.] ]), np.array([ [9., 10.], [11., 12.] ]) ]
        for dtype in ('f', 'd', 'object'):
            res = csp.run(g, typ=csp.typing.NumpyNDArray[float], values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            np.testing.assert_equal(res['out'], list(zip(test_dts, raw_vals)))

        raw_vals = [ np.array([ [1], [2], [3] ]), np.array([ [4], [5], [6] ]), np.array([ [7], [8], [9] ]) ]
        for dtype in ('b', 'h', 'i', 'l', 'object'):
            res = csp.run(g, typ=csp.typing.NumpyNDArray[int], values=np.array(raw_vals, dtype=dtype), dts=test_dts_ndarray, starttime=test_starttime)
            np.testing.assert_equal(res['out'], list(zip(test_dts, raw_vals)))

        # views and striding: transpositions, reshape, flips, broadcast to zero-stride dimensions
        raw_vals = np.array([ np.array([1.,2.,3.]), np.array([4.,5.,6.]), np.array([7.,8.,9.]) ])
        transposed = raw_vals.T # change from C-contig to F-contig
        res = csp.run(g, typ=csp.typing.NumpyNDArray[float], values=transposed, dts=test_dts_ndarray, starttime=test_starttime)
        ...
        res = csp.run(g, typ=csp.typing.NumpyNDArray[str], values=reshaped, dts=test_dts_ndarray, starttime=test_starttime)
        ...
        res = csp.run(g, typ=csp.typing.NumpyNDArray[float], values=flipped, dts=test_dts_ndarray, starttime=test_starttime)
        np.testing.assert_equal(res['out'], list(zip(test_dts, flipped)))

        raw_vals = np.array([ np.array([1.,2.,3.])])
        broadcast = np.broadcast_to(raw_vals, (3,3)) # create 0-stride in first dimension
        res = csp.run(g, typ=csp.typing.NumpyNDArray[float], values=broadcast, dts=test_dts_ndarray, starttime=test_starttime)
    ...
    def test_type_match(self):
        ...
        with self.assertRaisesRegex(ValueError, "numpy type .* requires timedelta output type"):
            raw_vals = [timedelta(seconds=3), timedelta(days=5), timedelta(milliseconds=7)]
            csp.run(g, typ=int, values=np.array(raw_vals, dtype='timedelta64[ns]'), dts=test_dts_ndarray, starttime=test_starttime)
