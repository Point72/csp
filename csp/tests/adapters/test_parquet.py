...
class TestParquetReader(unittest.TestCase):
    ...
    def do_test_body(self, filename, arrow=False):
        ...
        def write_ts_to_file(x: csp.ts['T']):
            ...
            parquet_writer = ParquetWriter(file_name=temp_file.name, timestamp_column_name='TIME', config=config, )
            parquet_writer.publish_struct(x)

        def graph():
            if arrow:
                reader = ParquetReader(filename, symbol_column='SYMBOL', time_column='TIME', binary_arrow=arrow, tz=pytz.utc)
                reader_all = ParquetReader(filename, symbol_column='SYMBOL', time_column='TIME', binary_arrow=arrow, tz=pytz.utc)
            else:
            ...
            aapl2 = reader.subscribe('AAPL', PriceQuantity2,
        ...
        self.assertEqual([v[1] for v in result['aapl']], [
        ...
        self.assertEqual([v[1] for v in result['aapl2']], [
        ...
        self.assertEqual([v[1] for v in result['aapl_price']], [500., 400., 300., 200.])
    ...
    def test_parquet_output_file_rotation(self):
        ...
        def g(d: str, empty_last_file: bool = True):
            ...
            rotate_file = csp.curve(str, [(timedelta(seconds=4), os.path.join(d, 'out1.parquet')),
            ...
            parquet_writer = ParquetWriter(file_name=os.path.expanduser(os.path.join(d, 'out0.parquet')),
                                           timestamp_column_name='csp_timestamp', filename_provider=rotate_file,
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for timestamp_column_name in ['timestamp', None]:
                ...
                def graph():
                    ...
                    writer = ParquetWriter(file_name=filename, timestamp_column_name=timestamp_column_name, config=ParquetOutputConfig(allow_overwrite=True))
                    writer.publish('x', x)
                    writer.publish('y', y)
                ...
                if timestamp_column_name:
                    expected_columns[timestamp_column_name] = [start_time + timedelta(seconds=sec + 1) for sec in range(10)]
                expected_columns.update({'x': list(range(1, 11)),
    ...
    def test_arrow_writer(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            expected_df = pandas.DataFrame.from_dict({'timestamp': [start_time + timedelta(seconds=sec + 1) for sec in range(10)],
                                                      'x': list(range(1, 11)),
                                                      'y': list(map(float, range(0, 100, 10))),
                                                      })
            self.assertTrue((df.dtypes == expected_df.dtypes).all())
    ...
    def test_parquet_multiple_values_same_tick(self):
        ...
        def reader_g2(input_file: str):
            ...
            csp.add_graph_output('symbols', csp.collect([reader.subscribe('AAPL', str, 'symbol'),
                                                         reader.subscribe('IBM', str, 'symbol'),
                                                         reader.subscribe('Z', str, 'symbol')]))
            csp.add_graph_output('values', csp.collect([reader.subscribe('AAPL', float, 'value'),
    ...
    def test_duplicate_field_name(self):
        ...
        def writer_graph(file_name, arrow: bool = False):
            ...
            values = csp.curve(MyStruct, [(timedelta(seconds=1), MyStruct(time=datetime.now(), value=1)),
    ...
    def test_non_python_c_fields(self):
        ...
        with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as f:
            ...
            with open(f.name, 'wb') as w:
                ...
                writer.write_table(pyarrow.table([pyarrow.array([start_t + timedelta(seconds=s) for s in range(5)]),
    ...
    def test_in_memory_arrow_read(self):
        ...
        resT = csp.run(read_tables, pyarrow.concat_tables([empty_t, t1, empty_t, t2, empty_t]), starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # List of tables
    ...
    def test_parquet_mismatched_column_type_error(self):
        ...
        with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
            ...
            with self.assertRaisesRegex(TypeError, ".*Unexpected column type for column value , expected STRING got int64.*"):
    ...
    def test_parquet_schema_change(self):
        ...
        @csp.graph
        def g_read(temp_files: object, read_value2: bool = False, allow_missing_columns: bool = True) -> csp.Outputs(value=csp.ts[int], value2=csp.ts[int]):
            reader = ParquetReader(temp_files, symbol_column='symbol', time_column='csp_timestamp', allow_missing_columns=allow_missing_columns)
            ...
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file2:
                ...
                with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file3:
                    ...
                    with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file4:
                        ...
                        with self.assertRaisesRegex(RuntimeError, ".*Missing column sub.*"):
                            csp.run(g_read_struct, file_names, MyStruct2, starttime=s1, endtime=e4, allow_missing_columns=False)

                        res = csp.run(g_read, file_names, True, starttime=s1, endtime=e4)
                        res2 = csp.run(g_read_struct, file_names, MyStruct, starttime=s1, endtime=e4, allow_missing_columns=False)
                        res3 = csp.run(g_read_struct, file_names, MyStruct2, starttime=s1, endtime=e4)

                        self.assertEqual(res,
                                         {'value': [(datetime(2021, 1, 1, 0, 0), 1), (datetime(2021, 1, 1, 0, 0, 10), 2), (datetime(2021, 1, 1, 0, 0, 20), 3),
                                                    (datetime(2021, 1, 1, 0, 0, 30), 4)],
                                          'value2': [(datetime(2021, 1, 1, 0, 0, 20), 30)]})
                        self.assertEqual(res2,
                                         {0: [(datetime(2021, 1, 1, 0, 0), MyStruct(value=1)), (datetime(2021, 1, 1, 0, 0, 10), MyStruct(value=2)),
                                              (datetime(2021, 1, 1, 0, 0, 20), MyStruct(value=3)),
                                              (datetime(2021, 1, 1, 0, 0, 30), MyStruct(value=4))]}
                                         )
                        self.assertEqual(res3,
                                         {0: [(datetime(2021, 1, 1, 0, 0), MyStruct2(value=1)), (datetime(2021, 1, 1, 0, 0, 10), MyStruct2(value=2, sub=SubStruct())),
                                              (datetime(2021, 1, 1, 0, 0, 20), MyStruct2(value=3, sub=SubStruct(value2=30))), (datetime(2021, 1, 1, 0, 0, 30), MyStruct2(value=4))]}
                                         )
    ...
    def test_parquet_missing_files_read(self):
        ...
        with tempfile.NamedTemporaryFile(prefix='csp_unit_tests_f1', mode='w') as temp_file1:
            ...
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests_f2', mode='w') as temp_file2:
                temp_file2.close()
                csp.set_capture_cpp_backtrace(True)
                csp.run(g_write, temp_file1.name, temp_file2.name, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=60))
                with self.assertRaisesRegex(Exception, ".*Failed to open local file"):
                    res = csp.run(g_read, [temp_file1.name, 'dummy', temp_file2.name], starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=60))

                res = csp.run(g_read, [temp_file1.name, 'dummy', temp_file2.name], allow_missing_files=True,
                              starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=60))
                self.assertEqual(res[0], [(datetime(2022, 1, 1, 0, 0, 1), 1), (datetime(2022, 1, 1, 0, 0, 2), 2), (datetime(2022, 1, 1, 0, 0, 3), 3), (datetime(2022, 1, 1, 0, 0, 4), 4)])
    ...
    def test_numpy_array_on_struct_with_field_map(self):
        ...
        def write_data(file_name: str) -> csp.ts[MyStruct]:
            ...
            values = csp.curve(MyStruct, [(timedelta(0), MyStruct(v1=1, v2='my_s', v3=numpy.array(['a', 'b', 'c']))),
                                          (timedelta(0), MyStruct(v4=numpy.array([1, 2, 3]))),
                                          (timedelta(seconds=5), MyStruct(v1=2, v2='my_s'))])
            parquet_writer.publish_struct(values, field_map={'v1': 'int', 'v2': 'str', 'v3': 'np_arr_str', 'v4':'np_arr_int'})
            return values
    ...
    def test_string_array_with_nulls(self):
        with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
            ...
            with self.assertRaisesRegex(ValueError, ".*Can't read empty value to array from arrow array of type utf8.*"):
    ...
    def test_all_types(self):
        ...
        def write_data(file_name: str, binary_arrow: bool) -> csp.ts[AllTypes]:
            parquet_writer = ParquetWriter(file_name=file_name, timestamp_column_name='TIME', config=ParquetOutputConfig(write_arrow_binary=binary_arrow))

            sType = AllTypesArrow if binary_arrow else AllTypesParquet
            v1 = sType(b = True, i = 123, d = 123.456, dt = datetime.utcnow(), dte = date.today(), t = time(1,2,3),
                       s = 'hello hello', e = MyEnum.A )
            ...
            values = csp.curve(sType, [(timedelta(0), v1),
                                       (timedelta(0), v2),
                                       (timedelta(seconds=5), v3)])
        ...
        for binary_arrow in [ False, True]:
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                temp_file.close()
                v1 = csp.run(write_data, temp_file.name, binary_arrow, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
                v2 = csp.run(read_data, temp_file.name, binary_arrow, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
                self.assertEqual(v1, v2)
    ...
    def test_write_metadata(self):
        ...
        @csp.graph
        def write_data(file_name: str, file_metadata : dict, column_metadata: dict, binary_arrow:bool) -> csp.ts[MyStruct]:
            parquet_writer = ParquetWriter(file_name=file_name, timestamp_column_name='TIME',
                                           file_metadata=file_metadata, column_metadata=column_metadata,
        ...
        for binary_arrow in [False,True]:
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                ...
                column_metadata = { 'a' : { 'type' : 'int' },
                                    'b' : { 'type' : 'float'} }
                csp.run(write_data, temp_file.name, file_metadata, column_metadata, binary_arrow, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
        ...
        with self.assertRaisesRegex(TypeError, 'parquet metadata can only have string values'):
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                temp_file.close()
                csp.run(write_data, temp_file.name, { 'a' : 1}, None, False, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))

        with self.assertRaisesRegex(TypeError, 'parquet column metadata can only have string values, got non-string value for metadata on column \'foo\''):
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                temp_file.close()
                csp.run(write_data, temp_file.name, None, { 'foo' : { 'bar' : 1 }}, False, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))

        with self.assertRaisesRegex(TypeError, 'parquet column metadata expects dictionary entry per column, got unrecognized type for column \'foo\''):
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                temp_file.close()
                csp.run(write_data, temp_file.name, None, { 'foo' : 'bar'}, False, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))

        with self.assertRaisesRegex(ValueError, 'parquet column metadata has unmapped column: \'foo\'' ):
            with tempfile.NamedTemporaryFile(prefix='csp_unit_tests', mode='w') as temp_file:
                temp_file.close()
                csp.run(write_data, temp_file.name, None, { 'foo' : { 'bar' : 'baz' } }, False, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
    ...
    def test_parquet_int_symbol(self):
        ...
        def g_write(temp_file: object):
            ...
            data = csp.count(csp.timer(timedelta(seconds=.1)))
            symbol = _mod( data, 3 )
    ...
    def test_parquet_polars_read(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet') as output:
            output.file.close()
            g = csp.run(test_tz, output.name, start,
                        starttime=start)
