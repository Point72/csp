...
class TestCspDataFrame(unittest.TestCase):
    def test_basic(self):
        ...
        df['b'] = csp.count(csp.timer(timedelta(seconds=.5))) * 10
    ...
    def test_math_ops(self):
        ...
        df['b'] = csp.merge(csp.count(csp.timer(timedelta(seconds=.5))),csp.const(1))
        df['add'] = df['a'] + df['b']
        df['sub'] = df['a'] - df['b']
        df['mul'] = df['a'] * df['b']
        df['div'] = df['a'] / df['b']
    ...
    def test_perspective(self):
        ...
        df = csp.DataFrame()
        df['a'] = csp.count(csp.timer(timedelta(seconds=.01)))
        df['b'] = csp.count(csp.timer(timedelta(seconds=.01))) * 2
