...
class TestPandas(unittest.TestCase):
    ...
    def test_make_pandas_basic(self):
        ...
        out1 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 20, None),
                      starttime=start, endtime=end)[0]
        out2 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 20, tindex=x),
                      starttime=start, endtime=end)[0]
        out3 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 2, tindex=x),
                       starttime=start, endtime=end)[0]
        ...
        target = pd.DataFrame({'x': [2,3,4], 'y': [1.,NaN,2.]}, index=idx)
        ...
        ## Third Tick
        idx = pd.DatetimeIndex([start+dt2, start+dt2+dt1, start+dt2+2*dt1, start+dt2+3*dt1, start+dt2+4*dt1])
        target = pd.DataFrame({'x': [2, 3, 4, 5, 6], 'y': [1., NaN, 2., NaN, 3.]}, index=idx)
    ...
    def test_make_pandas_window(self):
        ...
        out1 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 2, None),
                       starttime=start, endtime=end)[0]
        # Test that a timedelta window works the same way
        out2 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, dt1, None),
                       starttime=start, endtime=end)[0]
        # Test a timedelta window that's too small
        out3 = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, dt1/2, None),
                       starttime=start, endtime=end)[0]
        for out in [out1, out2, out3]:
        ...
        # - when there is missing data in an integer column, it uses float NaN and hence the column becomes float type
        idx = pd.DatetimeIndex([start + dt2, start + dt2 + dt1, start + dt2 + 2 * dt1])
        target = pd.DataFrame({'x': [NaN, 3., 4.], 'y': [1., NaN, 2.]}, index=idx)
        out1[1][1].index.freq = None
        pd.testing.assert_frame_equal(out1[1][1], target)

        idx = pd.DatetimeIndex([start + dt2 + dt1, start + dt2 + 2 * dt1])
        target = pd.DataFrame({'x': [3, 4], 'y': [NaN, 2.]}, index=idx)
        out2[1][1].index.freq = None
        pd.testing.assert_frame_equal(out2[1][1], target)
    ...
    def test_make_pandas_init(self):
        ...
        y = csp.count(csp.timer(dt2))
        q = csp.timer(dt1/2)
        out1 = csp.run(lambda: make_pandas(q, {'x': x, 'y': y}, 20, wait_all_valid=False),
                       starttime=start, endtime=end)[0]
        out2 = csp.run(lambda: make_pandas(q, {'x': x, 'y': y}, 20, wait_all_valid=True),
                       starttime=start, endtime=end)[0]
        ...
        target = pd.DataFrame({'x': [1, 2], 'y': [NaN, 1.]}, index=idx)
...
class TestDataFrame(unittest.TestCase):
    ...
    def test_pandas_ts(self):
        ...
        out = csp.run(lambda: df.to_pandas_ts('y', 2),
                      starttime=start, endtime=end)[0]
        target = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 2),
                         starttime=start, endtime=end)[0]
        ...
        # Case 2: Pass tindex
        out = csp.run( lambda: df.to_pandas_ts('y', 2, 'x'),
                       starttime=start, endtime=end)[0]
        target = csp.run(lambda: make_pandas(y, {'x': x, 'y': y}, 2, tindex=x),
                       starttime=start, endtime=end)[0]
        self.assertEqual(len(out), len(target))
        for i in range(len(out)):
            self.assertEqual(out[i][0], target[i][0])
        ...
        out = csp.run(lambda: df.to_pandas_ts(df['x'], 2, df['x'], wait_all_valid=False),
                      starttime=start, endtime=end)[0]
        target = csp.run(lambda: make_pandas(x, {'x': x, 'y': y}, 2, tindex=x, wait_all_valid=False),
                         starttime=start, endtime=end)[0]
        self.assertEqual(len(out), len(target))
        for i in range(len(out)):
            self.assertEqual(out[i][0], target[i][0])
    ...
    def test_pandas_ts_types(self):
        ...
        x = csp.count(csp.timer(dt1))
        y = csp.count(csp.timer(dt2))
        df = csp.DataFrame({('A','x'):x, ('B','x'):csp.const('Test'),
                            ('A','y'):y, ('B','y'):csp.const(start)})
        out = csp.run(lambda: df.to_pandas_ts(('A','y'), 2, ('A','x')),
                      starttime=start, endtime=end)[0]
        self.assertEqual(len(out), 3)
        idx = pd.DatetimeIndex([start+dt2+dt1, start+dt2+2*dt1])
        target = pd.DataFrame({('A','x'):[3,4],
                               ('B','x'):['Test','Test'],
                               ('A','y'):[1,2],
                               ('B','y'):[start,start]}, index=idx)
