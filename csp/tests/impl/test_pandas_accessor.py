...
def edge_eq(first, second):
    ...
    return first.run(starttime=datetime(2020, 1, 1), endtime=datetime(2020, 1, 2)) \
           == second.run(starttime=datetime(2020, 1, 1), endtime=datetime(2020, 1, 2))
...
class TestCspSeriesAccessor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        idx = ['ABC', 'DEF', 'GJH']
        self.series = pd.Series([csp.const(99.),
                                 csp.timer(timedelta(seconds=1), 103.),
                                 np.nan], dtype=TsDtype(float), index=idx)
    ...
    def test_binop(self):
        flag = pd.Series([csp.const(True),
                          csp.timer(timedelta(seconds=1), False),
                          np.nan], dtype=TsDtype(bool), index=self.series.index)
    ...
    def test_flatten(self):
        ...
        series = pd.Series([csp.const(MyStruct(a=4., b='x', c=SubStruct()))], dtype=TsDtype(MyStruct), name='test')
    ...
    def test_synchronize(self):
        x = csp.const(100.)
        series = pd.Series([x,
                            csp.delay(x, timedelta(seconds=1)),
                            np.nan,
                            csp.delay(x, timedelta(seconds=2))],
                            dtype=TsDtype(float))
        out = series.csp.synchronize(timedelta(seconds=10)).csp.run(starttime=datetime(2020, 1, 1),
        ...
        out = series.csp.synchronize(timedelta(seconds=1)).csp.run(starttime=datetime(2020, 1, 1),
    ...
    def test_run(self):
        ...
        target = pd.Series([99., 103., 103.], index=idx)
        ...
        out = self.series.csp.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=3),
                                  tick_history=timedelta(seconds=1))
        idx = pd.MultiIndex.from_tuples([('ABC', pd.Timestamp('2020-01-01')),
                                         ('DEF', pd.Timestamp('2020-01-01 00:00:02')),
                                         ('DEF', pd.Timestamp('2020-01-01 00:00:03'))])
        target = pd.Series([99., 103., 103.], index=idx)
    ...
    def test_run_repeat(self):
        ...
        t = datetime(2020, 1, 1)
        data = csp.curve(float, [(t, 99.), (t, 98.)])
        self.series = pd.Series([data,
                                 data+4,
                                 np.nan], dtype=TsDtype(float), index=self.series.index)
        ...
        idx = pd.MultiIndex.from_tuples([('ABC', pd.Timestamp('2020-01-01')),
                                         ...
                                         ('DEF', pd.Timestamp('2020-01-01'))])
        target = pd.Series([99., 98., 103., 102.], index=idx)
        pd.testing.assert_series_equal(out, target)

    def test_snap(self):
        out = self.series.csp.snap(starttime=datetime(2020, 1, 1))
        idx = pd.Index(['ABC', 'DEF'])
        target = pd.Series([99., 103.], index=idx)
        pd.testing.assert_series_equal(out, target)
...
class TestCspDataFrameAccessor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        bid = pd.Series([csp.const(99.), csp.timer(timedelta(seconds=1), 103.), np.nan],
                        dtype=TsDtype(float), index=self.idx)
        ask = pd.Series([csp.const(100.), csp.timer(timedelta(seconds=2), 104.), csp.const(100.)],
                        dtype=TsDtype(float), index=self.idx)
        sector = ['X', 'Y', 'X']
    ...
    def test_run(self):
        ...
        records = [('ABC Corp', 99., 100., 'X'),
                   ('DEF Corp', 103., np.nan, 'Y'),
                   ('DEF Corp', 103., 104., 'Y'),
                   ('GJH Corp', np.nan, 100., 'X')]
        ...
        # tick_history test
        out = self.df.csp.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2),
                              tick_history=timedelta(seconds=0.5))
        idx = pd.MultiIndex.from_tuples([('ABC', pd.Timestamp('2020-01-01')),
                                         ('DEF', pd.Timestamp('2020-01-01 00:00:02')),
                                         ('GJH', pd.Timestamp('2020-01-01'))])
        records = [('ABC Corp', 99., 100., 'X'),
    ...
    def test_run_repeat(self):
        ...
        t = datetime(2020, 1, 1)
        bid = csp.curve(float, [(t, 99.), (t, 98.)])
        ask = csp.curve(float, [(t, 100.)])
        self.df['bid']['ABC'] = bid
        self.df['ask']['ABC'] = ask
        ...
        idx = pd.MultiIndex.from_tuples([('ABC', pd.Timestamp('2020-01-01')),
                                         ...
                                         ('GJH', pd.Timestamp('2020-01-01'))])
        records = [('ABC Corp', 99., 100., 'X'),
                   ('ABC Corp', 98., np.nan, 'X'),
                   ('DEF Corp', 103., np.nan, 'Y'),
                   ('DEF Corp', 102., np.nan, 'Y'),
                   ('GJH Corp', np.nan, 100., 'X')]
        target = pd.DataFrame.from_records(records, columns=['name', 'bid', 'ask', 'sector'], index=idx)
        pd.testing.assert_frame_equal(out, target)
    ...
    def test_run_types(self):
        ...
        s_float = pd.Series([csp.const(0.) for _ in self.idx], dtype=TsDtype(float), index=self.idx)
        ...
        self.df = pd.DataFrame({'s_str': s_str, 's_int': s_int, 's_float': s_float, 's_bool': s_bool, 's_date': s_date,
                                's_object': s_object})
        out = self.df.csp.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=0))
        dtypes = pd.Series([object, np.int64, np.float64, bool, object, object],
    ...
    def test_collect_nested(self):
        ...
        series = pd.Series([csp.const(MyStruct(a=4., b='x', c=SubStruct()))], dtype=TsDtype(MyStruct))
    ...
    def _performance_test(self, starttime, endtime, N, M, dt, offset, collect, seed=1234, profile=False):
        ...
        fields = {f'field_{i}': pd.Series([csp.timer(dt + i * offset, 100. + i) for _ in self.idx],
                                          dtype=TsDtype(float), index=self.idx) for i in range(M)}
...
class TestToCspSeriesAccessor(unittest.TestCase):
    def setUp(self) -> None:
        self.idx = pd.DatetimeIndex([pd.Timestamp('2020-01-01'),
                                     pd.Timestamp('2020-01-01 00:00:01'),
                                     pd.Timestamp('2020-01-01 00:00:02')])
        self.midx = pd.MultiIndex.from_tuples([('ABC', pd.Timestamp('2020-01-01')),
                                               ('DEF', pd.Timestamp('2020-01-01 00:00:01')),
                                               ('DEF', pd.Timestamp('2020-01-01 00:00:02'))])
    ...
    def test_to_csp_multiindex(self):
        inp = pd.Series([99., 103., 103.], index=self.midx)
        series = inp.to_csp()
        ...
        inp = pd.Series([99., np.nan, 103.], index=self.midx)
        series = inp.to_csp()
        self.assertEqual(series.dtype, TsDtype(float))
        out = series.csp.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        pd.testing.assert_series_equal(out, inp)
...
class TestToCspFrameAccessor(unittest.TestCase):
    def setUp(self) -> None:
        ...
        bid = pd.Series([99., 103., np.nan, np.nan], index=self.midx)
        ask = pd.Series([100., np.nan, 104., 100.], index=self.midx)
        sector = ['X', 'Y', 'Z', 'X']
    ...
    def test_multiindex(self):
        ...
        self.assertEqual(df['ask'].tolist(), [100., 104., 100.])
