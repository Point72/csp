...
try:
    import perspective
    from csp.impl.pandas_perspective import CspPerspectiveTable, CspPerspectiveMultiTable
    import ipywidgets
except ImportError:
...
class TestCspPerspectiveTable(unittest.TestCase):
    ...
    def test_localize(self):
        ...
        self.df['datetime_col'] = pd.Series([csp.const(datetime(2020, 1, 1, 1)), csp.const(datetime(2020, 1, 1, 2)), np.nan],
                                                       dtype=TsDtype(datetime), index=self.idx)
        ...
        target = pd.Series(['2020-01-01', '2020-01-01 00:00:04', '2020-01-01'],
                           dtype='datetime64[ns]', name='my_timestamp')
        ...
        target = pd.Series(['2020-01-01 01', '2020-01-01 02', np.nan],
                           dtype='datetime64[ns]', name='datetime_col')
        pd.testing.assert_series_equal(utc, target)
    ...
    def test_run_types(self):
        ...
        dtypes = pd.Series({'index': pd.StringDtype(),
    ...
    def test_get_widget(self):
        ...
        self.assertEqual(widget.aggregates, {'name': 'last by index', 'bid': 'last by index', 'ask': 'last by index',
                                             'sector': 'last by index'})
    ...
    def test_multi_table(self):
        tables = {'test1': CspPerspectiveTable(self.df),
                  'test2': CspPerspectiveTable(self.df, keep_history=False)}
    ...
    def test_multi_table_widget(self):
        ...
        config = {'test1': dict(sort=[['foo', 'asc']]),
                  'test3': dict(sort=[['bar', 'desc']])}
