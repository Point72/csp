from csp.impl.pushadapter import PushInputAdapter, PushGroup, PushBatch
...
test_adapter = py_push_adapter_def('test', MyPushAdapter, ts['T'], MyPushAdapterManager, typ='T', interval =int, ticks_per_interval = int)
...
class TestPushAdapter(unittest.TestCase):
    def test_basic(self):
        ...
        csp.run(graph, starttime=datetime.utcnow(), endtime=timedelta(1), realtime=True,
