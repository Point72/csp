from csp.impl.pushpulladapter import PushPullInputAdapter, PushGroup
...
class TestPushPullAdapter(unittest.TestCase):
    def test_basic(self):
        ...
        res = csp.run(graph, num_sim, num_rt, num_sim_repeats, starttime=datetime.utcnow() - timedelta(hours=1),
                      endtime=datetime.utcnow() + timedelta(seconds=30), realtime=True)
        nc = res['nc']
        lv = res['lv']
