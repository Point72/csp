...
class AllTypes(csp.Struct):
    ...
    td : timedelta = timedelta(seconds=.123)
...
class TestCspStruct(unittest.TestCase):
    ...
    def test_copy_from(self):
        ...
        for typ in (BaseNative, BaseNonNative, BaseMixed):
            for typ2 in (DerivedFullyNative, DerivedPartialNative, DerivedMixed, DerivedMixedNoop, DerivedMixedAfterNoop):
    ...
    def test_update(self):
        ...
        self.assertEqual(
            dest,
            DerivedMixed(i2=5, b=True, l2=[1,2,3], s="foo", f2=5.5, s2="bar")
        )
    ...
    def test_from_dict_loop_no_defaults(self):
        looped = StructNoDefaults.from_dict(
            StructNoDefaults(a1=[9, 10]).to_dict()
        )
        self.assertEqual(looped, StructNoDefaults(a1=[9, 10]))

    def test_from_dict_loop_with_defaults(self):
        looped = StructWithDefaults.from_dict(
            StructWithDefaults().to_dict()
        )
    ...
    def test_from_dict_loop_with_generic_typing(self):
        ...
        looped = MyStruct.from_dict(
            MyStruct(foo=set((9, 10)), bar=('a', 'b'), np_arr=np.array([1,3])).to_dict()
        )
    ...
    def test_struct_yaml_serialization(self):
        ...
        self.assertEqual(loaded_struct, S(d={'s1': S1(i=1, f=1.5, b=True, default_i=0),
                                             's2': S1(i=2, f=2.5, b=False)},
                                          ls=[1, 2, 3],
                                          lc=[S2(value=(1, 2, 3), set_value={'x', 'y', 'z'}),
                                              S2(value=(4,))]))
    ...
    def test_struct_printing(self):
        ...
        f4 = 123.456
        f5 = (4/3)*10**(-9)
        s2 = StructB(a=timedelta(1), b=datetime(2020,1,1), c=False, d=f1, e=ClassA(), f=s1, g=EnumA.RED, h=f2, i=f3, j=f4, k=f5 )
        ...
        s5 = StructE(a=StructA(a=1, b='b', c=[1,2]), b=[ StructC(a=2, b='b', c=[3,4], d=2, e='e'), StructC(a=3, b='b', c=[5,6], d=3, e='e') ])
        ...
        exp_str_s6 = 'StructF( a=[1], b=[True], c=[[1.0]], d=[ClassA(a=1)], e=[<EnumA.RED: 1>, <EnumA.BLUE: 2>], f=<unset> )'
        ...
        s12 = StructI(a=1, b='b', c={ 1: StructJ(a=1, b={2: StructK(a=1.0)}, c=StructK()), 2: StructJ(a=2, b={3: StructK(a=2.0)})})
