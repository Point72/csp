...
class TestBasketOutputs(unittest.TestCase):
    ...
    def test_nodes_with_dict_syntax(self):
        ...
        def demux( trade : ts[ Trade ], accounts : [ str ] ) -> csp.Outputs(x=csp.OutputBasket(Dict[str, ts[ Trade ]], shape="accounts")):
            if csp.ticked( trade ):
        ...
        # list version
        @csp.node( debug_print = self.debug_print )
        def demux( trade : ts[ Trade ], accounts : [ str ] ) -> csp.Outputs(x=csp.OutputBasket(List[ts[ Trade ]], shape_of='accounts')):
            if csp.ticked( trade ):
        ...
        # unnamed output version
        @csp.node( debug_print = self.debug_print )
        def demux( trade : ts[ Trade ], accounts : [ str ] ) -> csp.Outputs(csp.OutputBasket(Dict[str, ts[ Trade ]], shape="accounts")):
            if csp.ticked( trade ):
                my_data = { trade.account : trade, 'ALL' : trade }
                csp.output( my_data )
    ...
    def test_main_graph_unnamed_basket_outputs(self):
        ...
        def main_list_basket() -> csp.OutputBasket(List[csp.ts[int]]):
            ...
            return [curve_generator.gen_int_curve(0, 10, 1, skip_indices=[5]), curve_generator.gen_int_curve(100, 10, 1, skip_indices=[4])]

        @csp.graph
        def main_dict_basket() -> csp.OutputBasket(Dict[str, ts[int]]):
            curve_generator = TypedCurveGenerator()
            return {'k1': curve_generator.gen_int_curve(0, 10, 1, skip_indices=[5]), 'k2': curve_generator.gen_int_curve(100, 10, 1, skip_indices=[4])}
    ...
    def test_main_graph_named_basket_outputs(self):
        ...
        def main() -> csp.Outputs(l=csp.OutputBasket(List[csp.ts[int]]), d=csp.OutputBasket(Dict[str, csp.ts[int]])):
            curve_generator = TypedCurveGenerator()
            l = [curve_generator.gen_int_curve(0, 10, 1, skip_indices=[5]), curve_generator.gen_int_curve(100, 10, 1, skip_indices=[4])]
            d = {'k1': curve_generator.gen_int_curve(0, 10, 1, skip_indices=[5]), 'k2': curve_generator.gen_int_curve(100, 10, 1, skip_indices=[4])}
