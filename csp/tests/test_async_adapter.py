"""
Tests for the async adapter module that bridges Python's asyncio with CSP.
"""
...
class TestAsyncFor(unittest.TestCase):
    ...
    def test_async_for_basic(self):
        ...
        @csp.graph
        def graph():
            values = csp.async_for(async_generator(5, delay=0.02))
...
class TestAsyncIn(unittest.TestCase):
    ...
    def test_async_in_basic(self):
        ...
        @csp.graph
        def graph():
            result = csp.async_in(async_return_value(42, delay=0.02))
            csp.add_graph_output("result", result)

        results = csp.run(graph, realtime=True, endtime=timedelta(seconds=0.2))
    ...
    def test_async_in_ticks_once(self):
        ...
        @csp.graph
        def graph():
            result = csp.async_in(async_return_value(100, delay=0.02))
            csp.add_graph_output("result", result)
    ...
    def test_await_blocking(self):
        ...
        def node_with_await() -> ts[int]:
            ...
            if csp.ticked(trigger):
                # Blocking await
                result = csp.await_(async_return_value(99, delay=0.02), block=True)
                return result
    ...
    def test_async_context_basic(self):
        ...
        def node_with_context() -> ts[int]:
            ...
            if csp.ticked(trigger):
                s_counter += 1
                if s_counter <= 3:
                    result = s_ctx.run(async_double(s_counter, delay=0.02))
...
class TestIntegration(unittest.TestCase):
    ...
    def test_combined_async_features(self):
        ...
        def graph():
            ...
            gen_values = csp.async_for(async_generator(3, delay=0.03))
            csp.add_graph_output("gen_values", gen_values)

            # async_in: single async value
            single_value = csp.async_in(async_return_value(42, delay=0.02))
...
class TestSharedLoop(unittest.TestCase):
    ...
    def test_multiple_adapters_use_shared_loop(self):
        """Verify that multiple adapters all use the same shared loop."""
        from csp.impl.async_adapter import get_shared_loop
        ...
        csp.run(graph, realtime=True, endtime=timedelta(seconds=0.3))
...
class TestCspRunAsyncioMode(unittest.TestCase):
    """Tests for csp.run with use_asyncio=True parameter."""

    def test_use_asyncio_requires_realtime(self):
        """Verify that use_asyncio=True requires realtime=True."""
        ...
        with self.assertRaises(ValueError) as context:
            csp.run(graph, starttime=datetime.now(), endtime=timedelta(seconds=0.1), realtime=False, use_asyncio=True)

        self.assertIn("realtime=True", str(context.exception))

    def test_use_asyncio_basic(self):
        """Test basic csp.run with use_asyncio=True."""
        values = []
        ...
        csp.run(graph, realtime=True, endtime=timedelta(milliseconds=200), use_asyncio=True)

        # Should have some values collected
        self.assertGreater(len(values), 0)

    def test_use_asyncio_async_adapter_uses_csp_loop(self):
        """Verify that async adapters use the CSP loop when use_asyncio=True."""
        ...
        result = csp.run(graph, realtime=True, endtime=timedelta(milliseconds=300), use_asyncio=True)
    ...
    def test_use_asyncio_async_for(self):
        """Test async_for works with use_asyncio=True."""
    ...
    def test_use_asyncio_async_in(self):
        """Test async_in works with use_asyncio=True."""
        ...
        output = csp.run(graph, realtime=True, endtime=timedelta(milliseconds=200), use_asyncio=True)
