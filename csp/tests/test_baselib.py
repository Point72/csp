...
class TestBaselib(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        self.assertEqual(result, [(st + timedelta(seconds=1.2), 1),
    ...
    def test_delay(self):
        ...
        def graph():
            ...
            y = csp.count(csp.timer(timedelta(seconds=.5), True))
        ...
        self.assertEqual(results['y'], [(st + timedelta(seconds=1.5 + .5 * x), x + 1) for x in range(0, 18)])
        self.assertEqual(results['z'], [(st + timedelta(seconds=2 + .5 * x), x + 1) for x in range(0, 17)])
    ...
    def test_delay_irregular_spacing(self):
        ...
        self.assertEqual(results['dtime'], [(st + timedelta(seconds=x), y) for x,y in {2: 1, 9: 2, 14: 3, 17: 4}.items()])
    ...
    def test_lag(self):
        ...
        def graph():
            x = csp.curve(typ=int, data=[
                ...
                (datetime(2000, 1, 1), 2), (datetime(2000, 1, 4), 5), (datetime(2000, 1, 5), 12), (datetime(2000, 1, 9), 23)])
        ...
        self.assertEqual(results['lagged_dt'],
                         [(datetime(2000, 1, 4), 2), (datetime(2000, 1, 5), 2), (datetime(2000, 1, 9), 12)])
        self.assertEqual(results['lagged_n'],
                         [(datetime(2000, 1, 5), 2), (datetime(2000, 1, 9), 5)])
    ...
    def test_diff(self):
        ...
        results = csp.run(graph, starttime=datetime(2000, 1, 1), endtime=datetime(2000, 1, 15))
        self.assertEqual(results['diff_dt'],
                         [(datetime(2000, 1, 4), 3), (datetime(2000, 1, 5), 10), (datetime(2000, 1, 9), 11)])
        self.assertEqual(results['diff_n'],
                         [(datetime(2000, 1, 5), 10), (datetime(2000, 1, 9), 18)])
    ...
    def test_merge(self):
        ...
        self.assertEqual(results, [(st + td, 1),
                                   (st + td * 2, 100),
                                   (st + td * 3, 3),
                                   (st + td * 4, 200),
                                   (st + td * 5, 5),
    ...
    def test_unroll(self):
        st = datetime(2020, 1, 1)
        td = timedelta(seconds=1)
        x = csp.curve([int], [(st, [1]),
                              (st + td * 1, [2, 3, 4]),
                              (st + td * 2, [5]),
                              (st + td * 3, []),
                              (st + td * 4, [6, 7, 8, 9, 10]),
                              ])
        x2 = csp.curve([[int]], [(st, [[1]]),
                                 (st + td * 1, [[2, 3, 4]]),
                                 (st + td * 2, [[5]]),
                                 (st + td * 3, [[]]),
                                 (st + td * 4, [[6, 7, 8, 9, 10]]),
    ...
    def test_collect(self):
        ...
        self.assertEqual(results, [(st + timedelta(seconds=1), [0]),
    ...
    def test_default(self):
        ...
        def g():
            ...
            # 456 should win
            csp.add_graph_output('c3', csp.default(csp.const(123, delay=timedelta(seconds=1)), 456, delay=timedelta(seconds=.5)))
            # 123 should win, 456 should not tick
            csp.add_graph_output('c4', csp.default(csp.const(123, delay=timedelta(seconds=1)), 456, delay=timedelta(seconds=2)))
    ...
    def test_exprtk(self):
        ...
        n = csp.curve(np.ndarray, data=[(start_time + timedelta(seconds=i), np.array([1, i, 7 + i], dtype=float) ) for i in range(5)])
        n2 = csp.curve(np.ndarray, data=[(start_time + timedelta(seconds=i), np.array([-2, i, i*i], dtype=float) ) for i in [1, 3, 6]])

        # basic
        results = csp.run(csp.exprtk("x+y", {"x": x, "y": y}), starttime=start_time, endtime=end_time)
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [0, 1, 22, 23, 44])))

        # string input
        results = csp.run(csp.exprtk("if (s == '+') x + y; else if (s == '*') x * y;", {"x": x, "y": y, "s": s}),
                          starttime = start_time, endtime=end_time)
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [0, 1, 40, 60, 160])))

        # state
        results = csp.run(csp.exprtk("FOO := FOO + x", {"x": x}, {"FOO": 100}), starttime=start_time, endtime=end_time)
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [100, 101, 103, 106, 110])))

        # csp.now()
        results = csp.run(csp.exprtk("y + csp.now()", {"y": y}), starttime=start_time, endtime=end_time)
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(0, 5, 2)], [1578009600, 1578009622, 1578009644])))
        ...
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [16, 20, 24, 28, 32])))
        ...
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in [1, 2, 3, 4, 6]], [0, 20, 190, 370, 1480])))

        # function
        results = csp.run(csp.exprtk("2 * foo(x) + bar(x,y)", {"x": x, "y": y}, functions = {"foo": (("x",), "x*x"), "bar": (("x", "y"), "x*y")}), starttime=start_time, endtime=end_time)
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [0, 2, 48, 78, 192])))

        # ndarray output
        results = csp.run(csp.exprtk("return [x, x*x, 10*x]", {"x": x}, output_ndarray=True), starttime=start_time, endtime=end_time)
        expected = list(zip([start_time + timedelta(seconds=i) for i in range(5)], [np.array([0., 0., 0.]), np.array([1., 1., 10.]), np.array([2., 4., 20.]), np.array([3., 9., 30.]), np.array([4., 16., 40.])]))
        ...
        self.assertEqual(results[0], list(zip([start_time + timedelta(seconds=i) for i in range(5)], [0, 77, 154, 231, 308])))
    ...
    def test_math_ops(self):
        ...
        def graph(use_promotion: bool):
            x = csp.count(csp.timer(timedelta(seconds=.25)))
            if use_promotion:
                y = 10
                y_edge = csp.const(y)
            else:
                y = csp.default(csp.count(csp.timer(timedelta(seconds=1))), 1, delay=timedelta(seconds=.25))
                y_edge = y
        ...
        for use_promotion in [False, True]:
            ...
            for op, comp in OPS.items():
                if op in [csp.min, csp.max] and use_promotion:
                    continue
                self.assertEqual([v[1] for v in results[op.__name__]], [comp(x, y) for x, y in zip(xv, yv)], op.__name__)
                self.assertEqual([v[1] for v in results[op.__name__ + '-rev']], [comp(y, x) for x, y in zip(xv, yv)], op.__name__)
    ...
    def test_comparisons(self):
        ...
        for use_promotion in [False, True]:
            ...
            for op, comp in OPS.items():
                self.assertEqual([v[1] for v in results[op.__name__]], [comp(x, y) for x, y in zip(xv, yv)], op.__name__)
                self.assertEqual([v[1] for v in results[op.__name__ + '-rev']], [comp(y, x) for x, y in zip(xv, yv)],
                                 op.__name__)

    def test_boolean_ops(self):
        def graph():
            x = csp.default(csp.curve(bool, [(timedelta(seconds=s), s % 2 == 0) for s in range(1, 20)]), False)
            y = csp.default(csp.curve(bool, [(timedelta(seconds=s * .5), s % 2 == 0) for s in range(1, 40)]), False)
    ...
    def test_multiplex_decode(self):
        ...
        trade_list_1 = [(st + timedelta(seconds=1), Trade(price=100.0, size=50, account='TEST1', symbol='AAPL')),
                        (st + timedelta(seconds=6), Trade(price=101.4, size=500, account='TEST1', symbol='IBM'))]

        trade_list_2 = [(st + timedelta(seconds=2), Trade(price=101.5, size=500, account='TEST2', symbol='AAPL')),
                        (st + timedelta(seconds=5), Trade(price=101.3, size=500, account='TEST2', symbol='IBM'))]

        trade_list_3 = [(st + timedelta(seconds=3), Trade(price=100.50, size=100, account='TEST3', symbol='AAPL')),
                        (st + timedelta(seconds=4), Trade(price=101.2, size=500, account='TEST3', symbol='IBM'))]

        int_trade_list_1 = [(st + timedelta(seconds=1), IntTrade(price=100.0, size=50, account=0, symbol='AAPL')),
                            (st + timedelta(seconds=6), IntTrade(price=101.4, size=500, account=0, symbol='IBM'))]

        int_trade_list_2 = [(st + timedelta(seconds=2), IntTrade(price=101.5, size=500, account=1, symbol='AAPL')),
                            (st + timedelta(seconds=5), IntTrade(price=101.3, size=500, account=1, symbol='IBM'))]

        int_trade_list_3 = [(st + timedelta(seconds=3), IntTrade(price=100.50, size=100, account=2, symbol='AAPL')),
                            (st + timedelta(seconds=4), IntTrade(price=101.2, size=500, account=2, symbol='IBM'))]

        account_list = [(st + timedelta(seconds=1), 'TEST1'),
        ...
        account_list_tick_early = [(st + timedelta(seconds=0.9), 'TEST1'),
        ...
        invalid_account_list = [(st + timedelta(seconds=1), 'INVALID1'),
        ...
        int_account_list = [(st + timedelta(seconds=1), 0),
        ...
        invalid_int_account_list = [(st + timedelta(seconds=1), 4),
        ...
        trade_list = [(st + timedelta(seconds=1), Trade(price=100.0, size=50, account='TEST1', symbol='AAPL')),
        ...
        trade_list_tick_early = [(st + timedelta(seconds=1), Trade(price=100.0, size=50, account='TEST1', symbol='AAPL')),
                                 (st + timedelta(seconds=2), Trade(price=101.5, size=500, account='TEST2', symbol='AAPL')),
                                 (st + timedelta(seconds=3), Trade(price=100.50, size=100, account='TEST3', symbol='AAPL')),
                                 (st + timedelta(seconds=3.9), Trade(price=100.50, size=100, account='TEST3', symbol='AAPL')),  # last tick on test3
                                 (st + timedelta(seconds=4), Trade(price=101.2, size=500, account='TEST3', symbol='IBM')),
                                 (st + timedelta(seconds=4.9), Trade(price=101.5, size=500, account='TEST2', symbol='AAPL')),  # last tick on test2
                                 (st + timedelta(seconds=5), Trade(price=101.3, size=500, account='TEST2', symbol='IBM')),
                                 (st + timedelta(seconds=5.9), Trade(price=100, size=50, account='TEST1', symbol='AAPL')),  # last tick on test2
                                 (st + timedelta(seconds=6), Trade(price=101.4, size=500, account='TEST1', symbol='IBM'))]

        int_trade_list = [(st + timedelta(seconds=1), IntTrade(price=100.0, size=50, account=0, symbol='AAPL')),
        ...
        def decode_graph(test_type=TestType.Standard):
            ...
            if test_type == TestType.InvalidKeys:
                ...
                account = csp.curve(str, account_list[1:])  # test correct keys, but first trade ticks before first key ticks
            else:
    ...
    def test_times(self):
        ...
        times_list = [ datetime(2020,1,1), datetime(2020,1,1) + timedelta(microseconds=1), datetime(2020,1,1) + timedelta(seconds=2)]

        @csp.graph
        def g() -> csp.Outputs(times=csp.ts[datetime], times_f=csp.ts[int]):

            x = csp.curve(typ=bool, data=[
                (times_list[i], True) for i in range(3)
            ])
        ...
        self.assertEqual(timestamps, [(times_list[i], times_list[i].replace(tzinfo=timezone.utc).timestamp()) for i in range(3)] )
    ...
    def test_gate(self):
        ...
        def g():
            ...
            gate = csp.flatten([csp.const(True, delay=timedelta(seconds=2)),
    ...
    def test_drop_dups(self):
        ...
        for k,ts in res.items():
            ...
            for v in ts:
                ...
                self.assertTrue( type(v) != float or type(prev) != float or ( math.isnan(v) != math.isnan(prev)))
                prev = v
    ...
    def test_struct_fromts(self):
        ...
        def g():
            ...
            b = csp.curve(float, [(timedelta(seconds=.2), 123.),  (timedelta(seconds=1), 456.)])
        ...
        self.assertEqual([ x[1] for x in fromts],
                         [ S( a=False),
                           S( a=False, b = 123.),
                           S(a=False,b=123., c='hey'),
                           S(a=True, b=456., c='hey'),
                           S(a=True, b=456., c='yo') ] )
        self.assertEqual(len(fromts_trigger), 2)
        self.assertEqual([x[1] for x in fromts_trigger],
                      [S(a=True, b=456., c='hey'),
                       S(a=True, b=456., c='yo')])
    ...
    def test_struct_collectts(self):
        ...
        self.assertEqual([ x[1] for x in collectts],
                         [ S( a=False),
                           S( b = 123.),
                           S( c='hey'),
                           S(a=True, b=456.),
    ...
    def test_cppimpls(self):
        ...
        def graph():
            ...
            nodes = {
                ...
                csp.demultiplex          : lambda node, typ: node(random_gen(trigger1, typ), random_basket_key(trigger2, basket_keys), basket_keys),
                csp.baselib._delay_by_timedelta: lambda node, typ: node(random_gen(trigger1, typ), timedelta(seconds=.333)),
            ...
            # Struct_fromts
            struct_fromts         = ( csp.baselib._struct_fromts,   lambda node: node(MyStruct2, fields_ts, csp.null_ts(bool), False) )
            struct_fromts_trigger = ( csp.baselib._struct_fromts,   lambda node: node(MyStruct2, fields_ts, trigger1, True) )
    ...
    def test_log(self):
        ...
        exp_out = [ f'CRITICAL:csp:{(st+timedelta(seconds=(i+1))).strftime("%Y-%m-%d %H:%M:%S")} x:{(structs[i])}' for i in range(60)]
        ...
        l = lambda x: 'y:2.0' if x%2 else f'x:1.0'
        exp_out = [ f'CRITICAL:myVeryOwnLogger:{(st+timedelta(seconds=((i+2)//2))).strftime("%Y-%m-%d %H:%M:%S")} {l(i)}' for i in range(120)]
        self.assertEqual(cm.output, exp_out)
        ...
        with self.assertLogs('logger1', level='CRITICAL') as cm1, self.assertLogs('logger2', level='CRITICAL') as cm2, \
                self.assertLogs('logger3', level='CRITICAL') as cm3:
            csp.run(graph, starttime=st, endtime=timedelta(seconds=60))

        exp_out_log1 = [ f'CRITICAL:logger1:{(st+timedelta(seconds=(i+1))).strftime("%Y-%m-%d %H:%M:%S")} x:{float(i+1)}' for i in range(60) ]
        exp_out_log2 = [ f'CRITICAL:logger2:{(st+timedelta(seconds=(i+1))).strftime("%Y-%m-%d %H:%M:%S")} y:{float(i+2)}' for i in range(60) ]
        exp_out_log3 = [ f'CRITICAL:logger3:{(st+timedelta(seconds=(i+1))).strftime("%Y-%m-%d %H:%M:%S")} z:{float(i+3)}' for i in range(60) ]
