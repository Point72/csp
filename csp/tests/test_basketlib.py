...
class TestBasket(unittest.TestCase):
    ...
    def test_cppimpl(self):
        ...
        def test_graph():
            ...
            trigger2 = csp.timer(timedelta(minutes=1) + timedelta(seconds=random.random()*2 - 1)) # Every minute +/- 1 second

            random_floats1 = random_gen(trigger1)
            random_floats_async = random_gen(trigger2)

            # test _basket_synchronize_list
            synced_py = basketlib.sync_list.python(x=[random_floats1, random_floats_async], threshold=self.sync_threshold, output_incomplete=True)
            synced_cpp = basketlib.sync_list(x=[random_floats1, random_floats_async], threshold=self.sync_threshold, output_incomplete=True)

            synced_auto_list = basketlib.sync(x=[random_floats1, random_floats_async], threshold=self.sync_threshold, output_incomplete=True)
            synced_auto_dict = basketlib.sync(x={'a': random_floats1, 'b': random_floats_async}, threshold=self.sync_threshold, output_incomplete=True)
    ...
    def test_basic(self):
        @csp.graph()
        def basic_graph():
            a = csp.curve(typ=float, data=[(datetime(2022, 6, 17, 9, 30), 1.0), (datetime(2022, 6, 17, 9, 45), 2.0), (datetime(2022, 6, 17, 9, 50), 3.0)])
            b = csp.curve(typ=float, data=[(datetime(2022, 6, 17, 9, 35), 4.0), (datetime(2022, 6, 17, 9, 46), 5.0), (datetime(2022, 6, 17, 9, 50), 6.0)])
            c = csp.curve(typ=float, data=[(datetime(2022, 6, 17, 9, 40), 7.0), (datetime(2022, 6, 17, 9, 47), 8.0), (datetime(2022, 6, 17, 9, 50), 9.0)])
        ...
        self.assertEqual(results['synced_0'], [(datetime(2022, 6, 17, 9, 31), 1.0),
                                               (datetime(2022, 6, 17, 9, 46), 2.0),
                                               (datetime(2022, 6, 17, 9, 50), 3.0)])

        self.assertEqual(results['synced_1'], [(datetime(2022, 6, 17, 9, 36), 4.0),
                                               (datetime(2022, 6, 17, 9, 46), 5.0),
                                               (datetime(2022, 6, 17, 9, 50), 6.0)])

        self.assertEqual(results['synced_2'], [(datetime(2022, 6, 17, 9, 41), 7.0),
    ...
    def test_sample_dict_basket(self):
        ...
        def graph():
            ...
            b = csp.curve(typ=float, data=[(datetime(2020, 1, 1), 3.0), (datetime(2020, 1, 1, 0, 0, 49), 4.0), (datetime(2020, 1, 1, 0, 0, 50), 6.0)])
        ...
        ans = [(st + timedelta(seconds=i * 7), 3.0) for i in range(1, 7)]\
    ...
    def test_sample_list_basket(self):
        ...
        st = datetime(2020, 1, 1)
        result = csp.run(graph, starttime=st, endtime=st + timedelta(minutes=1))
        ans = [(st + timedelta(seconds=7), 1.0)]\
            + [(st + timedelta(seconds=i * 7), 2.0) for i in range(2, 9)]
