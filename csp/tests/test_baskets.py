...
class TestBaskets(unittest.TestCase):
    def test_functionality(self):
        ...
        for node, args in zip([list_basket, dict_basket], [[x0, x1, x2, x3],
                                                           {'A': x0, 'B': x1, 'C': x2, 'D': x3}]):
    ...
    def test_dynamic_baskets(self):
        @csp.node
        def gen(x: ts[object], t: 'T') -> csp.Outputs(dyn=csp.DynamicBasket[str, 'T'], changes = ts[list], ticks=ts[list]):
    ...
    def test_dynamic_basket_tick_remove_exception(self) -> csp.OutputBasket({ts[str]:ts[int]}):
        ...
        with self.assertRaisesRegex(RuntimeError, 'Attempted to delete dynamic basket key \'FOOBAR\' which was already ticked this cycle'):
    ...
    def test_dynamic_basket_buffering_policy(self):
        ...
        csp.run(consume, gen(csp.timer(timedelta(seconds=1))), starttime=datetime(2021, 5, 25), endtime=timedelta(hours=1))
