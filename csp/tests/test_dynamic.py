...
class TestDynamic(unittest.TestCase):
    ...
    def test_clean_shutdown(self):
        ...
        def dyn_graph(key: str):
            ...
            # node with alarm
            csp.add_graph_output(f'{key}_alarm', csp.merge(csp.delay(csp.const(1), timedelta(seconds=1)),
                                                           csp.delay(csp.const(2), timedelta(seconds=1))))
    ...
    def test_dynamic_args(self):
        ...
        for key in actual_keys:
            ...
            self.assertEqual([v[1] for v in res[f'{key}_ts']], [DynData(key=key, val=n) for n in range(1, ts_ticks + 1)])
    ...
    def test_nested_dynamics(self):
        ...
        def dyn_graph(key:str, x:ts[int]) -> ts[{str : DynData}]:
            keys = random_keys(list('ABC'), timedelta(seconds=.5), False)
    ...
    def test_initial_ticks(self):
        ...
        def g():
            keys = random_keys(list(string.ascii_uppercase), timedelta(seconds=1), False)
            basket = gen_basket(keys, csp.null_ts([str]))
            csp.dynamic(basket, dyn_graph, csp.attach(), csp.timer(timedelta(seconds=1)), csp.timer(timedelta(seconds=0.17)))
            csp.add_graph_output('keys', keys)
