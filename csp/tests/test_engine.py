...
class TestEngine(unittest.TestCase):
    ...
    def test_multiple_inputs(self):
        ...
        self.assertEqual(result, [(datetime(2020, 2, 7, 9, 0, 2), 3),  # First tick only once x and y are valid
    ...
    def test_active_passive(self):
        ...
        self.assertEqual(result, [(datetime(2020, 2, 7, 9, 0, 1), 1),
    ...
    def test_single_csp_numpy_output(self):
        ...
        x = csp.timer(timedelta(seconds=1), True)
        result = csp.run(count, x, starttime=datetime(2020, 2, 7, 9), endtime=timedelta(seconds=10), output_numpy=True)[0]
        expected_times = np.array(list(np.datetime64(datetime(2020, 2, 7, 9) + timedelta(seconds=x)) for x in range(1,11)))
    ...
    def test_bugreport_csp28(self):
        ...
        result = csp.run(buggy, [csp.const(1, delay=timedelta(seconds=1))], csp.const(True), starttime=datetime.now())[0]
    ...
    def test_output_validation(self):
        ...
        with self.assertRaisesRegex(CspParseError,
                                    'node has __outputs__ defined but no return or csp.output statements'):
            @csp.node
            def n(x: ts[bool]) -> ts[bool]:
                pass
    ...
    def test_cell_access(self):
        ...
        res = csp.run(node, csp.const(1), [csp.const(1)], csp.const(100), [csp.const(200)], 5,
                      starttime=datetime.utcnow())['o2'][0]
        ...
        res = csp.run(node2, csp.const(1), 5,
                      starttime=datetime.utcnow())[0][0]
        ...
        res = csp.run(node3, csp.const(1), 5,
                      starttime=datetime.utcnow())[0][0]
        self.assertEqual(res[1], 5)
    ...
    def test_duplicate_time(self):
        data = [(timedelta(seconds=0), 1),
                (timedelta(seconds=1), 2), (timedelta(seconds=1), 3),
                (timedelta(seconds=2), 4), (timedelta(seconds=2), 5),
    ...
    def test_sim_push_mode(self):
        data = [(timedelta(seconds=0), 1),
                (timedelta(seconds=1), 2), (timedelta(seconds=1), 3),
                (timedelta(seconds=2), 4), (timedelta(seconds=2), 5), (timedelta(seconds=2), 6),
    ...
    def test_managed_sim_input_pushmode(self):
        ...
        TestAdapter = py_managed_adapter_def('test_adapter', TestAdapterImpl, ts['T'], TestAdapterManager, typ='T', id=str)
    ...
    def test_feedback(self):
        ...
        def graph():
            fb = csp.feedback(int)
            with self.assertRaisesRegex(TypeError, re.escape(r'''Expected csp.impl.types.tstype.TsType[''') + '.*' + re.escape(r'''('T')] for argument 'x', got 1 (int)''')):
                fb.bind(1)

            with self.assertRaisesRegex(TypeError, re.escape(r'''Expected ts[T] for argument 'x', got ts[str](T=int)''')):
    ...
    def test_list_feedback_typecheck(self):
        ...
        def g() -> csp.ts[[int]]:
            fb = csp.feedback([int])
            with self.assertRaisesRegex(TypeError, re.escape(r'''Expected ts[T] for argument 'x', got ts[int](T=typing.List[int])''')):
    ...
    def test_union_type_check(self):
        ...
        build_graph(graph, 's')
        with self.assertRaisesRegex(TypeError, "Expected typing.Union\\[int, float, str\\] for argument 'x', got \\[1.1\\] \\(list\\)"):
            build_graph(graph, [1.1])
        ...
        with self.assertRaisesRegex(TypeError, "Expected ts\\[typing.Union\\[int, float, str\\]\\] for argument 'x', got ts\\[typing.List\\[float\\]\\]"):
            build_graph(graph, csp.const([1.1]))

    def test_realtime_timers(self):
        ''' was a bug '''
        rv = csp.run(csp.timer, timedelta(seconds=1), starttime=datetime.utcnow(), endtime=timedelta(seconds=3),
                     realtime=True, queue_wait_time=timedelta(seconds=.001))[0]
    ...
    def test_realtime_timer_lag(self):
        ...
        timer_interval = timedelta(seconds=.1)
    ...
    def test_multiple_alarms_bug(self):
        ...
        self.assertEqual(res,
                         {'o': [(datetime(2020, 1, 1, 0, 0, 5), 1), (datetime(2020, 1, 1, 0, 0, 10), 2)]})
    ...
    def test_memoize_non_comparable(self):
        ...
        csp.run(g, {A(): 'f1', A(): 'f2'}, starttime=datetime(2020, 1, 1, ), endtime=timedelta(seconds=1))
        csp.run(g, {A(), A()}, starttime=datetime(2020, 1, 1, ), endtime=timedelta(seconds=1))
    ...
    def test_curve_multiple_values_same_time(self):
        ...
        def g() -> csp.Outputs(o1=csp.ts[int], o2=csp.ts[int], o3=csp.ts[int]):
            ...
            return csp.output(o1=csp.curve(int, values), o2=csp.curve(int, values, push_mode=PushMode.NON_COLLAPSING),
        ...
        for k in ('o1', 'o2'):
            ...
            self.assertEqual(times, tuple(datetime(2020, 1, 1) + timedelta(seconds=s) for s in [0, 0, 1, 2, 3, 3, 4, 5, 5]))
    ...
    def test_datetime_timedelta_ranges(self):
        ...
        for d in [datetime(2020, 12, 24, 1, 2, 3, 123456),
                  datetime(1970, 1, 1),
    ...
    def test_realtime_endtime(self):
        ...
        csp.run(adapter(), starttime=datetime.utcnow(), endtime=timedelta(seconds=.5), realtime=True, queue_wait_time=timedelta(days=1))

    def test_threaded_run(self):
        # simple test
        runner = csp.run_on_thread(csp.count, csp.timer(timedelta(seconds=1)), starttime=datetime(2021, 4, 23), endtime=timedelta(seconds=60))
        res = runner.join()[0]
    ...
    def test_int_to_float_ts_conversion(self):
        ...
        def basket_wrapper(l: [csp.ts[float]], d: {str: csp.ts[float]}) -> csp.Outputs(
                l=csp.OutputBasket(typing.List[csp.ts[float]], shape_of='l'), d= csp.OutputBasket(typing.Dict[str, csp.ts[float]], shape_of='d')
                ):
            if csp.ticked(l):
        ...
        csp.run(g, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
    ...
    def test_outputs_with_dict_naming(self):
        ...
        rv = csp.run( g, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
    ...
    def test_pass_none_as_ts(self):
        ...
        res1 = csp.run(main, True, False, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
        self.assertEqual(res1['o'][0][1], 6)
        res2 = csp.run(main, True, True, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
        self.assertEqual(res2['o'][0][1], -1)
        res3 = csp.run(main, False, False, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
        self.assertEqual(res3['o'][0][1], 6)
        with self.assertRaises(TSArgTypeMismatchError):
            csp.run(main, False, True, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
    ...
    def test_global_context(self):
        try:
            ...
            res1 = csp.run(g, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
            c = g()
            res2 = csp.run(c, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
            ...
            def g() -> csp.Outputs(a=csp.ts[int], b=csp.ts[int]):
                key = csp.curve(str, [(timedelta(seconds=i), v) for i, v in enumerate(['A', 'B', 'A', 'A', 'A', 'B', 'C', 'B'])])
            ...
            outputs = g()
            res2a = csp.run(outputs.a, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
            self.assertEqual(res2a[0], res1['a'])
            res2b = csp.run(outputs.b, starttime=datetime(2021, 1, 1, ), endtime=timedelta(seconds=10))
    ...
    def test_unnamed_basket_return(self):
        ...
        self.assertEqual(res,
                         {'a': [(datetime(2021, 1, 1, 0, 0), 'v1')], 'b': [(datetime(2021, 1, 1, 0, 0), 'v2')], 'c': [(datetime(2021, 1, 1, 0, 0), 'v1')],
    ...
    def test_delayed_edge(self):
        ...
        with self.assertRaisesRegex(RuntimeError, r'Attempted to bind DelayedEdge multiple times, previously bound to output from node \"csp.const\"' ):
            x.bind( csp.const( 456 ) )

        # Type check
        with self.assertRaisesRegex(TypeError, re.escape(r'''Expected ts[T] for argument 'edge', got ts[int](T=str)''')):
    ...
    def test_graph_shape_bug(self):
        ...
        def aux(x: [ts[float]], y: {str: ts[float]}) -> csp.Outputs(
                o1=csp.OutputBasket(typing.List[ts[float]], shape_of='x'), o2=csp.OutputBasket(typing.Dict[str, ts[float]], shape_of='y')
                ):
            return csp.output(o1=x, o2=y)

        @csp.graph
        def g() -> csp.Outputs(
                o1=csp.OutputBasket(typing.List[ts[float]]), o2=csp.OutputBasket(typing.Dict[str, ts[float]])
                ):
    ...
    def test_graph_node_pickling(self):
        ...
        :return:
        """
        from csp.tests.test_engine import _dummy_graph, _dummy_node, _dummy_graph_cached, _dummy_node_cached
    ...
    def test_separate_graph_build_and_run(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Start time and end time should either both be specified or none of them should be specified when building a graph"):
    ...
    def test_graph_kwargs_return(self):
        ...
        # Error testing
        with self.assertRaisesRegex(csp.CspParseError, 'only unpacking of other csp.node or csp.graph calls are allowed'):
            @csp.graph
