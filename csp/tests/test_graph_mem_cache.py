...
class TestGraphMemCache(unittest.TestCase):
    ...
    def _subgraph_aux(self, memoize=True, force_memoize=False):
        ...
        node_decor = csp.node if memoize and not force_memoize else csp.node(None, memoize=False, force_memoize=force_memoize)
        graph_decor = csp.graph if memoize and not force_memoize else csp.graph(None, memoize=False, force_memoize=force_memoize)

        @node_decor
        def f(inp_ts: csp.ts['T'], inp_basket: [csp.ts['T']], inp_basket2: {str: csp.ts['T']}, scalar: object) -> csp.ts['T']:
    ...
    def test_memoize_disable_enabled(self):
        with csp.memoize(False):
            self._check_no_memoization(*self._subgraph_aux(memoize=True, force_memoize=False), all_memoize_disabled=True)
            ...
            with csp.memoize(True):
                ...
                self._check_no_memoization(*self._subgraph_aux(memoize=False, force_memoize=False), all_memoize_disabled=False)
