...
@csp.node
def _collect_value(x: ts[int], getter_str: str, timestamp_delta: timedelta = None, fixed_time: datetime = None, force_min: int = 0, default_value: object = csp.UNSET):
    with csp.start():
...
class TestHistory(unittest.TestCase):
    def test_simple(self):
        ...
        self.assertEqual(_Globals.instance().values,
                         {0: 4, 1: 3, 2: 2, 3: 1})

        self.assertEqual(_Globals.instance().timestamps,
                         {0: datetime(2020, 2, 7, 9, 0, 4), 1: datetime(2020, 2, 7, 9, 0, 3), 2: datetime(2020, 2, 7, 9, 0, 2), 3: datetime(2020, 2, 7, 9, 0, 1)})

        self.assertEqual(_Globals.instance().items,
                         {0: (datetime(2020, 2, 7, 9, 0, 4), 4), 1: (datetime(2020, 2, 7, 9, 0, 3), 3), 2: (datetime(2020, 2, 7, 9, 0, 2), 2),
    ...
    def test_range_access( self ):
        ...
        @csp.node
        def collect_index( x : ts['T'], startIndex : int, endIndex : int,
                           startIndexPolicy : csp.TimeIndexPolicy, endIndexPolicy : csp.TimeIndexPolicy ) -> csp.Outputs(values = ts[ np.ndarray ], timestamps = ts[ np.ndarray ], items = ts[ tuple ]):
            with csp.start():
        ...
        @csp.node
        def collect_timedelta( x : ts['T'], startIndex : timedelta, endIndex : timedelta,
                               startIndexPolicy : csp.TimeIndexPolicy, endIndexPolicy : csp.TimeIndexPolicy, with_datetime : bool = False ) -> csp.Outputs( 
            ...
            if csp.ticked( x ):
                if with_datetime:
                    csp.output( values,     csp.values_at( x, csp.now() + startIndex, csp.now() + endIndex, startIndexPolicy, endIndexPolicy ) )
                    csp.output( timestamps, csp.times_at(  x, csp.now() + startIndex, csp.now() + endIndex, startIndexPolicy, endIndexPolicy ) )
                    csp.output( items,      csp.items_at(  x, csp.now() + startIndex, csp.now() + endIndex, startIndexPolicy, endIndexPolicy ) )
                else:
                    csp.output( values,     csp.values_at( x, startIndex, endIndex, startIndexPolicy, endIndexPolicy ) )
                    csp.output( timestamps, csp.times_at(  x, startIndex, endIndex, startIndexPolicy, endIndexPolicy ) )
                    csp.output( items,      csp.items_at(  x, startIndex, endIndex, startIndexPolicy, endIndexPolicy ) )
        ...
        @csp.node
        def collect_passive( x : ts[ 'T' ], t : ts[ bool ], startIndex : timedelta, endIndex : timedelta,
                             startIndexPolicy : csp.TimeIndexPolicy, endIndexPolicy : csp.TimeIndexPolicy ) -> csp.Outputs(values = ts[ np.ndarray ], timestamps = ts[ np.ndarray ], items = ts[ tuple ]):
            with csp.start():
                csp.set_buffering_policy( x, tick_history = timedelta( seconds = 5 ) )
                csp.make_passive( x )
        ...
        def graph():
            ...
            x_timedelta = csp.curve( timedelta, [ ( timedelta( seconds = v + 1 ), timedelta( seconds = v + 1 ) ) for v in range( 40 ) ] )
            ...
            x_struct    = csp.curve( MyStruct,  [ ( timedelta( seconds = v + 1 ), MyStruct( i = v + 1, s = 'v+1', b = True ) ) for v in range( 40 ) ] )

            x_duplicate_timestamp = csp.curve( int, [ ( timedelta( seconds = v + 1 ), v + 1 ) for v in range( 35 ) ] +
                                                    [ ( timedelta( seconds = 35 ), v + 1 ) for v in range( 35, 38 ) ] + # t = 35 should have 3 ticks
                                                    [ ( timedelta( seconds = v + 1 ), v + 1 ) for v in range( 38, 40 ) ] )
            x_duplicate_datetime = csp.curve( int, [ ( timedelta( seconds = v + 1 ), v + 1 ) for v in range( 35 ) ] +
                                                   [ ( timedelta( seconds = 35 ), v + 1 ) for v in range( 35, 38 ) ] +
                                                   [ ( timedelta( seconds = v + 1 ), v + 1 ) for v in range( 38, 40 ) ] )
            x_duplicate_enum = csp.curve( MyEnum, [ ( timedelta( seconds = v + 1 ), MyEnum.first ) for v in range( 35 ) ] +
                                                  [ ( timedelta( seconds = 35 ), MyEnum.second ) for v in range( 35, 38 ) ] +
                                                  [ ( timedelta( seconds = v + 1 ), MyEnum.first ) for v in range( 38, 40 ) ] )

            c_bool      = collect_timedelta( x_bool, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                             csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 5
            c_int       = collect_index( x_int,       None, None, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 5
            c_float     = collect_index( x_float,     -3, -2, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 2
            c_timedelta = collect_index( x_timedelta, -4, -2, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 3
            c_enum      = collect_index( x_enum,      -3, -1, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 3
            c_list      = collect_index( x_list,      -3, -2, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 2
            c_string    = collect_index( x_string,    -3, -1, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 4
            c_struct    = collect_index( x_struct,    -4, -2, csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE ) # length: 4

            c_const_inclusive = collect_passive( x_const, t, -timedelta( seconds = 5 ), -timedelta( seconds = 0 ),
                                                 csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE )
            c_const_exclusive = collect_passive( x_const, t, -timedelta( seconds = 5 ), -timedelta( seconds = 0 ),
                                                 csp.TimeIndexPolicy.EXCLUSIVE, csp.TimeIndexPolicy.EXCLUSIVE )
            c_const_extrapolate = collect_passive( x_const, t, -timedelta( seconds = 5 ), -timedelta( seconds = 0 ),
                                                   csp.TimeIndexPolicy.EXTRAPOLATE, csp.TimeIndexPolicy.EXTRAPOLATE )
            c_duplicate_timestamp_exclusive = collect_timedelta( x_duplicate_timestamp, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                 csp.TimeIndexPolicy.EXCLUSIVE, csp.TimeIndexPolicy.EXCLUSIVE )
            c_duplicate_timestamp_inclusive = collect_timedelta( x_duplicate_timestamp, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                 csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE )
            c_duplicate_timestamp_extrapolate = collect_timedelta( x_duplicate_timestamp, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                   csp.TimeIndexPolicy.EXTRAPOLATE, csp.TimeIndexPolicy.EXTRAPOLATE )
            c_extrapolate_short_window = collect_timedelta( x_duplicate_timestamp, -timedelta( seconds = 2 ), -timedelta( seconds = 1 ),
                                                            csp.TimeIndexPolicy.EXTRAPOLATE, csp.TimeIndexPolicy.EXTRAPOLATE )
            c_extrapolate_enum = collect_timedelta( x_duplicate_enum, -timedelta( seconds = 2 ), -timedelta( seconds = 1 ),
                                                    csp.TimeIndexPolicy.EXTRAPOLATE, csp.TimeIndexPolicy.EXTRAPOLATE )
            c_enum_boundary = collect_timedelta( x_duplicate_enum, timedelta( seconds = -5 ), -timedelta( seconds = 0 ),
                                                 csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE )

            c_duplicate_datetime_exclusive = collect_timedelta( x_duplicate_datetime, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                csp.TimeIndexPolicy.EXCLUSIVE, csp.TimeIndexPolicy.EXCLUSIVE, True )
            c_duplicate_datetime_inclusive = collect_timedelta( x_duplicate_datetime, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                csp.TimeIndexPolicy.INCLUSIVE, csp.TimeIndexPolicy.INCLUSIVE, True )
            c_duplicate_datetime_extrapolate = collect_timedelta( x_duplicate_datetime, -timedelta( seconds = 4 ), -timedelta( seconds = 0 ),
                                                                  csp.TimeIndexPolicy.EXTRAPOLATE, csp.TimeIndexPolicy.EXTRAPOLATE, True )
            ...
            csp.add_graph_output( f'values_default', c_duplicate_datetime_extrapolate[ 'values_default' ] )

        results = csp.run( graph, starttime = datetime( 2020, 2, 7, 9 ), endtime = datetime( 2020, 2, 7, 9 ) + timedelta( seconds = 40 ) )
        ...
        self.assertTrue( np.array_equal( int_values[ -2 ], np.array( [ 35, 36, 37, 38, 39 ] ) ) ) # buffering policy is 5, so this tests wrap-around
        ...
        self.assertTrue( np.array_equal( timedelta_values[ -1 ], np.array( [ np.timedelta64( timedelta( seconds = 36 + i ) ) for i in range( 3 ) ] ) ) )
        self.assertTrue( np.array_equal( timedelta_values[ -2 ], np.array( [ np.timedelta64( timedelta( seconds = 35 + i ) ) for i in range( 3 ) ] ) ) )
        ...
        self.assertTrue( np.array_equal( exclusive_timestamp_values[ -1 ], np.array( [ 39 ] ) ) ) # excludes 35, 36, 37, 38 since they were published at t=35
        self.assertTrue( np.array_equal( exclusive_timestamp_values[ -2 ], np.array( [] ) ) ) # includes t=35 to t=39 exclusive
        self.assertTrue( np.array_equal( exclusive_timestamp_values[ -3 ], np.array( [ 32, 33, 34 ] ) ) ) # t=31 to t=35 exclusive
        self.assertTrue( np.array_equal( exclusive_timestamp_values[ -4 ], np.array( [ 32, 33, 34 ] ) ) ) # 4 ticks at this timestamp
        ...
        self.assertTrue( np.array_equal( exclusive_timestamp_values[ -7 ], np.array( [ 31, 32, 33 ] ) ) ) # t=30 to t=34 exclusive
        ...
        self.assertTrue( np.array_equal( inclusive_timestamp_values[ -2 ], np.array( [ 35, 36, 37, 38, 39 ] ) ) ) # includes 35 to 39
        self.assertTrue( np.array_equal( inclusive_timestamp_values[ -3 ], np.array( [ 31, 32, 33, 34, 35, 36, 37, 38 ] ) ) ) # includes 31 to 35
        ...
        self.assertTrue( np.array_equal( extrapolate_timestamps[ -1 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 36, 39, 40 ] ] ) ) )
        self.assertTrue( np.array_equal( extrapolate_timestamps[ -2 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 35, 39 ] ] ) ) )
        self.assertTrue( np.array_equal( extrapolate_timestamps[ -3 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 31, 32, 33, 34, 35, 35, 35, 35 ] ] ) ) )
        ...
        self.assertTrue( np.array_equal( extrapolate_short_window_timestamps[ -1 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 38, 39 ] ] ) ) )
        self.assertTrue( np.array_equal( extrapolate_short_window_timestamps[ -2 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 37, 38 ] ] ) ) )
        self.assertTrue( np.array_equal( extrapolate_short_window_timestamps[ -3 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 33, 34 ] ] ) ) )
        self.assertTrue( np.array_equal( extrapolate_short_window_timestamps[ -4 ], np.array( [ np.datetime64( st + timedelta( seconds = i ) ) for i in [ 33, 34 ] ] ) ) )
        ...
        self.assertTrue( np.array_equal( enum_boundary_values[ -1 ], np.array( [ MyEnum.first, MyEnum.second, MyEnum.second, MyEnum.second, MyEnum.first, MyEnum.first ] ) ) )

        exclusive_datetime_values = [ result[ 1 ] for result in results[ 'values_datetime_exclusive' ] ]
        self.assertTrue( all( np.array_equal( exclusive_datetime_values[ i ], exclusive_timestamp_values[ i ] ) for i in range( 35 ) ) )

        inclusive_datetime_values = [ result[ 1 ] for result in results[ 'values_datetime_inclusive' ] ]
        self.assertTrue( all( np.array_equal( inclusive_datetime_values[ i ], inclusive_timestamp_values[ i ] ) for i in range( 35 ) ) )

        extrapolate_datetime_values = [ result[ 1 ] for result in results[ 'values_datetime_extrapolate' ] ]
        self.assertTrue( all( np.array_equal( extrapolate_datetime_values[ i ], extrapolate_timestamp_values[ i ] ) for i in range( 35 ) ) )

        extrapolate_datetime_timestamps = [ result[ 1 ] for result in results[ 'timestamps_datetime_extrapolate' ] ]
        self.assertTrue( all( np.array_equal( extrapolate_datetime_timestamps[ i ], extrapolate_timestamps[ i ] ) for i in range( 35 ) ) )
    ...
    def test_timedelta(self):
        ...
        self.assertEqual({0: datetime(2020, 2, 7, 9, 0, 4), 1: datetime(2020, 2, 7, 9, 0, 3), 2: datetime(2020, 2, 7, 9, 0, 2), 3: datetime(2020, 2, 7, 9, 0, 1)},
                         _Globals.instance().timestamps)

        self.assertEqual({0: (datetime(2020, 2, 7, 9, 0, 4), 4), 1: (datetime(2020, 2, 7, 9, 0, 3), 3), 2: (datetime(2020, 2, 7, 9, 0, 2), 2),
                          3: (datetime(2020, 2, 7, 9, 0, 1), 1)}, _Globals.instance().items)
    ...
    def test_timedelta_less_than_tick(self):
        ...
        self.assertEqual({0: datetime(2020, 2, 7, 9, 0, 5), 1: datetime(2020, 2, 7, 9, 0, 4), 2: datetime(2020, 2, 7, 9, 0, 4), 3: datetime(2020, 2, 7, 9, 0, 3), 4: datetime(2020, 2, 7, 9, 0, 3)},
                         _Globals.instance().timestamps)
        self.assertEqual({0: (datetime(2020, 2, 7, 9, 0, 5), 5), 1: (datetime(2020, 2, 7, 9, 0, 4), 4), 2: (datetime(2020, 2, 7, 9, 0, 4), 4),
                          3: (datetime(2020, 2, 7, 9, 0, 3), 3), 4: (datetime(2020, 2, 7, 9, 0, 3), 3)},
    ...
    def test_missing_values(self):
        ...
        _Globals.reset()
        csp.run(lambda: raising_graph('value', 42), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        self.assertEqual({0: 4, 1: 3, 2: 2, 3: 1, 4: 42}, _Globals.instance().values)

        _Globals.reset()
        csp.run(lambda: raising_graph('time', datetime(2020, 2, 7, 8, 0, 0)), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        self.assertEqual({0: datetime(2020, 2, 7, 9, 0, 4), 1: datetime(2020, 2, 7, 9, 0, 3), 2: datetime(2020, 2, 7, 9, 0, 2), 3: datetime(2020, 2, 7, 9, 0, 1), 4: datetime(2020, 2, 7, 8, 0)},
                         _Globals.instance().timestamps)

        _Globals.reset()
        csp.run(lambda: raising_graph('item', (datetime(2020, 2, 7, 8, 0, 0), 42)), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        self.assertEqual({0: (datetime(2020, 2, 7, 9, 0, 4), 4), 1: (datetime(2020, 2, 7, 9, 0, 3), 3), 2: (datetime(2020, 2, 7, 9, 0, 2), 2),
                          3: (datetime(2020, 2, 7, 9, 0, 1), 1), 4: (datetime(2020, 2, 7, 8, 0), 42)},
                         _Globals.instance().items)

        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph('value'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph('time'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph('item'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))

        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_time_delta('value'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_time_delta('time'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_time_delta('item'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))

        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_fixed_time('value'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_fixed_time('time'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
        with self.assertRaisesRegex(OverflowError, ".*No matching value found"):
            csp.run(lambda: raising_graph_fixed_time('item'), starttime=datetime(2020, 2, 7, 9), endtime=datetime(2020, 2, 7, 9, 5))
    ...
    def test_duplicate_policy(self):
        ...
        def graph( policy:csp.DuplicatePolicy, num_ticks : int ):
            c = csp.curve( int, [ ( timedelta( seconds = 0 ), v + 1 ) for v in range( 0, num_ticks ) ] +
                                [ ( timedelta( seconds = 1 ), num_ticks + 1) ] )
    ...
    def test_out_of_bounds_exceptions(self):
        ...
        with self.assertRaisesRegex(IndexError,"requesting data at index 20 with buffer policy set to 10 ticks in node \'check\'"):
            csp.run(check, x, -20, starttime=datetime(2020, 11, 11))

        with self.assertRaisesRegex(IndexError, "requesting data at timedelta -1 day, 23:59:40 with buffer policy set to 0:00:10 in node \'check\'"):
    ...
    def test_graph_output(self):
        ...
        with csp.memoize(False):
            rv = csp.run(graph, csp.count(csp.timer(timedelta(seconds=1))), starttime=datetime(2021, 1, 1), endtime=timedelta(seconds=5))
