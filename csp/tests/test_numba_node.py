...
class TestContainerTypes(unittest.TestCase):
    def test_list_in_start_phase(self):
        @numba_node
        def scale_with_start_weights(x: ts[int], weights: NumbaList[int]) -> ts[int]:
            with csp.state():
                s_scale = 0

            with csp.start():
                s_scale = weights[0] + weights[1]
        ...
        def g():
            values = csp.curve(int, [(timedelta(seconds=i), i + 1) for i in range(3)])
            result = scale_with_start_weights(values, [2, 3])
            csp.add_graph_output("result", result)

        results = csp.run(g, starttime=datetime(2024, 1, 1), endtime=timedelta(seconds=10))
        actual = [v for _, v in results["result"]]
        self.assertEqual(actual, [5, 10, 15])

    def test_dict_in_stop_phase(self):
        @numba_node
        def read_constant_mapping_on_stop(x: ts[int], adjustments: NumbaDict[int, int]) -> ts[int]:
            with csp.state():
                s_stop_value = 0

            with csp.stop():
                s_stop_value = adjustments[1]
        ...
        def g():
            values = csp.curve(int, [(timedelta(seconds=i), i + 1) for i in range(3)])
            result = read_constant_mapping_on_stop(values, {1: 7, 2: 11})
            csp.add_graph_output("result", result)

        # The execute output verifies normal operation; successful graph shutdown
        # verifies the constant dict is available in the stop lifecycle phase.
        results = csp.run(g, starttime=datetime(2024, 1, 1), endtime=timedelta(seconds=10))
        actual = [v for _, v in results["result"]]
...
class TestFeatures(unittest.TestCase):
    ...
    def test_dict_basket_string_keys(self):
        @numba_node
        def basket_stats(prices: dict[str, ts[float]]) -> csp.Outputs(ticked_sum=ts[float], valid_sum=ts[float]):
            ticked_sum = 0.0
            valid_sum = 0.0
    ...
    def test_dict_basket_int_keys(self):
        @numba_node
        def weighted_sum(prices: dict[int, ts[float]]) -> ts[float]:
            total = 0.0
            for key in prices.validkeys():
        ...
        @csp.graph
        def g():
            ten = csp.curve(float, [(timedelta(seconds=0), 1.5), (timedelta(seconds=2), 3.0)])
            twenty = csp.curve(float, [(timedelta(seconds=1), 2.0)])
            result = weighted_sum({20: twenty, 10: ten})
            csp.add_graph_output("result", result)

        # t=0: 10 * 1.5 = 15
        # t=1: 10 * 1.5 + 20 * 2.0 = 55
        # t=2: 10 * 3.0 + 20 * 2.0 = 70
        results = csp.run(g, starttime=datetime(2024, 1, 1), endtime=timedelta(seconds=10))
        actual = [v for _, v in results["result"]]
        self.assertEqual(actual, [15.0, 55.0, 70.0])
    ...
    def test_single_set_output(self):
        ...
        def g():
            values = csp.curve(int, [(timedelta(seconds=i), i + 1) for i in range(3)])
            result = emit_scaled(values)
            csp.add_graph_output("result", result)

        results = csp.run(g, starttime=datetime(2024, 1, 1), endtime=timedelta(seconds=10))
        actual = [v for _, v in results["result"]]
        self.assertEqual(actual, [10, 20, 30])
