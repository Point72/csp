import csp
from csp import CspParseError, __outputs__, __return__, ts, Outputs, OutputBasket, OutputTypeError
...
class TestParsing(unittest.TestCase):
    ...
    def test_parse_errors(self):
        ...
        with self.assertRaisesRegex(OutputTypeError, 'Outputs must all be named or be a single unnamed output, cant be both'):
            @csp.node
        ...
        with self.assertRaisesRegex(CspParseError, 'csp.node and csp.graph outputs must be via return annotation or __outputs__ call, not both'):
            @csp.node
            def foo(x: [str]) -> Outputs(ts[int]):
                __outputs__(ts[int])
                pass

        with self.assertRaisesRegex(CspParseError, 'Invalid usage of __outputs__, it should appear at the beginning of the function \\(consult documentation for details\\)'):
            @csp.node
            def foo(x: [str]):
                x = 1
                __outputs__(ts[int])
        ...
        if sys.version_info.major > 3 or sys.version_info.minor >= 8:
            with self.assertRaisesRegex(CspParseError, 'position only arguments are not supported in csp nodes'):
                @csp.node
                def posonly_sample(posonlyargs, /, ):
                    __outputs__(ts[int])
                    pass
        ...
        with self.assertRaisesRegex(CspParseError, 'Returning multiple outputs must use the following syntax: return csp.output\\(out1=val1, \\.\\.\\.\\)'):
            @csp.node
            def foo():
        ...
        with self.assertRaisesRegex(CspParseError, 'csp.output expects to be called with \\(output, value\\) or \\(output = value, output2 = value2\\)'):
            @csp.node
            def foo(x: ts[int]):
                __outputs__(ts[int])
                csp.output(1, 2, 3)
        ...
        with self.assertRaisesRegex(CspParseError, 'return expects to be called with \\(output, value\\) or \\(output = value, output2 = value2\\)'):
            @csp.node
            def foo(x: ts[int]):
                __outputs__(ts[int])
                __return__(1, 2, 3)
        ...
        with self.assertRaisesRegex(CspParseError, 'csp.output\\(x\\[k\\],v\\) syntax can only be used on basket outputs'):
            @csp.node
            def foo(x: ts[int]):
        ...
        with self.assertRaisesRegex(CspParseError, 'Invalid use of return basket element returns is not possible with return'):
            @csp.node
            def foo(x: ts[int]):
                __outputs__(x=ts[bool])
                __return__(x[1], 7)
        ...
        with self.assertRaisesRegex(CspParseError, 'node has __outputs__ defined but no return or csp.output statements'):
            @csp.node
            def foo(x: ts[int]):
                __outputs__(ts[int])
                if csp.ticked(x):
        ...
        with self.assertRaisesRegex(CspParseError, 'output baskets must define shape using with_shape or with_shape_of'):
            @csp.node
            def foo():
    ...
    def test_graph_return_parsing(self):
        ...
        with self.assertRaisesRegex(CspParseError, 'return expects to be called with \\(value\\) or \\(output = value, output2 = value2\\)'):
            @csp.graph
            def graph():
                __outputs__(x=ts[int], y=ts[float])
    ...
    def test_graph_caching_parsing(self):
        with self.assertRaisesRegex(NotImplementedError, 'Caching is unsupported for argument type typing.List\\[int\\] \\(argument x\\)'):
            @csp.graph(cache=True)
            def graph(x: List[int]):
                __outputs__(o=ts[int])
                pass
        with self.assertRaisesRegex(NotImplementedError, 'Caching is unsupported for argument type typing.Dict\\[int, int\\] \\(argument x\\)'):
            @csp.graph(cache=True)
    ...
    def test_output_annotation_parsing_baskets(self):
        ...
        @csp.node
        def my_node_2_1(x: Dict[str, ts[str]], y: List[str]) -> Outputs(
            OutputBasket({str: ts[str]}, shape="y")
        ):
            if csp.ticked(x):
                return x
        ...
        @csp.node
        def my_node_2_2(x: Dict[str, ts[str]]) -> Outputs(
            OutputBasket({str: ts[str]}, shape_of='x')
        ):
            if csp.ticked(x):
                return x
    ...
    def test_output_annotation_parsing_graph_baskets(self):
        ...
        @csp.graph
        def g() ->  csp.Outputs(i=csp.OutputBasket(Dict[str, csp.ts[int]], shape=['V1']),
    ...
    def test_pythonic_node_syntax(self):
        ...
        # Proper parse errors
        node_specific = [ ('state', csp.state), ('alarms', csp.alarms), ('start', csp.start), ('stop', csp.stop),
                     ('__state__', csp.__state__), ('__alarms__', csp.__alarms__), ('__start__', csp.__start__), ('__stop__', csp.__stop__) ]
        ...
        for fn, func in node_or_graph:
            with self.assertRaisesRegex(RuntimeError, f"Unexpected use of {fn}, possibly using outside of @graph and @node?"):
                func()
        ...
        with self.assertRaisesRegex(CspParseError, f'state must be used in a with statement'):
            @csp.node
            def foo():
                csp.state()

        with self.assertRaisesRegex(CspParseError, f'alarms must be used in a with statement'):
            @csp.node
            def foo():
                csp.alarms()

        with self.assertRaisesRegex(CspParseError, f'start must be used in a with statement'):
            @csp.node
            def foo():
                csp.start()

        with self.assertRaisesRegex(CspParseError, f'stop must be used in a with statement'):
            @csp.node
            def foo():
                csp.stop()
    ...
    def test_state_context_logic(self):
        ...
        with self.assertWarnsRegex(DeprecationWarning, 'Only variable assignments and declarations should be present in a csp.state block. Any logic should be moved to csp.start'):
            @csp.node
            def n1():
                with csp.state():
                    i = 0
    ...
    def test_pythonic_alarm_syntax(self):
        ...
        with self.assertRaisesRegex(TypeError, 'function `csp.alarm` requires a single type argument: 0 arguments given'):
            @csp.node
            def n():
                with csp.alarms():
                    a: ts[int]=csp.alarm( )

        with self.assertRaisesRegex(TypeError, 'function `csp.alarm` requires a single type argument: 2 arguments given'):
            @csp.node
            def n():
                with csp.alarms():
    ...
    def test_return(self):
        ...
        # named multiple
        @csp.graph
        def g11() -> csp.Outputs(d1=csp.OutputBasket(Dict[str, ts[int]], shape=['v1', 'v2']), d2=csp.OutputBasket(Dict[str, ts[int]], shape=['v3', 'v4'])):
            x1 = csp.timer(timedelta(seconds=1), 1)
            x2 = csp.timer(timedelta(seconds=1), 2)
            x3 = csp.timer(timedelta(seconds=1), 3)
            x4 = csp.timer(timedelta(seconds=1), 4)
        ...
        @csp.graph
        def g12() -> csp.Outputs(d1=csp.OutputBasket(Dict[str, ts[int]], shape=['v1', 'v2']), d2=csp.OutputBasket(Dict[str, ts[int]], shape=['v3', 'v4'])):
            x1 = csp.timer(timedelta(seconds=1), 1)
            x2 = csp.timer(timedelta(seconds=1), 2)
            x3 = csp.timer(timedelta(seconds=1), 3)
            x4 = csp.timer(timedelta(seconds=1), 4)
        ...
        # smorgasbord
        @csp.node
        def n2(x: ts['T']) -> csp.Outputs(l=csp.OutputBasket(List[ts[int]], shape=2), d=csp.OutputBasket(Dict[str, ts[int]], shape=['v1', 'v2'])):
            csp.output( l[0], 1 )
            csp.output( l[1], 2 )
            csp.output( d['v1'], 3 )
            csp.output( d['v2'], 4 )
        ...
        @csp.graph
        def g13()  -> csp.Outputs(l=csp.OutputBasket(List[ts[int]], shape=2), d=csp.OutputBasket(Dict[str, ts[int]], shape=['v1', 'v2']), s=ts[str]):
            x1 = csp.timer(timedelta(seconds=1), 1)
            x = n2(x1)
            __return__(l=x.l, d=x.d, s=csp.timer(timedelta(seconds=1), "a"))

        @csp.graph
        def g14()  -> csp.Outputs(l=csp.OutputBasket(List[ts[int]], shape=2), d=csp.OutputBasket(Dict[str, ts[int]], shape=['v1', 'v2']), s=ts[str]):
            x1 = csp.timer(timedelta(seconds=1), 1)
            x = n2(x1)
