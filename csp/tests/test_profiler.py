...
import pytz
import string
from csp.tests.test_dynamic import DynData, random_keys, gen_basket
import time as Time
...
class TestProfiler(unittest.TestCase):
    ...
    def test_file_output(self):
        ...
        with open(node_fn, "r") as f:
            self.assertEqual(reduce(lambda a, b: a + b['executions'], prof_info.node_stats.values(), 0), len(f.readlines())-1)
        ...
        self.assertEqual(round(prof_info.node_stats['cast_int_to_float']['max_time'], 4), round(float(max_times.loc[max_times['Node Type'] == 'cast_int_to_float']['Execution Time']), 4))
