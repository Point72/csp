...
class TestStats(unittest.TestCase):
    ...
    def test_deque(self):
        ...
        @csp.graph
        def graph():
            x = csp.curve(typ=float, data=[
                (datetime(2020, 1, i+1), i+1) for i in range(10)])
            y = csp.curve(typ=float, data=[
                (datetime(2020, 1, 1), 1),
                (datetime(2020, 1, 2), 2),
                (datetime(2020, 1, 3), 3),
                (datetime(2020, 1, 8), 8),
            ...
            y_np = csp.curve(typ=np.ndarray, data=[
            ...
            trigger = csp.curve(typ=float, data=[
                (datetime(2020, 1, 4), True),
                (datetime(2020, 1, 12), True)
            ])
            recalc = csp.curve(typ=float, data=[
            ...
            trig_dq_np_time = _window_updates(y_np, timedelta(days=5), trigger, y_np, csp.null_ts(bool), csp.null_ts(bool))
            trig_dq_np_tick = _window_updates(y_np, timedelta(days=5), trigger, y_np, csp.null_ts(bool), csp.null_ts(bool))
        ...
        expected_add = [ (datetime(2020, 1, 1), [1]), (datetime(2020, 1, 4), [2,3]), (datetime(2020, 1, 12), [8,9,10]) ]
        expected_add_np = [ (datetime(2020, 1, 1), [np.array([1.0])]),
        ...
        # recalc handling
        expected_rem = [ (datetime(2020, 1, 1)+timedelta(days=i), [i-4]) for i in [5,7,8,9] ] # no removal on recalc day 7
    ...
    def test_time_window_update_bug(self):
        ...
        def g():
            ...
            updates_np = _window_updates(data_np, timedelta(hours=3), trigger, data_np, csp.null_ts(bool), csp.null_ts(bool))
    ...
    def test_rolling_sum_mean(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), int(dvalues[i])) for i in range(100)])
            x_i = csp.curve(typ=int, data=[
            (st + timedelta(seconds=i + 1), int(dvalues[i])) for i in range(100)])
    ...
    def test_unique(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), dvalues[i]) for i in range(10)])

            trigger = csp.curve(typ=float, data=[
                (st + timedelta(seconds=7), True), (st + timedelta(seconds=9), True)])
    ...
    def test_first_last_ignore_na_false(self):
        ...
        out = csp.stats.last(
            csp.unroll(csp.const([1., np.nan, 3., 4.])), 2, ignore_na=False, min_window=1
        ).run(starttime=st)
        ...
        exp2 = [
            (st + timedelta(seconds=5), np.nan), (st + timedelta(seconds=6), -4),
            (st + timedelta(seconds=7), 0.0), (st + timedelta(seconds=8), 5.0),
            (st + timedelta(seconds=9), np.nan), (st + timedelta(seconds=10), 9.0)
        ]
        self.assertEqual(len(exp2), len(out2))
        for e, o in zip(exp2, out2):
            np.testing.assert_equal(e, o)

        out3 = csp.stats.first(
            csp.unroll(csp.const([1., np.nan, 3., 4.])), 4, ignore_na=False, min_window=1
        ).run(starttime=st)
        self.assertEqual(out3, [(st, 1.0)] * 4)

        out4 = csp.stats.first(
            csp.unroll(csp.const([1., np.nan, 3., 4.])), 2, ignore_na=False, min_window=1
        ).run(starttime=st)
        ...
        out5 = csp.stats.first(
            csp.unroll(csp.const([1., np.nan, 3., 4.])), 2, ignore_na=False, min_window=2
        ).run(starttime=st)
    ...
    def test_rolling_min_max(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (datetime(2020, 1, 1)+timedelta(milliseconds=i+1), dvalues[i]) for i in range(30)])
            # By time interval
    ...
    def test_rolling_var_sd_sem_skew_kurt(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st+timedelta(seconds=i+1), dvalues[i]) for i in range(1000)])
        ...
        expected_sem = np.array(
            [(st + timedelta(seconds=i + 1), pdsem[i]) for i in range(1000)])
        expected_skew_unbias = np.array(
            [(st + timedelta(seconds=i + 1), pdskew_unbias[i]) for i in range(1000)])
        expected_kurt_excess = np.array(
            [(st + timedelta(seconds=i + 1), pdkurt_unbias[i]) for i in range(1000)])
    ...
    def test_ema(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(milliseconds=i + 1), dvalues[i]) for i in range(1000)])
    ...
    def test_triggers(self):
        ...
        @csp.graph
        def graph():
            trigger = csp.curve(typ=bool, data=[
            ...
            x = csp.curve(typ=int, data=[
                (st + timedelta(seconds=i + 1), dvalues[i]) for i in range(20)] +
                          [(st + timedelta(seconds=25), 2),
                           (st + timedelta(seconds=26), 2)])

            x_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i + 1), np.array( [[dvalues[i], dvalues[i]], [dvalues[i], dvalues[i]]], dtype=float )) for i in range(20)] +
                    [(st + timedelta(seconds=25), np.array([[2.,2.],[2.,2.]])),
                    (st + timedelta(seconds=26), np.array([[2.,2.],[2.,2.]]))]
            )

            # calculate values only on trigger
            dq = _window_updates(csp.cast_int_to_float(x), interval=timedelta(seconds=3), trigger=trigger, sampler=x, reset=csp.null_ts(float), recalc=csp.null_ts(float))
        ...
        exp_sum = [(st + timedelta(seconds=3.5), 6),
        ...
        exp_mean = [(st + timedelta(seconds=6.5), 5),
        ...
        exp_std = [(st + timedelta(seconds=6.5), np.std([4,5,6], ddof=1)),
        ...
        exp_max = [(st + timedelta(seconds=6.5), 6),
        ...
        exp_gmean = [(st + timedelta(seconds=6.5), 120**(1/3)),
        ...
        exp_sum[-2:] = [(st + timedelta(seconds=23.5), 57),
                (st + timedelta(seconds=26.5), 24)]

        exp_mean[-2:] = [(st + timedelta(seconds=23.5), 19),
                    (st + timedelta(seconds=26.5), 8)]

        exp_std[-2:] = [(st + timedelta(seconds=23.5), np.std([18,19,20], ddof=1)),
                    (st + timedelta(seconds=26.5), np.std([20,2,2], ddof=1))]

        exp_max[-2:] = [(st + timedelta(seconds=23.5), 20),
                   (st + timedelta(seconds=26.5), 20)]

        exp_gmean[-2:] = [(st + timedelta(seconds=23.5), 6840**(1/3)),
    ...
    def test_nan_handling_base(self):
        ...
        def graph():
            y = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), val) for i, val in enumerate(my_curve)
            ])
        ...
        expected_count_wn = np.array([1,2,3,float('nan'),float('nan'),float('nan'),float('nan'),float('nan'),float('nan'),3])
    ...
    def test_nan_handling_ema(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), dvalues[i]) for i in range(12)])
        ...
        np.testing.assert_almost_equal(np.array(pd_alpha_var)[1:], np.array(results['ema_var_ign'])[:, 1][1:], decimal=7)
        ...
        np.testing.assert_almost_equal(np.array(exp_ema_hz_ign), np.array(results['ema_horizon_ignore'])[:, 1], decimal=7)
        np.testing.assert_almost_equal(np.array(exp_ema_hz_wn), np.array(results['ema_horizon_withnan'])[:, 1], decimal=7)
    ...
    def test_adv_ema(self):
        ...
        def graph():
            x = csp.curve(typ=int, data=[
                (st + timedelta(seconds=i + 1), val) for i, val in enumerate(my_data)])
        ...
        # Test horizon
        exp_ema = [(0.081*x[0]+0.09*x[1]+0.1*x[2]) / (0.081+0.09+0.1) for x in [
            [i,i+1,i+2] for i in range(8)
        ]]
        np.testing.assert_almost_equal(exp_ema, np.array(results['ema'])[:,1], decimal=7)

        # Test half-life
        exp_emahalf = [ sum(0.5**(i-k)*my_data[k] for k in range(i+1)) / sum(0.5**(i-k) for k in range(i+1))
                        for i in range(2,10)]
        ...
        def graph(hl_days: float):
            d = csp.curve(typ=np.ndarray, data=[(datetime(2010, 1, 4, 7, 0), np.array([1.0, 2.0, 3.0])),
        ...
        np.testing.assert_almost_equal(np.array(r1['out'], dtype=object)[0, 1], np.array([1., 2., 3.], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r1['out'], dtype=object)[1, 1], np.array([1.4, 2.4, 3.4], dtype=object), decimal=4)

        r2 = csp.run(graph, 1, starttime=datetime(2010, 1, 4, 7, 0), endtime=datetime(2010, 1, 5, 16, 0))
        np.testing.assert_almost_equal(np.array(r2['out'], dtype=object)[0, 1], np.array([1., 2., 3.], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r2['out'], dtype=object)[1, 1], np.array([1.3333, 2.3333, 3.3333], dtype=object), decimal=4)

        r3 = csp.run(graph, 1+1e-5, starttime=datetime(2010, 1, 4, 7, 0), endtime=datetime(2010, 1, 5, 16, 0))
        np.testing.assert_almost_equal(np.array(r3['out'], dtype=object)[0, 1], np.array([1., 2., 3.], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r3['out'], dtype=object)[1, 1], np.array([1.3333, 2.3333, 3.3333], dtype=object), decimal=4)

        r4 = csp.run(graph, 2, starttime=datetime(2010, 1, 4, 7, 0), endtime=datetime(2010, 1, 5, 16, 0))
        norm = 1+1/math.sqrt(2)
        np.testing.assert_almost_equal(np.array(r4['out'], dtype=object)[0, 1], np.array([1., 2., 3.], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r4['out'], dtype=object)[1, 1], np.array([(1/math.sqrt(2)*1+1.5)/norm, (1/math.sqrt(2)*2+2.5)/norm, (1/math.sqrt(2)*3+3.5)/norm], dtype=object), decimal=4)
        ...
        norm1 = 1 + k1
        norm2 = norm1 + k2
        np.testing.assert_almost_equal(np.array(r5['out'], dtype=object)[0, 1], np.array([1., 2., 3.], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r5['out'], dtype=object)[1, 1], np.array([(k1*1+1.5)/norm1, (k1*2+2.5)/norm1, (k1*3+3.5)/norm1], dtype=object), decimal=4)
        np.testing.assert_almost_equal(np.array(r5['out'], dtype=object)[2, 1], np.array([(k2*1 + k1*1.5 + 2)/norm2, (k2*2 + k1*2.5 + 3)/norm2, (k2*3 + k1*3.5 + 4)/norm2], dtype=object), decimal=4)
    ...
    def test_cov_corr(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), val) for i, val in enumerate(data1)])
            y = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), val) for i, val in enumerate(data2)])
    ...
    def test_irregularly_spaced_data(self):
        ...
        def graph():
            ...
            y = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i+1), i+1) for i in range(100)
            ])
            y_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i+1), np.array([i+1], dtype=float)) for i in range(100)
            ])
            trigger = csp.curve(typ=bool, data=[
                (st + timedelta(seconds=30), True), (st + timedelta(seconds=70), True)
            ])
            trigger_partial = csp.curve(typ=bool, data=[
                (st + timedelta(seconds=2), True), (st + timedelta(seconds=70), True)
            ])
            ...
            mean_tp = csp.stats.mean(y, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), trigger=trigger_partial)
            ...
            reset_partial = csp.curve(typ=bool, data=[
                (st + timedelta(seconds=2), True), (st + timedelta(seconds=7), True)
            ])
            reset_mean_t = csp.stats.mean(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), reset=reset_partial)
        ...
        np.testing.assert_almost_equal(np.array(exp_mean_reset_t)[:], np.array(results['reset_mean_t'])[:,1], decimal=7)
        np.testing.assert_almost_equal(np.array(exp_mean_reset_n)[:], np.array(results['reset_mean_n'])[:,1], decimal=7)
    ...
    def test_quantiles_median(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), val) for i, val in enumerate(data)])

            low = csp.stats.quantile(x, interval=timedelta(seconds=120), quant=[0.25, 0.5, 0.75], min_window=timedelta(seconds=2), interpolate="lower")
            ...
            high = csp.stats.quantile(x, interval=timedelta(seconds=120), quant=0.25, min_window=timedelta(seconds=2), interpolate="higher")
            near = csp.stats.quantile(x, interval=timedelta(seconds=120), quant=0.333, min_window=timedelta(seconds=2), interpolate="nearest")
    ...
    def test_weighted_statistics(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), x_data[i]) for i in range(10)])
            x2 = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), x_data[i]) for i in range(30)])
            y = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), y_data[i]) for i in range(10)])
            x_int = csp.curve(typ=int, data=[
                (st + timedelta(seconds=i + 1), x_data[i]) for i in range(10)])

            weights1 = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), 2*x_data[i]) for i in range (10)])
            weights2 = csp.curve(typ=float, data=[
                (st + timedelta(seconds=2*i + 1), x_data[i]) for i in range (5)])
            weights3 = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), w_data[i]) for i in range (10)])
            ...
            wpsum = csp.stats.sum(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), precise=True, weights=weights2)
            wmean = csp.stats.mean(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), weights=weights1)

            # var, std, sem, cov, corr
            wvar = csp.stats.var(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=3), ddof=0, weights=weights3)
            wstd = csp.stats.stddev(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=3), ddof=0, weights=weights3)
            wsem = csp.stats.sem(x, interval=timedelta(seconds=3), min_window=timedelta(seconds=3), ddof=0, weights=weights3)
            wcov = csp.stats.cov(x, y, interval=timedelta(seconds=3), min_window=timedelta(seconds=3), ddof=0, weights=weights3)
            wcorr = csp.stats.corr(x, y, interval=timedelta(seconds=3), min_window=timedelta(seconds=3), weights=weights3)

            # kurtosis and skew - should converge to unweighted with linearly increasing weights
            wkurt = csp.stats.kurt(x2, interval=timedelta(seconds=10), min_window=timedelta(seconds=5), bias=False, weights=weights1)
            kurt = csp.stats.kurt(x2, interval=timedelta(seconds=10), min_window=timedelta(seconds=5), bias=False)
            wskew = csp.stats.skew(x2, interval=timedelta(seconds=10), min_window=timedelta(seconds=5), bias=True, weights=weights1)
            skew = csp.stats.skew(x2, interval=timedelta(seconds=10), min_window=timedelta(seconds=5), bias=True)

            # make sure ints work
            int_sum = csp.stats.sum(x_int, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), weights=weights1)
            int_mean = csp.stats.mean(x_int, interval=timedelta(seconds=3), min_window=timedelta(seconds=1), weights=weights1)
        ...
        # test mean
        exp_mean = [1, 5/3] + [ sum(2*i**2 for i in range(k,k+3) ) / sum(2*i for i in range(k,k+3) )
                                for k in range(1,9) ]
        ...
        def weighted_cov(x, y, weights, interval, ddof, total_span):
            return [ np.cov( [ x[i] for i in range(j,j+interval) ], [ y[i] for i in range(j,j+interval) ], aweights=[ weights[i] for i in range(j,j+interval)], ddof=ddof )[0][1] for j in range(total_span) ]
        ...
        def weighted_corr(x, y, weights, interval, total_span):
            return weighted_cov(x, y, weights, interval, 0, total_span) / ( weighted_std(x, weights, interval, 0, total_span) * weighted_std(y, weights, interval, 0, total_span)) # only if all ticks are non-nan
    ...
    def test_numerical_stability(self):
        ...
        @csp.graph
        def graph():
            weights = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), weight_func(i)) for i in range(n_samples)])

            weights_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i + 1), np.array([weight_func(i), weight_func(i)], dtype=np.float64) ) for i in range(n_samples)])

            # every 10 ticks, set the next 10 ticks to zero
            x = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), 1) for i in range(n_samples)])
            x_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i + 1), np.array([1., 1.], dtype=np.float64)) for i in range(n_samples)])
    ...
    def test_numpy_statistics(self):
        ...
        def graph():
            x_np = csp.curve(typ=np.ndarray, data=[
                ...
                np.arange(i+1, i+11, dtype=np.float64).reshape(2,5))
                for i in range(10)
            ])

            v_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i+1),
                np.array([i+1, -i, 2*i], dtype=np.float64).reshape(3,))
                for i in range(10)
            ])
            v2_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i+1),
                np.array([-i, 2*i, i+1], dtype=np.float64).reshape(3,))
                for i in range(10)
            ])
            weights = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i+1), np.full((2,5), i+1, dtype=np.float64))
                for i in range(10)
            ])
            weights_f = csp.curve(typ=int, data=[
                (st + timedelta(seconds=i+1), i+1)
                for i in range(10)
            ]) # int typed
            weights_np = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=i+1), np.array([i+1, i+1, i+1], dtype=float))
                for i in range(10)
        ...
        expected_sum = [(st+ timedelta(seconds=i), np.sum( [np.arange(k,k+10).reshape(2,5) for k in range(i-2, i+1)], axis=0)) for i in range(3,11) ]
        np.testing.assert_equal(expected_sum, results['sum_np'])

        expected_wsum = [(st+ timedelta(seconds=i), np.sum( [k*np.arange(k,k+10).reshape(2,5) for k in range(i-2, i+1)], axis=0)) for i in range(3,11) ]
        np.testing.assert_equal(expected_wsum, results['wsum_np'])

        # test mean
        expected_mean = [ np.sum( [1/3*np.arange(k,k+10).reshape(2,5) for k in range(i-2, i+1)], axis=0) for i in range(3,11) ]
        np.testing.assert_almost_equal(expected_mean, list_nparr_to_matrix(results['mean_np']), decimal=7)
        expected_wmean = [ np.sum( [k/(3*i-3)*np.arange(k,k+10).reshape(2,5) for k in range(i-2, i+1)], axis=0) for i in range(3,11) ]
        ...
        expected_wcov = [ np.cov( [ [ i+1 for i in range(j,j+5) ], [-i for i in range(j,j+5)], [2*i for i in range(j,j+5)] ], aweights=[k+1 for k in range(j,j+5)], ddof=0 )
                        for j in range(6) ]
        ...
        for i in range(6):
            # pairwise cov/corr
            np.testing.assert_almost_equal(expected_cov, np.array(results['cov_matrix_np'], dtype=object)[i,1], decimal=7)
            np.testing.assert_almost_equal(expected_wcov[i], np.array(results['wcov_matrix_np'], dtype=object)[i,1], decimal=7)
            np.testing.assert_almost_equal(expected_corr, np.array(results['corr_matrix_np'], dtype=object)[i,1], decimal=7)
            np.testing.assert_almost_equal(expected_wcorr, np.array(results['wcorr_matrix_np'], dtype=object)[i,1], decimal=7)
    ...
    def test_numpy_shape_setting(self):
        ...
        @csp.graph
        def g():
            x = csp.curve(typ=np.ndarray, data=[
                (datetime(2020,1,2), np.array([1.0,1.0]))
            ])
            trigger = csp.curve(typ=bool, data=[
                (datetime(2020,1,6), True)
            ])
            reset = csp.curve(typ=bool, data=[
                (datetime(2020,1,1), True)
            ])
        ...
        np.testing.assert_equal(res['sum_np_t'][0][1], np.array([0.,0.]))


    def test_numpy_nan_handling(self):
        # 3x1 vector
        my_curve = [
            np.array([i+1, i+1, i+1], dtype=float).reshape(3,) for i in range(10)
        ]
        ...
        def graph():
            x = csp.curve(typ=np.ndarray, data=[
                (st+timedelta(seconds=i + 1), my_curve[i]) for i in range(len(my_curve))
            ])
            x_samp = csp.curve(typ=np.ndarray, data=
                [(st+timedelta(seconds=i + 1), my_curve[i]) for i in range(4)] +
                [(st + timedelta(seconds=6), my_curve[5])] +
                [(st+timedelta(seconds=i + 1), my_curve[i]) for i in range(7,10)])
            sampler = csp.curve(typ=bool, data=[
                (st+ timedelta(seconds=i + 1), True) for i in range(10)
            ])
        ...
        expected_sum_ig = [np.array([v, v, v]).reshape(3,) for v in expected_sum_ig]
        ...
        expected_mean_ig = [np.array([v, v, v]).reshape(3,) for v in expected_mean_ig]
    ...
    def test_sampler_nan_handling(self):
        ...
        @csp.graph
        def graph():
            y_samp = csp.curve(typ=float, data=[
                (st + timedelta(seconds=1), 1), (st + timedelta(seconds=2), 2), (st + timedelta(seconds=3), 3),
                (st + timedelta(seconds=5), 4), (st + timedelta(seconds=8), 5), (st + timedelta(seconds=9), 6),
                (st + timedelta(seconds=10), 7)
            ])
            sampler = csp.curve(typ=float, data=[
                (st + timedelta(seconds=i + 1), True) for i in range(10)
            ])
            ...
            s_ig_t = csp.stats.sum(y_samp, timedelta(seconds=3), timedelta(seconds=3), precise=True, ignore_na=True, sampler=sampler)
            ...
            mx_wn_t = csp.stats.max(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=False, sampler=sampler)
            csp.add_graph_output('mx_ig_n', mx_ig_n)
            csp.add_graph_output('mx_wn_n', mx_wn_n)
            csp.add_graph_output('mx_ig_t', mx_ig_t)
            csp.add_graph_output('mx_wn_t', mx_wn_t)
            ...
            mu_ig_t = csp.stats.mean(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=True, sampler=sampler)
            mu_wn_t = csp.stats.mean(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=False, sampler=sampler)
            csp.add_graph_output('mu_ig_n', mu_ig_n)
            csp.add_graph_output('mu_wn_n', mu_wn_n)
            csp.add_graph_output('mu_ig_t', mu_ig_t)
            csp.add_graph_output('mu_wn_t', mu_wn_t)
            ...
            std_ig_t = csp.stats.stddev(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=True, sampler=sampler)
            std_wn_t = csp.stats.stddev(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=False, sampler=sampler)
            csp.add_graph_output('std_ig_n', std_ig_n)
            csp.add_graph_output('std_wn_n', std_wn_n)
            csp.add_graph_output('std_ig_t', std_ig_t)
            csp.add_graph_output('std_wn_t', std_wn_t)
            ...
            med_ig_n = csp.stats.median(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=True, sampler=sampler)
            med_wn_n = csp.stats.median(y_samp, timedelta(seconds=3), timedelta(seconds=3), ignore_na=False, sampler=sampler)
            csp.add_graph_output('qt_ig_n0', qt_ig_n[0])
            csp.add_graph_output('qt_ig_n1', qt_ig_n[1])
            csp.add_graph_output('qt_ig_n2', qt_ig_n[2])
            csp.add_graph_output('qt_wn_n0', qt_wn_n[0])
    ...
    def test_bivariate_out_of_seq_nan_handling(self):
        ...
        def g():
            x = csp.curve(typ=float, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), x_val) for i, x_val in enumerate(x_values)])
            y = csp.curve(typ=float, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), y_val) for i, y_val in enumerate(y_values)])
            w = csp.curve(typ=float, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), weight) for i, weight in enumerate(weights)])
            w_np = csp.curve(typ=np.ndarray, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), np.array([weight, weight], dtype=float)) for i, weight in enumerate(weights)])
            xy = csp.curve(typ=np.ndarray, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), np.array([x_val, y_values[i]], dtype=float)) for i, x_val in enumerate(x_values)])
            yx = csp.curve(typ=np.ndarray, data=[(datetime(2020, 1, 1)+timedelta(seconds=i), np.array([y_val, x_values[i]], dtype=float)) for i, y_val in enumerate(y_values)])
        ...
        # 1. cov and corr
        res = csp.run(g, starttime=datetime(2020, 1, 1), endtime=datetime(2020, 1, 1)+timedelta(seconds=len(x_values)))
    ...
    def test_resetter(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=
                [(st+timedelta(seconds=i + 1), i+1) for i in range(8)] +
                [(st+timedelta(seconds=9), float('nan')), (st+timedelta(seconds=10), 10)
            ])

            reset = csp.curve(typ=float, data=[
                (st+ timedelta(seconds=4.5), True), (st+ timedelta(seconds=4.7), True), (st+ timedelta(seconds=7.5), True), (st+ timedelta(seconds=9.5), True)
            ])
            ...
            trigger = csp.curve(typ=bool, data=[
                (st+ timedelta(seconds=4.5), True), (st+ timedelta(seconds=7.5), True), (st+ timedelta(seconds=9.5), True)
            ])
            sum_trig_t = csp.stats.sum(x, timedelta(seconds=3), timedelta(seconds=1), True, trigger=trigger, reset=reset)
            ...
            reset2 = csp.curve(typ=float, data=[
                (st+ timedelta(seconds=4), True), (st+ timedelta(seconds=7), True), (st+ timedelta(seconds=9), True)
            ])
            ...
            x_np = csp.curve(typ=np.ndarray, data=
                [(st+timedelta(seconds=i + 1), np.array([i+1, i+1], dtype=float)) for i in range(8)] +
                [(st+timedelta(seconds=9), np.array([float('nan'), float('nan')], dtype=float)), (st+timedelta(seconds=10), np.array([10, 10], dtype=float))
            ])
            ...
            reset3 = csp.curve(typ=bool, data=[
                (st + timedelta(seconds=3), True)
            ])
            trigger2 = csp.curve(typ=bool, data=[
            ...
            reset4 = csp.curve(typ=bool, data=[
                (st + timedelta(seconds=3), True), (st + timedelta(seconds=3.5), True), (st + timedelta(seconds=4), True)
            ])
            trigger3 = csp.curve(typ=bool, data=[
        ...
        expected_sum_trig = [(st+ timedelta(seconds=4.5), 0), (st+ timedelta(seconds=7.5), 0), (st+ timedelta(seconds=9.5), 0)]
    ...
    def test_engine_cycle_case(self):
        ...
        def source() -> csp.Outputs(value=csp.ts[float], np_val=csp.ts[np.ndarray], trigger=csp.ts[float]):
            ...
            if csp.ticked(alarm2):
                csp.output(np_val=np.array([0.])) # so shape is known
        ...
        def graph() -> csp.ts[float]:
            src = source()
            sum_t = csp.stats.sum(src.value, timedelta(seconds=3), min_window=timedelta(seconds=0.5), trigger=src.trigger)
            np_sum_t = csp.stats.sum(src.np_val, timedelta(seconds=3), min_window=timedelta(seconds=0.5), trigger=src.trigger)
        ...
        np.testing.assert_equal(results['sum_n'][1], [0., 1., 2., 3., 3., 3., 3., 3.])
        np.testing.assert_equal(results['sum_t'][1], [0., 1., 2., 2., 2., 2., 2., 2.])
        exp_np = [[0.], [1.], [2.], [2.], [2.], [2.], [2.], [2.]]
        for i in range(8):
    ...
    def test_error_messages(self):
        ...
        @csp.graph
        def graph():
            s = csp.curve(typ=str, data=
                [(st+timedelta(seconds=i + 1), "i") for i in range(8)])
    ...
    def test_numpy_first_last_prod_unique_gmean(self):
        ...
        @csp.graph
        def graph():
            x_np = csp.curve(typ=Numpy1DArray[float], data=[
            ...
            # unique
            x_np2 = csp.curve(typ=Numpy1DArray[float], data=[
                (st+ timedelta(seconds=1), np.array([[1,1],[1,1]], dtype=np.float64)),
                (st+ timedelta(seconds=2), np.array([[2,2],[2,2]], dtype=np.float64)),
                (st+ timedelta(seconds=3), np.array([[float('nan'), float('nan')],[float('nan'), float('nan')]], dtype=np.float64)),
        ...
        np.testing.assert_almost_equal(expected_unq, list_nparr_to_matrix(np.array(results['unq_t'], dtype=object)), decimal=7)
    ...
    def test_numpy_min_max_qtl_median(self):
        ...
        def graph():
            x_np = csp.curve(typ=np.ndarray, data=[
                (st+ timedelta(seconds=i+1), data[i])
                for i in range(100)]
            )
    ...
    def test_numpy_var_std_sem_skew_kurt(self):
        ...
        def graph():
            ...
            weights = csp.curve(typ=np.ndarray, data=[
                (st+ timedelta(seconds=i+1), np.full((2,2),i+1, dtype=float))
                for i in range(100)]
            )
    ...
    def test_numpy_ewm(self):
        ...
        def graph():
            x = csp.curve(typ=np.ndarray, data=[
                (st + timedelta(seconds=j + 1),
                np.array([[elems[0][j], elems[1][j]], [elems[2][j], elems[3][j]]])
                ) for j in range(100)])
        ...
        for i in range(98):
            np.testing.assert_almost_equal(expected_std_ema[i,:,:], np.array(results['ema'][i][1], dtype=object), decimal=7)
            np.testing.assert_almost_equal(expected_adjusted_ema[i,:,:], np.array(results['ema_adj'][i][1], dtype=object), decimal=7)
            np.testing.assert_almost_equal(expected_stddev_ema[i,:,:], np.array(results['ema_std'][i][1], dtype=object), decimal=7)
            np.testing.assert_almost_equal(expected_time_ema[i,:,:], np.array(results['ema_time'][i][1], dtype=object), decimal=7)
            np.testing.assert_almost_equal(expected_time_ema_var[i,:,:], np.array(results['ema_var_time'][i][1], dtype=object), decimal=7)
    ...
    def test_unnatural_strides(self):
        ...
        def graph():
            ...
            x_np = csp.curve(typ=np.ndarray, data=[
                (st+ timedelta(seconds=1), x1),
                (st+ timedelta(seconds=2), x1.T)
            ])
            x_np2 = csp.curve(typ=np.ndarray, data=[
                (st+ timedelta(seconds=1), x2[:,0]),
                (st+ timedelta(seconds=2), x2[:,1])
            ])

            # add dimension of 0
            x_np3 = csp.curve(typ=np.ndarray, data=[
            ...
            bts = broadcast_to(csp.sample(csp.timer(timedelta(seconds=1)), csp.const(np.array([1.0, 2.0, 3.0]))), (3, 3))
            bts2 = broadcast_to(csp.sample(csp.timer(timedelta(seconds=1)), csp.const(np.array([1.0, 2.0, 3.0]))), (2, 3, 3))
        ...
        exp_arr2 = [ np.array([1,0,0], dtype=float).reshape(3,), np.array([1,1,0], dtype=float).reshape(3,)]
    ...
    def test_listbasket(self):
        ...
        def graph():
            ...
            x_list1 = csp.curve(typ=float, data=[
                (st+ timedelta(seconds=1), 1),
                (st+ timedelta(seconds=2), 2)
            ])
            x_list2 = csp.curve(typ=float, data=[
                (st+ timedelta(seconds=1), 3),
                (st+ timedelta(seconds=2), 4)
            ])
    ...
    def test_elementwise_nan_handling(self):
        ...
        def graph():
            x = csp.curve(typ=np.ndarray, data=[
                (st+ timedelta(seconds=i+1), data[i]) for i in range(100)
            ])
    ...
    def test_min_data_points(self):
        ...
        def graph():
            x = csp.curve(typ=float, data=[
                (st+ timedelta(seconds=i+1), data[i]) for i in range(10)
            ])
        ...
        np.testing.assert_almost_equal(exp_result, np.array(results['mean'], dtype=object)[:,1].astype(float), decimal=7)
        np.testing.assert_almost_equal(exp_result, np.array(results['median'], dtype=object)[:,1].astype(float), decimal=7)
        np.testing.assert_almost_equal(exp_result, np.array(results['max_t'], dtype=object)[:,1].astype(float), decimal=7)
        np.testing.assert_almost_equal(exp_std, np.array(results['stddev'], dtype=object)[:,1].astype(float), decimal=7)
    ...
    def test_rank_argmin_argmax(self):
        ...
        dval_np = 100*np.random.normal(size=(100,4,))
        dval2 = [1,1,1,1,2,1]

        # more data for testing rank
        dval3 = [10,1,5,6,5,6,5,6,1.2,2.4]*10
        dval_np3 = np.zeros(shape=(100,4,))
        for i in range(100):
            for j in range(4):
        ...
        def graph():
            ...
            x = csp.curve(typ=float, data=[
                (st+timedelta(seconds=i+1), dval[i]) for i in range(100)
            ])

            x_np = csp.curve(typ=np.ndarray, data=[
                (st+timedelta(seconds=i+1), dval_np[i]) for i in range(100)
            ])

            x2 = csp.curve(typ=float, data=[
                (st+timedelta(seconds=i+1), dval2[i]) for i in range(6)
            ])

            x3 = csp.curve(typ=float, data=[
                (st+timedelta(seconds=i+1), dval3[i]) for i in range(100)
            ])

            x3_np = csp.curve(typ=np.ndarray, data=[
                (st+timedelta(seconds=i+1), dval_np3[i]) for i in range(100)
            ])
        ...
        for i in range(96):
            ...
            exp_max_np = np.array([st+timedelta(seconds=i+1+int(np.argmax(window_np, axis=0)[j])) for j in range(4)], dtype=np.datetime64)
            exp_min_np = np.array([st+timedelta(seconds=i+1+int(np.argmin(window_np, axis=0)[j])) for j in range(4)], dtype=np.datetime64)
        ...
        for i in range(96):
            ...
            # numpy
            np.testing.assert_equal(np.array([exp_rank_min[i] for j in range(4)], dtype=object), results['rank_min_np'][i][1])
            np.testing.assert_equal(np.array([exp_rank_avg[i] for j in range(4)], dtype=object), results['rank_avg_np'][i][1])
            np.testing.assert_equal(np.array([exp_rank_max[i] for j in range(4)], dtype=object), results['rank_max_np'][i][1])
        ...
        def graph_rank():
            ...
            x_np = csp.curve(typ=np.ndarray, data=(np.array([st+timedelta(x) for x in range(len(_x))]), np.expand_dims(_x, axis=0).T))
    ...
    def test_cross_sectional(self):
        ...
        def graph():
            ...
            x_np = csp.curve(typ=np.ndarray, data=[
                (st+timedelta(seconds=i), np.array([i, i], dtype=float)) for i in range(1,101)
            ])
        ...
        exp_cs_np = [(st+timedelta(seconds=i), [np.array([j,j]) for j in range(i+1-10,i+1)]) for i in range(51,101)]
        exp_mean = [(st+timedelta(seconds=i), sum([j for j in range(i+1-10,i+1)])/10) for i in range(10,101)]
        exp_np_mean = [(st+timedelta(seconds=i), np.array([sum([j for j in range(i+1-10,i+1)])/10, sum([j for j in range(i+1-10,i+1)])/10])) for i in range(10,101)]

        exp_cs_as_np = [(st+timedelta(seconds=i), np.array([j for j in range(i+1-10,i+1)], dtype=float)) for i in range(10,101)]
        exp_cs_np_as_np = [(st+timedelta(seconds=i), np.array([[j,j] for j in range(i+1-10,i+1)], dtype=float)) for i in range(51,101)] # 10x2 array
        ...
        def graph3():
            td = timedelta()
            x_np = csp.curve(csp.typing.Numpy1DArray[float], [(td, np.array([1., 1., 1.])), (td, np.array([2., 2., 2.])), (td, np.array([3., 3., 3.]))])
            x_f = csp.curve(float, [(td, 1.), (td, 2.), (td, 3.)])
        ...
        numpy.testing.assert_array_equal([v[1] for v in results['x_np'] ], [[np.array([1., 1., 1.]), np.array([2., 2., 2.])],
                                                                            [np.array([2., 2., 2.]), np.array([3., 3., 3.])]])

        self.assertEqual([v[1] for v in results['x_f']], [ [1., 2.], [2., 3.]] )

        # Case where we're copying an empty window buffer
        data = csp.curve(typ=float, data=[(st, 1.0), (st+timedelta(seconds=4), 2.0), (st+timedelta(seconds=5), 3.0)])
        data_np = csp.curve(typ=np.ndarray, data=[(st, np.array([1.0, 2.0])),
                                                (st+timedelta(seconds=4), np.array([2.0, 3.0])),
                                                (st+timedelta(seconds=5), np.array([3.0, 4.0]))])

        trigger = csp.curve(typ=bool, data=[(st+timedelta(seconds=3), True),
        ...
        np.testing.assert_equal(res_float_list[0], [(st+timedelta(seconds=3), []),(st+timedelta(seconds=4), [2.0]),(st+timedelta(seconds=5), [2.0,3.0]),(st+timedelta(seconds=8), [])])
        np.testing.assert_equal(res_float_array[0], [(st+timedelta(seconds=3), np.array([], dtype=np.float64)),(st+timedelta(seconds=4), np.array([2.0], dtype=np.float64)),
                                                    (st+timedelta(seconds=5), np.array([2.0,3.0], dtype=np.float64)),(st+timedelta(seconds=8), np.array([], dtype=np.float64))])
        ...
        np.testing.assert_equal(res_np_as_list[0][2], (st+timedelta(seconds=5), [np.array([2.0, 3.0]), np.array([3.0, 4.0])]))
    ...
    def test_null_data(self):
        ...
        def graph2():
            ...
            x = csp.curve(typ=float, data=[
                ...
                (st+timedelta(seconds=6), 1)
            ])

            x_np = csp.curve(typ=np.ndarray, data=[
                (st+timedelta(seconds=1), np.array([1.,1.])),
                (st+timedelta(seconds=2), np.array([1.,1.])),
                (st+timedelta(seconds=6), np.array([1.,1.]))
            ])
        ...
        exp_numpy_zero = [(st+timedelta(seconds=5), np.array([0., 0.]))]
    ...
    def test_recalc(self):
        ...
        def g():
            ...
            nodes = {'mean_nr': mean_nr, 'sum_nr': sum_nr, 'var_nr': var_nr, 'ema_nr': ema_nr, 'mean_recalc': mean_recalc, \
                    'sum_recalc': sum_recalc, 'var_recalc': var_recalc, 'ema_recalc': ema_recalc}
        ...
        for j in range(5):
            ...
            for i, node in enumerate(recalc_nodes_f):
                ...
                np.testing.assert_almost_equal(results[node][idx][1], results[no_recalc_nodes_f[i]][idx][1], decimal=6) # close to the un-recalculated value

            np.testing.assert_equal(results['var_recalc'][idx][1], np.array([0,0,0], dtype=float))
            np.testing.assert_almost_equal(results['var_recalc'][idx][1], results['var_nr'][idx][1], decimal=6) # close to the un-recalculated value

            np.testing.assert_equal(results['ema_recalc'][idx][1], np.array([0,0,0], dtype=float))
            np.testing.assert_almost_equal(results['ema_recalc'][idx][1], results['ema_nr'][idx][1], decimal=6) # happens to be the same due to defaults
        ...
        def g():
            ...
            x_np = csp.curve(typ=np.ndarray, data=[(st+timedelta(seconds=i+1), np.array([val[i], val[i]], dtype=float)) for i in range(20)]) # 2x1
        ...
        for i in range(2,20):
            ...
            np.testing.assert_almost_equal(results['corr_matrix_n'][i][1], np.array([ [1,1], [1,1] ], dtype=float), decimal=7)
            np.testing.assert_almost_equal(results['corr_matrix_t'][i][1], np.array([ [1,1], [1,1] ], dtype=float), decimal=7)
