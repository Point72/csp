...
class TestTypeChecking(unittest.TestCase):
    ...
    def test_runtime_type_check(self):
        ...
        with self.assertRaisesRegex(TypeError,
                                    '"typed_int" node expected output type on output #0 to be of type "int" got type "str"'):
            csp.run(typed_int, csp.const('5'), starttime=datetime(2020, 2, 7))
        ...
        # Invalid
        with self.assertRaisesRegex(TypeError,
                                    '"typed_list" node expected output type on output #0 to be of type "list" got type "str"'):
    ...
    def test_primitive_to_obj_casting(self):
        ...
        def graph():
            ...
            with self.assertRaisesRegex(TypeError, "Conflicting type resolution for T when calling to typed_scalar_two_args : " +
    ...
    def test_dict_type_resolutions(self):
        ...
        @csp.node
        def deep_nested_generic_resolution2(x: 'T1', y: 'T2', z: typing.Dict['T1', typing.Dict['T2', typing.List[typing.Set['T1']]]]):
            pass
        ...
        def graph():
            ...
            for f in (deep_nested_generic_resolution, deep_nested_generic_resolution2):
                f(TestTypeChecking.Dummy, TestTypeChecking.Dummy2,
                ...
                f(TestTypeChecking.Dummy, TestTypeChecking.Dummy2,
                  {TestTypeChecking.Dummy(): {TestTypeChecking.Dummy2(): [{TestTypeChecking.Dummy()},
            ...
            with self.assertRaisesRegex(TypeError, "Conflicting type resolution for T when calling to typed_ts_and_scalar_generic .*"):
                # Passing a Dummy value instead of expected float
                typed_ts_and_scalar_generic(d_i_i, {1: 2.0}, TestTypeChecking.Dummy())

            with self.assertRaisesRegex(TypeError, "Conflicting type resolution for T1 when calling to deep_nested_generic_resolution : "
                                                   ".*"):
                # Here for inernal sets we pass Dummy and Dummy3 - they result in conflicting type resolution for T1
                deep_nested_generic_resolution(TestTypeChecking.Dummy, TestTypeChecking.Dummy2,
                                               {TestTypeChecking.Dummy(): {TestTypeChecking.Dummy2(): [{TestTypeChecking.Dummy()},
                                                                                                       {TestTypeChecking.Dummy3()}]}}
                                               )
            l_good = csp.const.using(T={int: float})({})
            with self.assertRaisesRegex(TypeError, "Unable to resolve container type for type variable T explicit value must have" +
    ...
    def test_typed_to_untyped_container(self):
        ...
        def main():
            g(d=csp.const.using(T=typing.Dict[int, int])({}),
