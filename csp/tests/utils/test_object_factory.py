...
from csp.utils.object_factory_registry import AutoInjectable, Injected, ObjectFactoryRegistry, auto_inject, get_injected_value, injected_property, register_injected_object, \
...
class TestObjectFactory(unittest.TestCase):
    ...
    def test_injected_function_values(self):
        ...
        with set_new_registry_thread_instance():
            ...
            self.assertEqual(f(), 42)
            self.assertEqual(f(), 42)
            register_injected_provider('injected_y', self.get_running_index_func(3), singleton=True, allow_override=True)
