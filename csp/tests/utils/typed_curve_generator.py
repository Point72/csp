...
class TypedCurveGenerator(object):
    ...
    def _generate_int_values(self, start_value, num_cycles, increment, duplicate_timestamp_indices, output_on_initial_cycle, skip_indices):
    ...
    def _transform_int_to_datetime(self, value):
        ...
        return datetime.datetime(year=1950 + hashed_value % 100, month=hashed_value % 12 + 1, day=hashed_value % 30 + 1,
                                 hour=hashed_value % 24, minute=hashed_value % 60, second=hashed_value % 60,
    ...
    def _transform_int_to_timedelta(self, value):
        hashed_value = hash(str(value))
        return datetime.timedelta(days=hashed_value % 23, seconds=hashed_value % 90, microseconds=hashed_value % 10000001,
                                  milliseconds=hashed_value % 1000001, minutes=hashed_value % 101, hours=hashed_value % 557,
    ...
    def gen_int_curve(self, start_value, num_cycles, increment, skip_indices=None, output_on_initial_cycle: bool = True,
                      duplicate_timestamp_indices=None):
        values = self._generate_int_values(start_value, num_cycles, increment, duplicate_timestamp_indices, output_on_initial_cycle, skip_indices)
