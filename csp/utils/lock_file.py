...
from csp.utils.file_permissions import (
    FilePermissions,
...
class _TimeoutMonitor:
    ...
    def timed_out(self):
        ...
        if self._timed_out is None:
            ...
            self._timed_out = (
                datetime.datetime.now() - self._init_time
...
class LockFile:
    ...
    def __init__(
        ...
        self._retry_period_seconds = (
            ...
            if retry_period_seconds is not None
            else self.DEFAULT_RETRY_PERIOD_SECONDS
        )
        self._shared = shared
    ...
    def csp_lock_for_folder(cls, folder_path, shared=True):
        return LockFile(
            os.path.join(folder_path, cls._CSP_LOCK_FILE_NAME), shared=shared
        )
    ...
    def lock(self):
        ...
        with _LockWrapper(self._LOCKS_DICT_GUARD) as locks_dict_guard:
            ...
            while self._lock_record is None:
                ...
                if existing_lock_record is not None and (
                    not self._shared or not existing_lock_record.shared
                ):
                    if timed_out:
                    ...
                    lock_file = self._try_open_and_lock_file(
                        self._file_path, self._shared, self._file_permissions
                    )
                    if lock_file is not None:
                        ...
                        with self._lock_record.thread_lock:
                            ...
                            self._cur_object_lock_count = 1
                            assert (
                                self._lock_record.ref_count
                                == self._cur_object_lock_count
                            )
                            return
                    elif timed_out:
                        raise BlockingIOError(
...
class MultipleFilesLock:
    def __init__(
        self, file_paths_or_locks, shared: bool = False, remove_on_unlock=True
    ):
