...
class ObjectFactoryRegistry:
    ...
    def set_object_factory(self, key, value, allow_override=False):
        ...
        if (
            not allow_override
            and self._get_object_factory_impl(key, default_value=None) is not None
        ):
            raise RuntimeError(
                f"Trying to override existing key {key} without allow_override set"
            )
    ...
    def get_object_factory(self, key, default_value=UNSET):
        ...
        if isinstance(res, ObjectFactoryRegistry):
            raise KeyError(
                f"Trying to access item via partial path specification '{key}'"
            )
        return res
    ...
    def set_new_thread_instance(cls, inherit_from_parent=True):
        ...
        with cls.set_existing_thread_instance(
            ObjectFactoryRegistry(parent)
        ) as instance:
            yield instance
...
def register_injected_object(key, value, allow_override=False):
    ...
    :return:
    """
    ObjectFactoryRegistry.instance().set_object_factory(
        key, lambda: value, allow_override
    )
...
def get_injected_value(key, *args, default_value=UNSET, **kwargs):
    ...
    if default_value is UNSET:
        ...
        provider = ObjectFactoryRegistry.instance().get_object_factory(
...
class Injected:
    ...
    def __str__(self):
        ...
        if self._kwargs:
            kwargs_str = ',' + ','.join(
                f'{k}={str(v)}' for k, v in self._kwargs.items()
            )
        else:
            kwargs_str = ''
...
def auto_inject(f):
    ...
    if spec.defaults:
        ...
        for i, (arg, default_val) in enumerate(
            zip(spec.args[skipped_args:], spec.defaults)
        ):
            if isinstance(default_val, Injected):
                injected_by_index.append(
                    _InjectedArg(i + skipped_args, arg, default_val)
                )
    ...
    def _impl(*args, **kwargs):
        ...
        while positional_index < len(injected_by_index) and injected_by_index[
            positional_index
        ].index < len(args):
            positional_index += 1
...
def injected_property(
    func=None, *args, inject_key=None, read_only=True, default_value=UNSET, **kwargs
):
    ...
    if func is not None:
        ...
        ), "func and default_value can't be set at the same time"
    ...
    def _impl(func):
        ...
        if not read_only:
            res = res.setter(
                lambda self, value: prop_value_container.set_value(self, value)
            )
        return res
    ...
    if func is not None:
        assert (
            inject_key is None
        ), "inject_key can't be provided with implementation function"
        assert (
            not args
        ), "Invalid call to injected_property, when positional args are provided, inject_key must be first positional argument"
        return _impl(func)
