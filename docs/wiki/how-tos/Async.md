...
### Option 2: CSP Asyncio Mode

**New in recent versions:** Run CSP with `use_asyncio=True` to execute the graph inside an asyncio event loop. This allows async operations to run directly on CSP's engine loop, eliminating the need for a separate background thread:
...
csp.run(my_graph, realtime=True, endtime=timedelta(seconds=5), use_asyncio=True)
```
...
| ------------------------------------------- | ------------------------------------------------ |
| Simple async I/O, default behavior          | **Default** (no `loop` parameter)                |
| Heavy async operations, want efficiency     | **CSP Asyncio Mode** (`use_asyncio=True`)        |
...
1. If CSP is running with `use_asyncio=True`, use that loop
...
- The async generator runs in a background thread (or on CSP's asyncio loop if `use_asyncio=True`)
...
### Running with CSP Asyncio Mode

For better efficiency, especially with heavy async operations, run with `use_asyncio=True`:

```python
if __name__ == "__main__":
    # Run in asyncio mode - all async operations use the same loop as CSP
    csp.run(main_graph, realtime=True, endtime=timedelta(seconds=2), use_asyncio=True)
```
...
1. **Consider `use_asyncio=True` for async-heavy graphs**: When your graph has many async operations, running with `use_asyncio=True` eliminates cross-thread overhead and simplifies coordination.
...
1. **Realtime mode required**: Async adapters only work in realtime mode (`realtime=True`). If you use `use_asyncio=True`, it automatically implies realtime mode.
