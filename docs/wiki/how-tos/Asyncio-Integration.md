...
1. **Asyncio Mode in csp.run()**: Run CSP graphs with `use_asyncio=True` for direct asyncio integration
...
The `csp.run()` function provides:

- **`use_asyncio=True`**: Run the CSP engine in step-based mode inside an asyncio event loop
...
## Part 0: CSP Run with Asyncio Mode

The simplest way to combine CSP graphs with asyncio is using `csp.run()` with `use_asyncio=True`:
...
csp.run(my_graph, realtime=True, endtime=timedelta(seconds=2), use_asyncio=True)
```

### How It Works

When you call `csp.run(..., use_asyncio=True)`:

1. CSP creates a new asyncio event loop
1. The CSP engine runs in step-based mode, yielding control to the asyncio loop between steps
1. All async adapters (`async_for`, `async_in`, `async_out`, etc.) detect this mode and use CSP's asyncio loop directly
1. No background thread is needed for async operations
...
- `use_asyncio=True` requires `realtime=True`
...
| Feature            | `csp.run(use_asyncio=True)`     | `CspEventLoop`                        | `AsyncioBridge`                |
| ------------------ | ------------------------------- | ------------------------------------- | ------------------------------ |
...
1. **I want async adapters to work without extra configuration:**
   → Use `csp.run(..., use_asyncio=True)` - adapters auto-detect the loop
